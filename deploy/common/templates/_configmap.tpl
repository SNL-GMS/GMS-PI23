...
{{- define "gms.common.configMaps.copy" }}
  {{- if .Values.copyConfigMaps }}
    {{- range $key, $configDef := .Values.copyConfigMaps }}
      ...
      {{- end }}
      {{- include "gms.common.configMap.copy" (dict "context" $ "namespace" $configDef.namespace "source_name" $key "dest_name" ($configDef.destName | default $key) "optional" ($configDef.optional | default false)) }}
    {{- end }}
  {{- end }}
{{- end }}
