...
def config_services_available() -> bool:
    ...
    try:
        response = requests.get(
            f'http://{current_app.config["OSD_SERVICE_NAME"]}:8080/'
            f'{ current_app.config["OSD_SERVICE_NAME"] }/alive',
            timeout=1
        )
        if response.status_code == 200:
            ...
            logger.info(
                f'{current_app.config["OSD_SERVICE_NAME"]} '
                "not alive... waiting"
            )
    ...
    return (
        processing_config_service_ready and osd_service_ready
...
def dataload(
    ...
    try:
        ...
        osd_url = (
            f'http://{current_app.config["OSD_SERVICE_NAME"]}:8080/'
            f'{current_app.config["OSD_SERVICE_NAME"] }/osd'
        )
        ...
        # this must always be done AFTER the stations processing data
        # has been loaded
