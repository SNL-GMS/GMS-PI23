...
def test_upgrade_soh_livedata(mocker, capsys):
    ...
    mocker.patch(
        'gmskube.gmskube.delete_unmanaged_resources',
        return_value=None
    )
...
def test_upgrade_with_augmentations(mocker, capsys):
    mocker.patch(
        'gmskube.gmskube.get_instance_labels',
        return_value={'gms/type': 'soh'}
    )
...
def test_upgrade_helm_upgrade_fail(mocker, capsys):
    ...
    mocker.patch(
        ...
        return_value=(1, "", "helm upgrade failed")
    )  # yapf: disable
    mocker.patch(
        'gmskube.gmskube.delete_unmanaged_resources',
        return_value=None
...
def test_list(mocker, capsys):
    ...
    out, err = capsys.readouterr()
    assert "fleet-agent-local" not in out
    for line in [
...
def test_list_show_all(mocker, capsys):
    ...
    for line in [
        ...
        "2021-12-20T210438Z   8041,9000-9090   develop"
    ]:
        assert line in out
...
def test_list_name(mocker, capsys):
    mocker.patch(
        'gmskube.gmskube.run_helm_list',
        return_value=(
            0,
...
def test_list_name_not_exist(mocker, capsys):
    mocker.patch(
        'gmskube.gmskube.run_helm_list',
        return_value=(
            0,
    ...
    out, err = capsys.readouterr()
    assert ('Instance name "doesnotexist" does not exist') in out


def test_list_helm_list_fail(mocker, capsys):
    mocker.patch(
        'gmskube.gmskube.run_helm_list',
        return_value=(1, "", "helm list failed")
    )  # yapf: disable
...
def test_list_kubectl_get_configmap_all_namespaces_fail(mocker, capsys):
    ...
    mocker.patch(
        'gmskube.gmskube.run_kubectl_get_configmap_all_namespaces',
        return_value=(1, "", "get configmap fail")
    )  # yapf: disable
...
def test_ingress_istio(mocker, capsys):
    ...
    gmskube.ingress(instance_name='test', ingress_port=None, service=None)

    out, err = capsys.readouterr()
    assert out.count('\n') == 18
    assert (
        'cache-service                                                  '
        'https://test.cluster.com:8443/cache-service'
    ) in out
    assert (
        'prometheus                                                     '
    ...
    assert (
        ...
        'https://test.cluster.com:8443/reactive-interaction-gateway/api/'
    ) in out
...
def test_ingress_service_istio(mocker, capsys):
    mocker.patch('gmskube.gmskube.is_instance_istio', return_value=True)
    mocker.patch('gmskube.gmskube.get_ingress_port', return_value='8443')
    mocker.patch(
        'gmskube.gmskube.run_kubectl_get',
    ...
    out, err = capsys.readouterr()
    assert out.count('\n') == 1
    assert ('https://test.cluster.com:8443/cache-service') in out
    assert (
        'cache-service                                                  '
    ) not in out


def test_ingress_service_non_istio(mocker, capsys):
    mocker.patch('gmskube.gmskube.is_instance_istio', return_value=False)
    mocker.patch('gmskube.gmskube.get_ingress_port', return_value='443')
    mocker.patch(
        'gmskube.gmskube.run_kubectl_get',
    ...
    assert out.count('\n') == 1
    assert (
        'https://test-develop.cluster.gms.domain.com:443/cache-service'
    ) in out
    assert (
        'cache-service                                                  '
    ) not in out
...
def test_get_kubernetes_version_1_20(mocker):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_version',
        return_value=(
            0,
            get_test_file_contents('cmds/run_kubectl_version_1_20.json'),
            ""
        )
    )
    gmskube.get_kubernetes_version()

    assert '/opt/kubectl_1.20' in os.getenv('PATH')
    assert '/opt/helm_3.8' in os.getenv('PATH')


def test_get_kubernetes_version_1_24(mocker):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_version',
        return_value=(
            0,
            get_test_file_contents('cmds/run_kubectl_version_1_24.json'),
            ""
        )
    )
    gmskube.get_kubernetes_version()

    assert '/opt/kubectl_1.24' in os.getenv('PATH')
    assert '/opt/helm_3.12' in os.getenv('PATH')


def test_get_kubernetes_version_not_supported(mocker, capsys):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_version',
        return_value=(
            0,
            get_test_file_contents('cmds/run_kubectl_version_1_19.json'),
            ""
        )
    )
    with pytest.raises(SystemExit):
    ...
    out, err = capsys.readouterr()
    assert 'Only kubernetes versions 1.20-1.27 are supported' in out
    assert '1.19 detected' in out


def test_get_kubernetes_version_fail(mocker, capsys):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_version',
        return_value=(1,
                      '',
    ...
    assert 'run kubectl version failed' in out
...
def test_get_livedata_set_args():
    ...
    assert result[5] == (
        'da-connman.env.GMS_CONFIG_CONNMAN__'
        'DATA_MANAGER_IP_ADDRESS=192.168.1.1'
    )
    assert result[7] == (
        'da-connman.env.GMS_CONFIG_CONNMAN__'
        'DATA_PROVIDER_IP_ADDRESS=192.168.1.2'
    )
...
def test_run_command_list(mocker, capsys):
    ...
    ret, out, err = (
        gmskube.run_command(shlex.split('echo "test"'), print_output=True)
    )
...
def test_create_namespace_no_rancher_project(mocker):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_create_namespace',
        return_value=(0, "", "")
    )  # yapf: disable
    mocker.patch(
        'gmskube.gmskube.run_kubectl_label_namespace',
    ...
    mocker.patch('gmskube.gmskube.get_rancher_project_id', return_value=(None))
...
def test_create_namespace_rancher_project(mocker, capsys):
    ...
    mocker.patch(
        'gmskube.gmskube.get_rancher_project_id',
        return_value=('local:abc123')
    )
    mocker.patch(
        'gmskube.gmskube.run_kubectl_annotate_namespace',
        return_value=(0, "", "")
    )  # yapf: disable
...
def test_delete_unmanaged_resources(mocker):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_delete_unmanaged_resource',
        return_value=(0, "", "")
    )  # yapf: disable
...
def test_delete_unmanaged_resources_kubectl_delete_unmanaged_resource_fail(
    mocker
):
    mocker.patch(
        'gmskube.gmskube.run_kubectl_delete_unmanaged_resource',
