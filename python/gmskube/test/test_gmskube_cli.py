...
def test_main(mocker, monkeypatch):
    ...
    mocker.patch('gmskube.gmskube.get_kubernetes_version', return_value=None)
...
def test_main_no_command(mocker, monkeypatch):
    mocker.patch(
        'gmskube.gmskube_cli.get_args',
        return_value=argparse.Namespace(verbose='DEBUG')
    )
    mocker.patch('logging.basicConfig', return_value=None)
    mocker.patch('builtins.open', new_callable=mocker.mock_open)
    mocker.patch('os.chmod', return_value=None)
...
def test_get_args_name_fail(mocker):
    mocker.patch(
        'argparse.ArgumentParser.parse_args',
        return_value=argparse.Namespace(name="Te$t")
    )
    parser = gmskube_cli.get_parser()
...
def test_argparse_tag_name_type():
    ret = gmskube_cli.argparse_tag_name_type(
        '-MyBranchName_1234567890123456789012345678901234567890'
    )
...
def test_argparse_set_type_fail():
    ...
    assert 'When specifying `--set`, you must supply helm chart name/value '
    'pair as: `Name=Value`' in ex.value.args[0]
...
def test_argparse_set_type_env_fail():
    ...
    assert 'The old top-level `env` has been replaced with '
    '`global.env`' in ex.value.args[0]
...
def test_argparse_ip_address_type_pass():
    gmskube_cli.argparse_ip_address_type(
        '192.168.1.1'
    )  # NOSONAR: ip address for testing
