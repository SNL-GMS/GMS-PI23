...
def install(
    ...
    :param connman_data_manager_ip: external ip address where dataman will
       be listening
    :param connman_data_provider_ip: external ip address where data will be
       sending from
    ...
    :param augmentations: list of augmentations to apply during install
    :return: None
    """
    if dry_run:
        global console
    ...
    # Set the instance type to be custom if loading in a user-defined custom
    # chart directory
    ...
    install_cmd = shlex.split(
        f'helm {"install" if dry_run < 2 else "template"} {instance_name} '
        f'{instance_type} '
        ...
        f'--set "elasticsearch.image='
        f'{docker_registry}/gms-common/logging-elasticsearch" '
        f'--set "elasticsearch.imageTag={image_tag}" '
        f'--set "elasticsearch.ingress.hosts[0]=elasticsearch.{base_domain}" '
        f'--set "elasticsearch.ingress.tls[0].hosts[0]='
        f'elasticsearch.{base_domain}" '
        # note helm does not merge lists so we have to set the secretName
        #     (https://github.com/helm/helm/issues/5711)
        f'--set "elasticsearch.ingress.tls[0].secretName=ingress-default-'
        'cert" '
        ...
        'keycloak" '
        f'--set "keycloakx.image.tag={image_tag}" '
        f'--set "postgresql.image.tag={image_tag}" '
        # These --set commands are for KEYCLOAK --END
    )

    if wallet_path is not None:
        install_cmd.extend(
            shlex.split('--set "global.oracleWalletOverride=true"')
        )
    ...
    # add injector args
    install_cmd.extend(get_injector_set_args(injector, injector_dataset))
    ...
    # if dry-run just exit here since everything else after this point
    # requires a real install
    if dry_run:
    ...
    console.log(
        f'\nTo list ingress routes for this instance, run `gmskube ingress '
        f'{instance_name}`'
    )
...
def upgrade(
    ...
    :param livedata: live data enabled
    :param connman_port: incoming port for connman to listen on
    :param connman_data_manager_ip: external ip address where dataman will be
        listening
    :param connman_data_provider_ip: external ip address where data will be
        sending from
    :param dataman_ports: port range for dataman
    :param augmentations: list of augmentations to apply during upgrade
    :return: None
    """
    if dry_run:
        global console
    ...
    if custom_chart_path is not None:
        ...
        # get the instance type from the labels. We don't use args.type here
        # because we don't allow the type to be changed during upgrade.
        instance_type = (
            get_instance_labels(instance_name).get('gms/type', None)
        )  # yapf: disable

    # check if the instance type is none and error
    if instance_type is None:
        print_error(
            f'Could not determine the type for instance {instance_name}. '
            f'Possible causes:\n        - Instance is not installed. Check '
            f'by running `gmskube ls`.\n        - Instance was not installed '
            f'using `gmskube` and is missing metadata. Uninstall then use '
            f'`gmskube install`.'
        )
        sys.exit(1)
    console.log(f'Instance type is: {instance_type}')

    # delete helm unmanaged resources. This is a backwards compatibility that
    # can be removed in PI22
    if not dry_run:
    ...
    if return_code == 0:
        ...
        print_error(
            f'Unable to get existing values for instance {instance_name}: '
            f'{err}'
        )
        sys.exit(return_code)

    # Run helm upgrade
    # Provide the values file from the chart followed by the existing values,
    ...
    # add injector args
    upgrade_cmd.extend(get_injector_set_args(injector, injector_dataset))
...
def uninstall(instance_name: str, timeout: int) -> None:
    """
    Perform helm uninstall command, wait for pods to terminate, then delete
        the namespace
    ...
    # wait for resources created by helm to terminate since helm uninstall is
    # async
    ...
    while (
        ...
        return_code, out, err = (
            run_kubectl_get_all_helm_resources(namespace=instance_name)
        )
    ...
    return_code, out, err = run_kubectl_delete_namespace(
    ...
    if return_code == 0:
        ...
        print_error(
            f'{instance_name} uninstall unsuccessful, '
...
def reconfig(
    ...
    """
    Perform the instance reconfig command - run a reduced dataload, then
    ...
    return_code, out, err = run_kubectl_get_deployments_restart_after_reconfig(
        namespace=instance_name
    )
...
def list_instances(
    ...
    :param show_all: Show all helm installed instances, even if they do not
        contain gms metadata
    :param name: Optional name of the instance to list. If name is specified
        and the instance does not exist, a non-zero return code will be sent
    :return: None
    """
    ...
    # check if name was specified and does not exist in the list
    if name is not None and all(i['name'] != name for i in instances):
        print_error(f'Instance name "{name}" does not exist')
        sys.exit(1)
    ...
    for instance in instances:
        ...
        # Only display something in the CD11-PORTS for instances attached to
        # live data
        livedata = "-"
        if (
            labels.get('gms/cd11-live-data',
                       '') == 'true'  # pragma: no branch
            and labels.get('gms/cd11-connman-port')
            and labels.get('gms/cd11-dataman-port-start')
            and labels.get('gms/cd11-dataman-port-end')
        ):
            livedata = (
                f"{labels.get('gms/cd11-connman-port')},"
                f"{labels.get('gms/cd11-dataman-port-start')}-"
                f"{labels.get('gms/cd11-dataman-port-end')}"
            )
...
def ingress(instance_name: str, ingress_port: str, service: str) -> None:
    """
    ...
    :param service: the service name to get ingress route for
    :return:
    """
...
def augment_apply(
    ...
    if not augmentation_exists(augmentation_name):
        print_error(
            f'Augmentation `{augmentation_name}` is not a valid augmentation '
            'name.'
        )
        sys.exit(1)
    ...
    try:
        console.log(
            f"[cyan]Applying augmentation '{augmentation_name}' to "
            f"{instance_name}"
        )
        ...
        print_error(
            f"Failed to apply augmentation '{augmentation_name}' to "
            f"{instance_name}: {e}"
        )
        sys.exit(1)

    console.log(
        f"[bold green]Augmentation '{augmentation_name}' "
        f"successfully applied to {instance_name}"
    )
...
def augment_delete(
    ...
    try:
        ...
        print_error(
            f"Failed to delete augmentation '{augmentation_name}' from "
            f"{instance_name}: {e}"
        )
        sys.exit(1)

    console.log(
        f"[bold green]Augmentation '{augmentation_name}' successfully deleted "
...
def get_kubernetes_version() -> None:
    """
    Gets the version of the kubernetes cluster API and set the PATH
    """
    return_code, out, err = run_kubectl_version()
    if return_code != 0:
        print_error(
    ...
    major_version = version_data['serverVersion']['major']
    minor_version = version_data['serverVersion']['minor']
    kubernetes_version = f'{major_version}.{minor_version}'
    logging.debug(f'Kubernetes API version: {kubernetes_version}')
    helm_version = None
    if kubernetes_version in {"1.20", "1.21", "1.22", "1.23"}:
        helm_version = "3.8"
    elif kubernetes_version in {"1.24", "1.25", "1.26", "1.27"}:
        helm_version = "3.12"
    else:
        print_error(
            "Only kubernetes versions 1.20-1.27 are supported. "
            f"Version {kubernetes_version} detected."
        )
        sys.exit(1)

    # kubectl and helm versions must be installed in the docker image
    os.environ['PATH'] = f"/opt/kubectl_{kubernetes_version}:" \
        f"/opt/helm_{helm_version}:{os.getenv('PATH')}"
...
def request_dataload(
    ...
    :param base_domain: FQDN of the kubernetes cluster where the config-loader
        service is running
    :param instance_name: Instance name to perform the dataload on
    :param endpoint: HTTP service target endpoint. Default is 'load'.
    :param config_overrides: Directory path for configuration overrides.
        Default is None.
    :param timeout: Timeout in minutes to wait for config-loader to be alive,
        and for dataload to complete.
    :param port: The port for making the https request to the config-loader.
        If None, then the default https port is used.
    ...
    try:
        ...
        # format the url - must be https on kube cluster, and the requests CA
        # bundle env var must be set
        config_loader_url = (
            f"https://{instance_name}.{base_domain}:{port}/config-loader"
        )
        ...
        while time_waited < timeout_seconds:
            ...
            if time_waited >= timeout_seconds:  # pragma: no branch
                print_warning(
                    'Timed out waiting for config loader to be alive, will '
                    'attempt data load anyway'
                )
        ...
        if response_json is None:
            ...
            print_error(
                f'Timed out waiting for data load after {timeout} minutes, '
                'Exiting'
            )
            sys.exit(1)
...
def get_ingress_port(istio_enabled: bool, override_port: str) -> str:
    """
    Gets the port for ingress depending on istio enabled or the override port
        argument is given
...
def get_ingress_ports_config() -> dict[str, str]:
    """
    Gets the values from the ingress-ports-config config map in the gms
        namespace
...
def get_rancher_project_id() -> str | None:
    """
    Gets the values from the rancher-project-config config map in the gms
        namespace
...
def get_injector_set_args(injector: bool, injector_dataset: str) -> list[str]:
    """
    ...
    if injector_dataset is not None:
        cmd.extend(
            shlex.split(
                '--set "augmentation.cd11-injector.env.'
                f'CD11_INJECTOR_CONFIG_NAME={injector_dataset}"'
            )
        )
...
def get_livedata_set_args(
    ...
    :param connman_data_manager_ip: external IP address of the CD11 dataman
        service
    :param connman_data_provider_ip: IP address of the data provider sending
        data to the CD11 dataman service
    ...
    if connman_data_manager_ip is not None:
        cmd.extend(
            shlex.split(
                '--set "da-connman.env.GMS_CONFIG_CONNMAN__DATA_MANAGER_IP_'
                f'ADDRESS={connman_data_manager_ip}"'
            )
        )
    if connman_data_provider_ip is not None:
        cmd.extend(
            shlex.split(
                '--set "da-connman.env.GMS_CONFIG_CONNMAN__DATA_PROVIDER_IP_'
                f'ADDRESS={connman_data_provider_ip}"'
            )
        )
    if dataman_ports is not None:
        cmd.extend(
            shlex.split(
                '--set "da-dataman.dataPortStart='
                f'{dataman_ports.split("-")[0]}"'
            )
        )
        cmd.extend(
            shlex.split(
...
def create_namespace(
    ...
    # standard labels
    labels = (
        f'app.kubernetes.io/instance={namespace} '
    ...
    return_code, out, err = run_kubectl_label_namespace(
        namespace=namespace,
        label=labels
    )
...
def delete_unmanaged_resources(namespace: str) -> None:
    ...
    """
    run_kubectl_delete_unmanaged_resource(
        namespace,
        'secret',
...
def run_command(
    ...
    stdin: bool = None
) -> tuple[int, str]:  # yapf: disable
    """
    Execute the specified system command. This will always be executed with
        /deploy as the PWD.
    :param command: The command string or list to execute. It is best to build
        this with shlex.split(), but if you don't it will be done internally.
    :param print_output: If True, print the stdout from the command
    :param stdin: The optional "input" argument should be data to be sent to
        the child process, or None, if no data should be sent to the child.
    :return: A tuple of the command return code, stdout string, and stderr
        string
    """
    ...
    # For debugging, you can either create /deploy on your machine, create a
    # soft-link, or temporarily change this value
...
def print_warning(message: str) -> None:
    ...
    """
    console.log(fr'[bold yellow]\[WARNING] {message}')
...
def print_error(message: str) -> None:
    ...
    :param message: Message string to print
    """
    console.log(fr'[bold red]\[ERROR] {message}')
...
def is_valid_fqdn(s: str) -> bool:
    ...
    cannot start or end with hyphens (RFC 952) labels can start with numbers
    (RFC 1123) trailing dot is not allowed max length of ascii hostname
    including dots is 253 characters TLD (last label) is at least 2 characters
    and only ASCII letters we want at least 1 level above TLD.
    ...
    pat = re.compile(
        r'(?=^.{4,253}$)(^((?!-)[a-zA-Z0-9-]'
        r'{0,62}[a-zA-Z0-9]\.)+[a-zA-Z]{2,63}$)'
    )
    return bool(pat.match(s))


# External run commands - each command is its own function so it can be mocked
# out during unit test. These all have "pragma: no coverage" because they are
# always mocked out in unit tests. Also none of these have their own unit tests
# since they call external commands
...
def run_helm_install(
    command: str | list[str]
) -> tuple[int, str]:  # pragma: no coverage; yapf: disable
...
def run_kubectl_get(
    ...
    return run_command(
        f'kubectl --namespace {namespace} get {resource_type} {resource_name} '
        '--output json',
        print_output=False
    )
