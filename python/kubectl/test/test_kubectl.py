...
@patch("python.kubectl.kubectl.kubectl.KubeCtl.get_pods")
def test_all_pods_ready_error(mock_get_pods, status):
    mock_get_pods.return_value = f"pod-name 1/1 {status} 0 1m"
    with pytest.raises(RuntimeError):
        KubeCtl().all_pods_ready()


@pytest.mark.parametrize(
    "pod_table",
    ["", "pod-name 1/1 Pending 0 0m", "pod-name 0/1 Running 0 1m"]
)  # yapf: disable
@patch("python.kubectl.kubectl.kubectl.KubeCtl.get_pods")
def test_all_pods_ready_retry(mock_get_pods, pod_table):
    mock_get_pods.return_value = pod_table
    assert not KubeCtl().all_pods_ready()


@patch("python.kubectl.kubectl.kubectl.KubeCtl.get_pods")
def test_all_pods_ready_success(mock_get_pods):
    mock_get_pods.return_value = "pod-name 1/1 Running 0 1m"
    assert KubeCtl().all_pods_ready()
...
@patch("python.kubectl.kubectl.kubectl.KubeCtl.get_pods")
def test_all_pods_ready_retry_wrapper_retry(mock_get_pods, pod_table):
    mock_get_pods.return_value = pod_table
    with pytest.raises(TryAgain):
        KubeCtl()._all_pods_ready_retry_wrapper()


@patch("python.kubectl.kubectl.kubectl.KubeCtl.get_pods")
def test_all_pods_ready_retry_wrapper_success(mock_get_pods):
    mock_get_pods.return_value = """
pod-1 1/1 Running   0 1m
pod-2 0/1 Completed 0 1m
""".strip()
    assert KubeCtl()._all_pods_ready_retry_wrapper() is None


@pytest.mark.parametrize("status", ["Failed", "Unknown"])
@patch("python.kubectl.kubectl.kubectl.KubeCtl.get_pods")
def test_all_pods_ready_retry_wrapper_error(mock_get_pods, status):
    mock_get_pods.return_value = f"pod-name 1/1 {status} 0 1m"
    with pytest.raises(RuntimeError):
        KubeCtl()._all_pods_ready_retry_wrapper()
...
@patch(
    ...
    "_all_pods_ready_retry_wrapper"
)
def test_wait_for_all_pods_ready_timeout(mock__all_pods_ready_retry_wrapper):
    mock__all_pods_ready_retry_wrapper.side_effect = TryAgain
    timeout = 2
    start_time = time()
    with pytest.raises(RetryError):
        KubeCtl().wait_for_all_pods_ready(timeout=timeout)
...
def test_wait_for_all_pods_ready_retry_delay_max_attempts(
    mock__all_pods_ready_wrapper
):
    mock__all_pods_ready_wrapper.side_effect = TryAgain
    retry_delay = 2
    max_attempts = 2
    start_time = time()
    with pytest.raises(RetryError):
        KubeCtl().wait_for_all_pods_ready(
...
def test_wait_for_all_pods_ready_no_retry(mock__all_pods_ready_wrapper):
    mock__all_pods_ready_wrapper.return_value = None
    start_time = time()
    KubeCtl().wait_for_all_pods_ready()
