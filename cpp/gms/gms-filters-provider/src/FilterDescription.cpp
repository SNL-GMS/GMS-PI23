                                           ...
                                           bool isCausal)
{
  ...
  {
    const auto errMsg{"Comments provided exceed maximum length: " + std::to_string(MAX_COMMENT_SIZE)};
    throw std::invalid_argument(errMsg);
  }

  return FilterDescription(linearIIRFilterDescription, filterComputationType, comments, isCausal);
};
...
FILTER_DESCRIPTION FilterDescription::to_cstruct(FilterDescription *filterDescription)
{
  ...
  defStruct.filter_type = (FILTER_COMPUTATION_TYPE)filterDescription->filterComputationType;
