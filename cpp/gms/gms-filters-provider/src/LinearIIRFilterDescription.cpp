...
LinearIIRFilterDescription LinearIIRFilterDescription::build(IIRFilterParameters iirFilterParameters,
                                                             ...
                                                             int taper)
{
  return LinearIIRFilterDescription(iirFilterParameters, filterDesignModel, filterBandType, cutoffLow, cutoffHigh, filterOrder, sampleRate, sampleRateTolerance, zeroPhase, taper);
};
...
LINEAR_IIR_FILTER_DESCRIPTION LinearIIRFilterDescription::to_cstruct(LinearIIRFilterDescription *lifd)
{
  ...
  defStruct.band_type = (FILTER_BAND_TYPE)lifd->filterBandType;
  defStruct.cutoff_frequency_high = lifd->cutoffHigh;
  defStruct.cutoff_frequency_low = lifd->cutoffLow;
  defStruct.design_model = (FILTER_DESIGN_MODEL)lifd->filterDesignModel;
