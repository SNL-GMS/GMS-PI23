                                         ...
                                         int numberOfFilterDescriptions)
{
  ...
  {
    const auto errMsg{"Comment provided exceed maximum length: " + std::to_string(MAX_COMMENT_SIZE)};
    throw std::invalid_argument(errMsg);
  }

  if (name.length() > MAX_NAME_SIZE - 1)
  {
    const auto errMsg{"Name provided exceed maximum length: " + std::to_string(MAX_NAME_SIZE)};
    throw std::invalid_argument(errMsg);
  }

  if (filterDescriptions.size() > MAX_FILTER_DESCRIPTIONS || numberOfFilterDescriptions > MAX_FILTER_DESCRIPTIONS)
  {
    const auto errMsg{"Too many filter descriptions provided: " + std::to_string(MAX_FILTER_DESCRIPTIONS)};
    throw std::invalid_argument(errMsg);
  }
...
FilterDefinition FilterDefinition::from_cstruct(FILTER_DEFINITION *fd)
{
  ...
  {
    filterDescriptions.push_back(FilterDescription::from_cstruct(&fd->filter_description[i]));
  }

  return FilterDefinition(cfp, filterDescriptions, fd->name, fd->comments, fd->is_designed, fd->remove_group_delay, fd->num_filter_descriptions);
};