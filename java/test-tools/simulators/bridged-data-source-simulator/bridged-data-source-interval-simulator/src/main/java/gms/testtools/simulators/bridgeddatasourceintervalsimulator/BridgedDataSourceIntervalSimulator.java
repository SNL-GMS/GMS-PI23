...
/**
 * The Bridged Data Source Interval Simulator is responsible for taking a set of intervals from the past and shifting
 * them into a time span that started sometime in the past and goes until now.
 */
public class BridgedDataSourceIntervalSimulator implements DataSimulator {
  ...
  private DataSimulatorSpec intervalSimulatorSpec;
  ...
  /**
   * Using the BridgedDataSourceSimulatorSpec passed in load the specified data from the seed database into the
   * simulation database
   *
   * @param bridgedDataSourceSimulatorSpec - The {@link DataSimulatorSpec} that specifies simulation parameters.
   */
  @Override
  ...
  //
  /**
   * The process of loading seed data into the simulation set has two parts: BackwardPass, before the
   * simulationStartTime and ForwardPass, after the simulationStartTime. This is the backward pass:
   * <p>
   * BackwardPass - Align the seed data so that it's endTime aligns with the simulationStartTime and then copy
   * numberBackwardSeeds shifting each by the seed duration
   *
  ...
  /**
   * The process of loading seed data into the simulation set has two parts: BackwardPass, before the
   * simulationStartTime and ForwardPass, after the simulationStartTime. This is the forward pass:
   * <p>
   * ForwardPass - Align the seed data so that it's startTime aligns with simulationStartTime and then copy
   * numberForwardSeeds shifting each by the seed duration
   *
   * @param seedMetadata SeedMetadata object containing metadata need to perform the backward pass
   * @param simulationStartTime Time that the simulation starts.
   * @param initialIntervalId Which intervalID to start eith when generating the intervals
   * @param initializationSeedTimeRef This is like an "out" paramter - it will contain the time of the first interval
   * used by start
   *
   * @return The last interval ID that was generated by this pass.
   */
  ...
  /**
   * Construct a Flux which emits a simulated interval in real time, based on the modification dates of the seed data
   * set.
   *
   ...
   * @param initializationSeedTimeOffset How far to go past the initialize time to find the start of seed data.
   * @param seedData Set of seed intervals.
   * @param nowSupplier A supplier that indicates "now". Normally, would return Instant.now, but can be made to return
   * whatever is needed for testing.
   * @param intervalIdStart The interval id after which new interval IDs will be created.
   * @param applyCorrections Whether to perform self-correcting. This is set to false for unit testing
   *
   * @return A flux which once subscribed to, will emit simulated intervals on a "dynamic cadence" dependant on the
   * relative modification dates of the seed intervals.
   */
  @VisibleForTesting
  ...
  private SeedMetadata computeSeedMetadata(
    DataSimulatorSpec intervalSimulatorSpec,
    Instant now
  ) {
  ...
  /**
   * "Sort" the given seed data by sorting by moddate, then spliting the data set at the given time, then rearranging so
   * that everything after the split time is placed first on the new list.
   *
  ...
  /**
   * Create a Flux of IntervalDaoNextDelayPair (in other words, "annotated" seed data) from the given sorted seed data.
   * <p>
   ...
   *
   * @return A copy of the seed data that has been shifted backwards in time byt the amount equal to (passIndex *
   * seedDuration)
   */
  @VisibleForTesting
