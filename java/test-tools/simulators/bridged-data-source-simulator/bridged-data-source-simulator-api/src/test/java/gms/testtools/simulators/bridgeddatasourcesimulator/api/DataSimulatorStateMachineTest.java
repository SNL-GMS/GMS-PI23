...
class DataSimulatorStateMachineTest {
  ...
  void testInvalidTransitionThrowsIllegalArgument(State currentState, Collection<State> nextStates) {
    simulatorStateMachine.setCurrentState(currentState);

    for (State nextState : nextStates) {
      var thrown = assertThrows(IllegalArgumentException.class, () -> simulatorStateMachine.transition(nextState));
  ...
  private static Stream<Arguments> invalidTransitionArguments() {
    return Stream.of(
      ...
      arguments(State.INITIALIZING, List.of(State.UNINITIALIZED, State.STARTED, State.STOPPED, State.UNINITIALIZING)),
      arguments(State.INITIALIZED, List.of(State.UNINITIALIZED, State.INITIALIZING, State.STOPPED)),
      arguments(State.STARTED, List.of(State.UNINITIALIZED, State.INITIALIZING, State.INITIALIZED, State.UNINITIALIZING)),
      arguments(State.STOPPED, List.of(State.UNINITIALIZED, State.INITIALIZED, State.INITIALIZING))
    );
  }
