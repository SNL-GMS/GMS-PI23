...
public abstract class DataSimulatorResult<T> {
  ...
  public static <T> DataSimulatorResult<T> create(String simulatorName, Instant timestamp, Optional<T> result, Optional<Throwable> throwable) {
    return new AutoValue_DataSimulatorResult<>(simulatorName, timestamp, result, throwable);
  }

  public static <T> Collector<DataSimulatorResult<T>, ?, Map<String, ExceptionSummary>> toExceptionSummaryMap() {
    return filtering(result -> result.getThrowable().isPresent(),
      toMap(DataSimulatorResult::getSimulatorName,
        result -> ExceptionSummary.create(result.getTimestamp(), result.getThrowable().orElseThrow())));
  }

  public static <T> Collector<DataSimulatorResult<T>, ?, Map<String, T>> toResultsMap() {
    return filtering(result -> result.getResult().isPresent(),
