...
public class BridgedDataSourceRepositoryJpa extends DatabaseConnector {
  ...
  /**
   * Initializes a {@link BridgedDataSourceRepositoryJpa} to be used against the
   * simulation schema.
   *
   * @param entityManagerFactory - An {@link EntityManagerFactory} that is
   * configured to connect to the simulation schema.
   *
   * @return a {@link BridgedDataSourceRepositoryJpa} to be used against the
   * simulation schema.
   */
   ...
   * @param entityManager the entity manager connected to a specific persistence
   * unit
   */
   ...
   *
   * @param simulationData - A collection of data of type {@link T} to be stored
  ...
  public <T> void store(List<T> simulationData) {
    ...
    EntityVoidFunction delegateFunc = (entityManager) -> {
