...
public class StationSohContributingUtility {
  ...
  private static final Map<String, Table<String, SohMonitorType, Boolean>> stationContributingMaps = new HashMap<>();
  ...
  /**
   * Initialize stationContributingMaps from the StationSohDefinitions in the
    ...
    stationSohParameters.getStationSohControlConfiguration()
      .getStationSohDefinitions().forEach(stationSohDefinition -> {
        ...
        for (ChannelSohDefinition channelSohDefinition : stationSohDefinition.getChannelSohDefinitions()) {
          ...
          SohMonitorType.validTypes().forEach(sohMonitorType -> {
            var contributing = Optional.ofNullable(stationSohDefinition.getChannelsBySohMonitorType().get(sohMonitorType))
              .map(channels -> channels.contains(channelSohDefinition.getChannelName())).orElse(false);
            stationContributingTable.put(channelSohDefinition.getChannelName(), sohMonitorType, contributing);
          });
        }
      });
  ...
  /**
   * Look up in the static map if for the station the channel and monitor type
   * were part of the rollup
   *
   * @param stationName
   * @param channelName
   * @param type
   ...
   *
   * @throws IllegalStateException if the stationContributingMaps data structure
   * is null
   */
  public boolean isChannelMonitorContributing(
    ...
    var table = stationContributingMaps.get(stationName);

    return Optional.ofNullable(table.get(channelName, type)).orElse(false);
  }
}
