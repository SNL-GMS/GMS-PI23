...
import gms.core.performancemonitoring.soh.control.configuration.StationEnvMonitorTypesFile;
...
public class PerformanceMonitoringConfigurationUtility {
  ...
  public static Set<StationSohDefinition> resolveStationSohDefinitions(
    ...
    return stations.stream().map(station -> {
      ...
      var envMonitorTypes =
        resolveEnvMonitorTypesForStation(configurationConsumerUtility, stationName);
  ...
  private static Set<SohMonitorType> resolveEnvMonitorTypesForStation(ConfigurationConsumerUtility ccu, String stationName) {
    return ccu.resolve(STATION_SOH_PREFIX + ".station-env-monitor-types", 
      List.of(Selector.from(STATION_NAME_SELECTOR_KEY, stationName)), StationEnvMonitorTypesFile.class)
      .getStationEnvMonitorTypes();
  }
}
