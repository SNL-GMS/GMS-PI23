...
//        );
  ...
  @Test
  void testStationEnvMonitorTypeValidation() {
    assertThrows(IllegalArgumentException.class, () -> init("gms/core/performancemonitoring/soh/configuration-base-bad-station-env-monitortype"));
  }
  
  @Test
  void testStationEnvMonitorType() {
    init("gms/core/performancemonitoring/soh/configuration-base-station-env-monitortype");

    var stationSohDefinitionsByName = stationSohControlConfiguration
      .getInitialConfigurationPair().getStationSohMonitoringDefinition()
      .getStationSohDefinitions().stream()
      .collect(Collectors.toMap(StationSohDefinition::getStationName, Function.identity()));

    var expectedEnvA = Set.of(SohMonitorType.ENV_AUTHENTICATION_SEAL_BROKEN, SohMonitorType.ENV_ZEROED_DATA);
    var expectedEnvB = Set.of(SohMonitorType.ENV_BACKUP_POWER_UNSTABLE, SohMonitorType.ENV_ZEROED_DATA);
    assertThat(stationSohDefinitionsByName.get("StationA").getEnvMonitorTypes())
      .containsExactlyInAnyOrderElementsOf(expectedEnvA);
    assertThat(stationSohDefinitionsByName.get("StationB").getEnvMonitorTypes())
      .containsExactlyInAnyOrderElementsOf(expectedEnvB);
    assertThat(stationSohDefinitionsByName.get("StationC").getEnvMonitorTypes())
      .isEmpty();
  }
}
