...
public class OriginSimulatorDatabaseConnector extends DatabaseConnector {
  ...
  private static final Logger logger = LoggerFactory.getLogger(OriginSimulatorDatabaseConnector.class);
  ...
  /**
   * Find origin records within the given time range. Note that this does not
   * adjust by what is in ORIGERR (like OriginDatabaseConnector does) because
   * the simulator will want exactly the ORIGIN records in the time range.
   *
  ...
  public List<OriginDao> findOriginDaosByPreciseTime(Instant startTime, Instant endTime) {
    ...
    EntityResultListFunction<OriginDao> delegateFunction = entityManager -> {
    ...
    return runWithEntityManagerResultListFunction(delegateFunction,
      ORIGIN_TIME_ERROR, errMessage);
  }

  /**
   * Find the EventControlDao record associated with the eventIdOriginIdKey, and
   * return an Optional empty if it doesn't exist.
   *
  ...
  public Optional<EventControlDao> retrieveEventControlDaoByEventIdAndOriginId(
    ...
    if (singletonList.size() > 1) {
      logger.error("Event control by event id and origin id failure: ", new IllegalStateException(
   ...
   *
   * @param recordClass The record class that represents the type of records to
   * query
   * @param idPathGenerator lambda that takes a JPA root and returns a JPA path,
   * used to identify the field to compare to.
  ...
  private <T, K> List<T> retrieveRecordsById(
    ...
    EntityResultListFunction<T> delegateFunction = entityManager -> {
