...
public class EventManager {
  ...
  private static final String FP_ERROR = "Error occurred when reaching out to Feature Prediction service";
  ...
  public ResponseEntity<Object> predictFeaturesForEventLocation(
    ...
    try {
      ...
      eventPredictionRequest.getReceivers().stream().flatMap(r -> r.getReceiverLocationsByName().keySet().stream())
        .forEach(locationName -> receiverNameMapResponse.put(locationName, FeaturePredictionContainer.create(new ArrayList<>())));
  ...
  /**
   * Throws {@link FeaturePredictionException} if the receiverNameMapResponse contains no {@link FeaturePrediction}s
   * @param receiverNameMapResponse
   * @throws FeaturePredictionException 
   */
  private void verifyPredictionsPopulated(Map<String, FeaturePredictionContainer> receiverNameMapResponse) throws FeaturePredictionException {
    var predictions = receiverNameMapResponse.values().stream()
