...
/**
 * A utility for constructing and sending a request to the
 * {@link gms.shared.event.coi.featureprediction.FeaturePrediction} service
 */
@Component
class WebRequests {
   ...
   *
   * @param predictForLocationSolutionAndChannelRequest
   * {@link PredictForLocationSolutionAndChannelRequest} request body to sent to
   * Feature Prediction Service
   *
   * @return {@link LocationSolution} processed by Feature Prediction Service
   *
   * @throws FeaturePredictionException When not able to communicate or parse
   * data from Feature Prediction Service
   */
  LocationSolution fpsWebRequestPredictForLocationSolutionAndChannel(
    ...
    var baseLogMsg = "Querying FeaturePredictorService endpoint " + predictForLocationSolutionAndChannelUri;
    if (logger.isDebugEnabled()) {
      logger.debug("{} with request... "
  ...
  FeaturePredictionContainer fpsWebRequestPredictForLocation(
    ...
    var locationJson = this.webClientBuilder.build().post()
      ...
      .onErrorResume(FeaturePredictionException.class, e -> {
        logger.warn("Caught FeaturePredictionException, unable to include those predictions in the response", e);
        String emptyFeaturePredictionContainerJson;
        try {
          emptyFeaturePredictionContainerJson = ObjectMapperFactory.getJsonObjectMapper().writeValueAsString(FeaturePredictionContainer.create(Set.of()));
        }
      ...
      .blockOptional().orElseThrow(() -> new FeaturePredictionException("Unable to process empty response from FeaturePredictionService"));
