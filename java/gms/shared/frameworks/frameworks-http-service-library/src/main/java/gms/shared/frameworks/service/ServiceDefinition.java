...
public abstract class ServiceDefinition {
   ...
   * Route#getPath()} are equal to {@link GmsCommonRoutes#HEALTHCHECK_PATH}
   *
  ...
  /**
   * Gets the ContextRoot for the service.  This maps to the service name
   *
   ...
   * @return a builder
   */
  public static Builder builder(ServerConfig serverConfig, String contextRoot) {
  ...
  public abstract static class Builder {
    ...
    public abstract Builder setContextRoot(String contextRoot);
     ...
     * invalid. The validations are: - Each {@link Route#getPath()} in {@link
     * ServiceDefinition#getRoutes()} must be unique - None of the {@link Route#getPath()} can equal
     * {@link GmsCommonRoutes#HEALTHCHECK_PATH}
     *
    ...
    public ServiceDefinition build() {
      ...
      Validate.isTrue(
        def.getRoutes().stream().map(Route::getPath).noneMatch(GmsCommonRoutes.HEALTHCHECK_PATH::equals),
        "Endpoint " + GmsCommonRoutes.HEALTHCHECK_PATH + " is reserved and cannot be provided as a Route's path");
      return def;
    }
  }
}
