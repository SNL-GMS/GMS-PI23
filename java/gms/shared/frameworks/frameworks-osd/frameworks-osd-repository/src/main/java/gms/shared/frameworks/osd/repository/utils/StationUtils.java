...
public class StationUtils {
  ...
  /**
   * Store a station represented by the provided {@link StationDao} to the database. 
   * If already persisted, will update the persistent entity.
  ...
  public StationDao storeStation(Station station) {
    ...
    if (stationDao == null) {
      ...
      removeOrphanedChannelGroups(stationDao, station.getChannelGroups().stream()
        .map(ChannelGroup::getName)
        .collect(Collectors.toList()));

      removeOrphanedStationChannelInfo(stationDao, station.getRelativePositionsByChannel().keySet());
    }
  ...
  /**
   * Remove {@link StationChannelInfoDao}s from the database that no longer represent valid associations from incoming station updates
   * @param stationDao Station persisted in the database used to query for associated {@link StationChannelInfoDao}s
   * @param channelNames channelGroupNames names of channel groups still associated with incoming station updates
   */
  private void removeOrphanedStationChannelInfo(StationDao stationDao, Set<String> channelNames) {
    ...
    Root<StationChannelInfoDao> fromStationChannelInfo = stationChannelInfoDelete.from(StationChannelInfoDao.class);

    var associatedWithStation = cb.equal(fromStationChannelInfo.get("id").get(STATION), stationDao);
    var isOrphaned = fromStationChannelInfo.get("id").get("channel").get("name").in(channelNames).not();
  ...
  /**
   * Generate a {@link Station} given the provided {@link StationDao}, optionally built 
   * with information based on persisted derived channels.
   * @param stationDao DAO to base the resulting {@link Station} on
   * @param retrieveDerivedChannels Whether or not derived channels should be retrieved/included to build the resulting {@link Station}
   * @return A {@link Station} built from the provided {@link StationDao} and related persistent information
   * @throws IOException If there is an issue retrieving station-related information from the database
   */
  ...
  private List<ChannelGroupDao> storeChannelGroups(Set<ChannelGroup> channelGroups, StationDao station,
  ...
  private List<ChannelDao> storeChannels(ChannelGroup channelGroup, StationDao stationDao,
    ...
    for (Channel channel : channelGroup.getChannels()) {
      ...
      if (channelDao == null) {
        ...
        var processingDefinition = mapper.valueToTree(channel.getProcessingDefinition())
          .toString();
        channelDao.setProcessingDefinition(processingDefinition);
        
        var processingMetadata = mapper.valueToTree(channel.getProcessingMetadata())
          .toString();
      ...
      if (channelRelativePosition != null) {
        ...
        logger.warn("StationChannelInfoDao exists in the database, but incoming relative positions map does not have an entry for channel {} to station {}. Will not modify persistent StationChannelInfoDao.",
          channel.getName(), stationDao.getName());
      } else {
        logger.warn("Incoming relative positions map does not have an entry for channel {} to station {}. Will not store station->channel info.",
          channel.getName(), stationDao.getName());
      }
  ...
  private List<ChannelGroupDao> retrieveChannelGroupDaos(StationDao stationDao) {
    ...
    return entityManager.createQuery(channelGroupQuery)
