...
class ChannelNameUtilities {
  ...
  static String createShortName(Channel channel) {
    return createShortName(channel.getStation(),
  ...
  private static String createSteerAttributeForChannelName(
    ...
    if (metadata.keySet().containsAll(required)) {
      return (Channel.COMPONENT_SEPARATOR
        ...
        + "az_"
        + String
        .format("%.3f", (double) metadata.get(ChannelProcessingMetadataType.STEERING_AZIMUTH))
        + "deg"
        + Channel.ATTRIBUTE_SEPARATOR
        + "slow_"
        + String
        .format("%.3f", (double) metadata.get(ChannelProcessingMetadataType.STEERING_SLOWNESS))
        + "s_per_deg")
        .toLowerCase(Locale.ENGLISH);
    }
