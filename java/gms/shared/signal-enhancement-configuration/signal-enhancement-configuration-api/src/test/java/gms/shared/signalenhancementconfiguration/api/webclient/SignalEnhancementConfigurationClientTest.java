...
public class SignalEnhancementConfigurationClientTest {
  ...
  @Mock
  SignalEnhancementConfigurationClientConfig clientConfig;
  ...
  private final String FD_BY_USER_FOR_SDHS_ROUTE = "/signal-enhancement-configuration-service/signal-enhancement-configuration/default-filter-definitions-for-signal-detection-hypotheses";
  private final URI FD_BY_USER_FOR_SDHS_URI = new URI("http://localhost:8080" + FD_BY_USER_FOR_SDHS_ROUTE);
  ...
  @Test
  void testFdByUsageForSDHs_returnsValidWhenSuccessful() throws JsonProcessingException {
    var expectedValue = FilterDefinitionByUsageBySignalDetectionHypothesis.from(Collections.emptyList());
    ...
    var hypotheses = List.of(SignalDetectionHypothesis.createEntityReference(UUID.randomUUID(), UUID.randomUUID()));
  ...
  @Test
  void testFdByUsageForSDHs_noRetryOnQuery4xx() {
    ...
    StepVerifier.create(secConfigClient.queryDefaultFilterDefByUsageForSDHs(request))
      .verifyErrorMatches(error -> error instanceof WebClientResponseException 
  ...
  @Test
  void testFdByUsageForSDHs_retryOnQuery5xx() throws JsonProcessingException {
    var retries = 2;
    ...
    var failResponse = ClientResponse.create(failStatus)
      .header(HttpHeaders.CONTENT_TYPE, MediaType.TEXT_HTML_VALUE)
      .body("OOPS")
      .build();

    var successValue = FilterDefinitionByUsageBySignalDetectionHypothesis.from(Collections.emptyList());
    var successResponse = ClientResponse.create(HttpStatus.OK)
        .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE)
  ...
  @Test
  void testFdByUsageForSDHs_throwsOnQueryBadResponse() {
