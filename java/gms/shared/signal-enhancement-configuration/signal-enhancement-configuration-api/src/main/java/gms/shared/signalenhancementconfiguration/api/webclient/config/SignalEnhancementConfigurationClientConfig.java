...
public class SignalEnhancementConfigurationClientConfig {
  ...
  public URI filterDefsByUsageForSDHsUrl() {
    var filterDefsByUsageForSdhUrlString = String.format("http://%s:%d/%s%s%s", signalEnhancementConfigHostname,
      signalEnhancementConfigPort, signalEnhancementConfigHostname, signalEnhancementConfigContextPath, filterDefsByUsageForSDHsUrlPath);

    try {
      return new URL(filterDefsByUsageForSdhUrlString).toURI();
    }
    catch (MalformedURLException | URISyntaxException e) {
      throw new IllegalStateException(String.format("Configured URL %s is malformed", filterDefsByUsageForSdhUrlString), e);
    }
  }
