...
public class SignalEnhancementConfigurationClient {
  ...
  private static final Logger logger = LoggerFactory.getLogger(SignalEnhancementConfigurationClient.class);
  ...
  @Autowired
  public SignalEnhancementConfigurationClient(WebClient.Builder clientBuilder, SignalEnhancementConfigurationClientConfig config) {
    this.clientBuilder = clientBuilder.defaultHeader(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON_VALUE);
    this.clientConfig = config;
  }

  public Mono<FilterDefinitionByUsageBySignalDetectionHypothesis> queryDefaultFilterDefByUsageForSDHs(
    ...
    logger.debug("Querying SignalEnhancementConfiguration endpoint {} with request... {}",
    ...
    return clientBuilder.build()
      ...
      .bodyValue(request)
      .retrieve()
      .bodyToMono(String.class) //TODO: Determine why declaring an specific class type here throws `UnsupportedMediaTypeException`
      .retryWhen(clientConfig.retrySpec()
        .doBeforeRetry(retry -> logger.warn(
          "Attempt to query for default filter definitions failed. Retrying...", retry.failure()))
        .filter(exception -> 
          !(exception instanceof WebClientResponseException) 
          || !(((WebClientResponseException) exception).getStatusCode().is4xxClientError())))
      .flatMap(bodyString -> {
        try {
          return Mono.just(mapper.readValue(bodyString, FilterDefinitionByUsageBySignalDetectionHypothesis.class));
