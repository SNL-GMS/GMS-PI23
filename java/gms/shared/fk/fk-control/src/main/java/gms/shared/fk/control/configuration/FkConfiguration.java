...
public class FkConfiguration {
   ...
   */
  public FkSpectraParameters createFkSpectraParameters(FkStreamingRequest request, double waveformSampleRateHz) {
    Objects.requireNonNull(request, "Cannot create FkSpectraParameters from null FkStreamingRequest");
    
    var builder = getDefaultFkSpectraDefinition().toBuilder()
    ...
    return FkSpectraParameters.from(DEFAULT_SPECTRA_PLUGIN_NAME, request.getChannels(), builder.build());
  }

  public List<FkAttributesParameters> createFkAttributesParameters(FkSpectraDefinition definition) {
    Objects.requireNonNull(definition, "Cannot create FkAttributesParameters from null FkSpectraDefinition");
  ...
  private FkSpectraDefinition getDefaultFkSpectraDefinition() {
    return configurationConsumerUtility
        .resolve(KEY_BUILDER.apply("fk-spectra-definitions"),
            List.of(), FkSpectraDefinition.class);
  }
}
