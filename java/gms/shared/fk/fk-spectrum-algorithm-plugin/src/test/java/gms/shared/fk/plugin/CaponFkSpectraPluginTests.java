...
class CaponFkSpectraPluginTests {
  
private FkSpectraPlugin plugin = new CaponFkSpectraPlugin();

  private static Stream<Arguments> handlerNullArguments() {
    return Stream.of(arguments(null, List.of(WaveformTestFixtures.CHANNEL_SEGMENT), DEFINITION),
  ...
  void testCreateNegativeArguments(Station station,
      Collection<ChannelSegment<Waveform>> channelSegments, FkSpectraDefinition definition) {
    assertThrows(NullPointerException.class,
        () -> plugin.generateFk(station, channelSegments, definition));
  }

  @Test
  void testGenerateFk() {
    List<ChannelGroup> channelGroups = BASE_CHANNELS.stream()
      .map(channel -> {
        ChannelGroup.Data data = ChannelGroup.Data.builder()
    ...
    List<String> channelNames = BASE_CHANNELS.stream()
      .map(Channel::getName)
      .collect(Collectors.toList());
    
    List<RelativePositionChannelPair> relativePositionChannelPairs = RELATIVE_POSITION_MAP.entrySet().stream()
    ...
    List<FkSpectra> fkSpectraList = plugin
        .generateFk(station,
          BASE_CHANNEL_SEGMENTS, 
          DEFINITION);
