...
public class CaponFkSpectraPlugin implements FkSpectraPlugin {
  ...
  @Override
  public List<FkSpectra> generateFk(Station station, 
    Collection<ChannelSegment<Waveform>> channelSegments, 
    FkSpectraDefinition definition) {

    Objects.requireNonNull(channelSegments,
        getName() + " cannot generate FK spectra from null channel segments");
    Objects.requireNonNull(definition,
        getName() + " cannot generate FK spectra from null FK spectra definition");
    ...
    double mediumVelocity = mediumVelocities.getMediumVelocity(
        station.getLocation(),
        definition.getPhaseType());

    Map<Channel, RelativePosition> relativePositionsByChannelName = station.getRelativePositionsByChannel();

    var algorithm = CaponFkSpectrumAlgorithm.create(definition, mediumVelocity,
        relativePositionsByChannelName);

    FkSpectra.Builder spectra = FkSpectra.builder()
        .setStartTime(
            channelSegments.iterator().next().getId().getStartTime().plus(definition.getWindowLead()))
