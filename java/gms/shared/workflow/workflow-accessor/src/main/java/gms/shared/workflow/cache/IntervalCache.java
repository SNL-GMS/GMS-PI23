...
/**
 * Cache that delegates to an Apache Ignite {@link IgniteCache} for CRUD operations on {@link StageInterval}s. Ignite is
 * configured to contain key-value pairs of Stage names to {@link NavigableMap}s of StageIntervals by start time. The
 * combination of stage name and start time define uniqueness for StageInterval, allowing this caching structure to
 * contain all relevant intervals without concerns of conflicting keys.
 */
public class IntervalCache {
  ...
  /**
   * Single-value retrieval via the two necessary keys to identify a {@link StageInterval}: the stage name and the start
   * time
   *
  ...
  /**
   * Atomically retrieves an Optional StageInterval from the cache. If present, will apply the update function and cache
   ...
   *
   * @return All {@link StageInterval}s with the provided stage name, whose start times fall within the provided time
   * range
   */
   ...
   * @param endTime End of the span of second retrieval keys, exclusive
   *
   * @return All Collection of {@link StageInterval}s with one of the provided stage names, whose start times fall
  ...
  public Optional<Instant> getLatestModificationTime() {
    return stageIntervalsByIntervalId.query(
      new ScanQuery<IntervalId, StageInterval>(),
      entry -> entry.getValue().getModificationTime()).getAll().stream()
      .max(Instant::compareTo);
  }

  /**
   * Atomically Inserts or updates a single StageInterval. This method will lock the entire stage before it updates or
   * inserts.
   *
  ...
  /**
   * Atomically prunes each {@link StageInterval}s with endTimes <= olderThan time and removes it from the cache <p>
  ...
  /**
   * Returns a {@link Lock} allowing the caller to lock a given {@link StageInterval} in the cache to prevent concurrent
   * updates
   *
