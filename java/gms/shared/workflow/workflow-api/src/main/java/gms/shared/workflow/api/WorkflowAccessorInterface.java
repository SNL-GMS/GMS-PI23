...
/**
 * Accessor interface of the Manager/Accessor/Repository pattern for the Workflow domain. Responsible for Managing a
 * cache of the {@link Workflow} and all {@link gms.shared.workflow.coi.Interval}s within the operational time period.
 * Delegates storage of intervals to an underlying {@link IntervalRepositoryInterface}
 */
public interface WorkflowAccessorInterface extends IntervalRepositoryInterface {
  ...
  /**
   * Atomically retrieves an Optional StageInterval from the cache and calls the UnaryOperator on it. Method will store
   * the result if the update method returns a non-empty Optional
   *
  ...
  /**
   * Atomically retrieves an Optional StageInterval from the cache and calls the update function on it. Method will
   * store the result if the update method returns a non-empty Optional
   *
   * @param stageIntervalId the interval Id to retrieve
   * @param update the update function to perform on the stageInterval
   *
