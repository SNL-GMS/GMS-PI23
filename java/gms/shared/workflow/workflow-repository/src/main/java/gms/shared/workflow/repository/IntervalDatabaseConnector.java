...
public class IntervalDatabaseConnector extends DatabaseConnector {
  ...
  static final String INTERVALS_TIME_RANGE_MOD_DATE_ERROR = "Intervals by time range and mod date exception";
  ...
  public List<IntervalDao> findIntervalsByTimeRange(Instant seedDataStartTime, Instant seedDataEndTime) {
    ...
    var errMessage = String.format(INTERVALS_TIME_RANGE_MESSAGE, seedDataStartTime,
      seedDataEndTime);
    EntityResultListFunction<IntervalDao> delegateFunc = entityManager -> {
    ...
    return runWithEntityManagerResultListFunction(delegateFunc,
      INTERVALS_TIME_RANGE_ERROR, errMessage);
  }

  /**
   * Retrieves all {@link IntervalDao}s with the given interval class and names
   * that lie within the given time range
   *
   ...
   *
   * @return All {@link IntervalDao}s with the given interval class and names
   * that lie within the given time range
   */
  public List<IntervalDao> findIntervalsByNameAndTimeRange(Set<Pair<String, String>> intervalClassAndNames,
    ...
    var errMessage = String.format(INTERVALS_NAMES_TIME_RANGE_MESSAGE,
      intervalClassAndNames.size(), startTime, endTime);
    EntityResultListFunction<IntervalDao> func = entityManager -> {
    ...
    return runWithEntityManagerResultListFunction(func,
      INTERVALS_NAMES_TIME_RANGE_ERROR, errMessage);
  }

  /**
   * Retrieves all {@link IntervalDao}s with the given interval class and names
   * that lie within the given time range and were modified after the given
   * modification date
   *
   * @param intervalClassAndNames Class and names of IntervalDaos to retrieve
   * @param startTime Start time of the query range, inclusive
   * @param endTime End time of the query range, exclusive
   * @param modDate Intervals are retrieved if they have a modDate after this
   * time
   *
   * @return All {@link IntervalDao}s with the given interval class and names
   * that lie within the given time range and were modified after the given
  ...
  public List<IntervalDao> findIntervalsByNameAndTimeRangeAfterModDate(Set<Pair<String, String>> intervalClassAndNames,
    ...
    var errMessage = String.format(INTERVALS_TIME_RANGE_MOD_DATE_MESSAGE,
      startTime, endTime, modDate);
    EntityResultListFunction<IntervalDao> func = entityManager -> {
      var cb = entityManager.getCriteriaBuilder();
      CriteriaQuery<IntervalDao> query = cb.createQuery(IntervalDao.class);
      Root<IntervalDao> intervalDaoRoot = query.from(IntervalDao.class);
    ...
    return runWithEntityManagerResultListFunction(func,
      INTERVALS_TIME_RANGE_MOD_DATE_ERROR, errMessage);
  }
}
