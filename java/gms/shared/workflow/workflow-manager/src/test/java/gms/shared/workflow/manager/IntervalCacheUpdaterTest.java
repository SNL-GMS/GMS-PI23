...
class IntervalCacheUpdaterTest {
  ...
  @Mock
  IntervalCache mockIntervalCache;
  ...
  @BeforeEach
  void setup() {
    this.intervalCacheUpdater = new IntervalCacheUpdater(mockWorkflow, mockIntervalCache, mockBridgedIntervalRepository);
  }
  ...
  void testUpdateIntervalCache() {
    ...
    given(mockBridgedIntervalRepository.findStageIntervalsByStageIdAndTime(startTime, endTime, stageIds, startTime))
    ...
    var expectedIntervals = Set.of(
      stageIntervals.get("Auto 1").get(0),
      stageIntervals.get("Auto 2").get(0),
      stageIntervals.get("Test 2").get(0)
    );
    var actualIntervals = intervalCacheUpdater.updateIntervalCache(OperationalPeriod.create(startTime, endTime));
    ...
    verify(mockBridgedIntervalRepository).findStageIntervalsByStageIdAndTime(startTime, endTime, stageIds, latestModTime);
