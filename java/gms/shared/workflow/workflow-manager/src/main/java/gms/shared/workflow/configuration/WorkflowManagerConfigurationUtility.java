...
public class WorkflowManagerConfigurationUtility {
  ...
  @Autowired
  public WorkflowManagerConfigurationUtility(SystemConfig systemConfig,
  ...
  public Workflow resolveWorkflowDefinition() {
    var workflowDefinition = processingConfigUtil.resolve(WORKFLOW_DEFINITION_CONFIG,
      List.of(), WorkflowDefinition.class);
    List<Stage> stages = workflowDefinition.getStageNames().stream()
      .map(stageName -> processingConfigUtil.resolve(STAGE_DEFINITION_CONFIG,
  ...
  public Duration resolveOperationalStartPeriod() {
    var operationalTimeConfig = processingConfigUtil.resolve(OPERATIONAL_TIME_PERIOD_CONFIG, Collections.emptyList());
  ...
  public Duration resolveBridgePollingPeriod() {
    var bridgedPollingPeriodConfig = processingConfigUtil.resolve(BRIDGE_POLLING_PERIOD_CONFIG, List.of());
