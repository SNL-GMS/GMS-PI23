...
/**
 * Manages the updating of the interval cache to stay in sync with intervals in persistence for an operational time
 * period.
 * <p>
 * NOTE: This class directly autowires both the IntervalCache and the bridge repository.
 * It is important to guarantee that you are only deleting intervals from the cache and only
 * fetching intervals directly from the bridge. Any adjustments to the type declarations of these should be managed
 * with care to prevent runtime errors (e.g. deleting intervals from the database, retrieving intervals from the cache
 * and never updating).
 */
@Component
public class IntervalCacheUpdater {
  ...
  @Autowired
  public IntervalCacheUpdater(Workflow workflow, IntervalCache intervalCache, BridgedIntervalRepository intervalRepository) {
    this.workflow = workflow;
    this.intervalCache = intervalCache;
    this.intervalRepository = intervalRepository;
    this.latestModTime.set(Instant.ofEpochSecond(-2208988800L));
  }

  /**
   * Main cache update method responsible for keeping the IntervalCache up-to-date with persisted stage intervals for
   * the operational time period. Note the first time this runs we will fetch all intervals for this operational period.
   * After this the updater has an understanding of the latest mod time it's seen, and will only fetch intervals newer
   * than that mod time.
   *
  ...
  public Set<StageInterval> updateIntervalCache(OperationalPeriod operationalPeriod) {
    ...
    var stageIntervals = intervalRepository.findStageIntervalsByStageIdAndTime(
      operationalTimeStart,
      operationalTimeEnd,
      stageIds,
      latestModTime.get());

    var intervalsToCache = stageIntervals
      .values().stream()
      .flatMap(List::stream)
  ...
  /**
   * Utility method for checking the latest mod time of already-cached intervals, and syncing latestModTime to it.
   */
