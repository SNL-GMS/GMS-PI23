...
class QcSegmentCategoryTest {
  ...
  void testAllowedTypes() {
    ...
    var allowedTypeMap = new EnumMap<QcSegmentCategory, List<QcSegmentType>>(
      Map.of(
        ANALYST_DEFINED,
        List.of(
          AGGREGATE, CALIBRATION, FLAT, GAP, NOISY, SENSOR_PROBLEM,
          SPIKE, STATION_PROBLEM, STATION_SECURITY, TIMING
        ),
        DATA_AUTHENTICATION, List.of(),
        LONG_TERM, List.of(),
        STATION_SOH,
        List.of(
        ...
        UNPROCESSED, List.of(),
        WAVEFORM, List.of(
          AGGREGATE, FLAT, GAP, NOISY, SPIKE
        )
      )
    );

    allowedTypeMap.forEach((category, types)
      -> Assertions.assertEquals(types, category.getAllowableTypes()));
  }
