...
/**
 * Helper operations used by {@link Channel.Builder} to create names for new
 * {@link Channel}s.
 */
public class ChannelNameUtilities {
  ...
  /**
   * Private constructor prevents instantiating instances of
   * {@link ChannelNameUtilities}.
   */
   ...
   *
   * @param channel create a name for this {@link Channel} using the Channel
   * properties, not null
   *
   ...
   * [STATION].[GROUP].[BAND_CODE][INSTRUMENT_CODE][ORIENTATION_CODE]. The
   * [ORIENTATION_CODE] is a SEED / FDSN standard channel name (see {@link
   * ChannelNameUtilities#getFdsnChannelName(Channel)}
   *
   * @param channel {@link Channel} that is fully specified but may not have the
   * correct name, not null
   *
  ...
  /**
   * Obtain the {@link ChannelProcessingMetadataType#CHANNEL_GROUP} String from
   * the provided metadata map.
   *
   * @param processingMetadata {@link ChannelProcessingMetadataType} to Object
   * map, must contain an entry for CHANNEL_GROUP, not null
   *
   * @return channel group String from the provided processingMetadata map, not
   * null
   */
  ...
  /**
   * Obtains a 3-character SEED / FDSN standard channel name in the form [BAND
   * CODE][INSTRUMENT CODE][ORIENTATION CODE] from the provided {@link Channel}.
   *
   ...
   * ChannelProcessingMetadataType} to object map. Creates attributes for as
   * many processing operations (e.g. beaming, filtering, FK, etc.) as possible,
   * but only creates attributes for a processing operation if all of the
   * necessary metadata fields appear in the map. Attributes for processing
   * operations appear in no particular order but the ordering is consistent
   * between calls. The output attribute string is empty if the metadata does
   * not contain sufficient entries to create an attribute string for any
   * processing operation. If the attribute string is not empty then it begins
   * with {@link Channel#COMPONENT_SEPARATOR} and each subsequent portion of the
   * attribute string is also separated by a {@link Channel#COMPONENT_SEPARATOR}
   * character.
   *
   * @param metadata processing metadata map of
   * {@link ChannelProcessingMetadataType} to object, not null
   *
   * @return String containing the processing operation attributes for a Channel
   * name, potentially empty but not null
   */
  ...
  /**
   * Create the filter specific Channel name attribute from a
   * {@link ChannelProcessingMetadataType} to object map if the map contains
   * metadata for a filtered channel. Returns an empty string if the metadata
   * map does not contain filter metadata. If the attribute string is not empty
   * then it begins with {@link Channel#COMPONENT_SEPARATOR}.
   *
   * @param metadata processing metadata map of
   * {@link ChannelProcessingMetadataType} to object, not null
   *
   * @return String containing the filter specific attributes for a Channel
   * name, potentially empty but not null
   */
  ...
  /**
   * Create the beam specific Channel name attribute from a
   * {@link ChannelProcessingMetadataType} to object map if the map contains
   * metadata for a beam channel. Returns an empty string if the metadata map
   * does not contain beam metadata. If the attribute string is not empty then
   * it begins with {@link
   * Channel#COMPONENT_SEPARATOR}.
   *
   * @param metadata processing metadata map of
   * {@link ChannelProcessingMetadataType} to object, not null
   *
   * @return String containing the beam specific attributes for a Channel name,
   * potentially empty but not null
   */
  ...
  /**
   * Create the steering (e.g. as done in beaming and rotation) specific Channel
   * name attribute from a {@link ChannelProcessingMetadataType} to object map
   * if the map contains metadata for a steered channel. Returns an empty string
   * if the metadata map does not contain steering metadata. If the attribute
   * string is not empty then it begins with
  ...
  /**
   * Computes a {@link Channel} hash using fields from the Channel. The getter
   * operations are public so Jackson can discover them for serialization.
   */
  @AutoValue
  ...
  abstract static class NameHash {
    ...
    @Nullable
    public abstract UUID getResponse();
    ...
    /**
     * Obtains a String representation of a type-3 {@link UUID} for this
     * NameHash. The UUID is computed as follows:
     * <p>
     * 1. Create a JSON serialization of the NameHash fields:
     * <p>
     * a. Serialize fields in this order: [sortedProcessingDefinition,
     * sortedConfiguredInputs, channelDataType, channelBandType,
     * channelInstrumentType, channelOrientationType, channelOrientationCode,
     * nominalSampleRateHz, orientationAngles]
     * <p>
     * b. processingDefinition is sorted alphabetically by key and
     * configuredInputs is sorted alphabetically.
     * <p>
     * c. The JSON does not use any whitespace or newline characters to separate
     * keys, values, formatting characters, etc.
     * <p>
     * 2. Write the JSON string to a UTF-8 byte array
     * <p>
     * 3. Compute the type-3 UUID from the byte array
     *
     * @return {@link UUID} formatted string computed from the NameHash
     * contents.
     */
    String getHash() {
      ...
      jsonMapper = CoiObjectMapperFactory.configureObjectMapper(jsonMapper)
    ...
    interface Builder {
      ...
      default Builder setProcessingDefinition(Map<String, Object> processingDefinition) {
        ...
        var sortedList = new TreeMap<>(processingDefinition).entrySet().stream()
      ...
      default Builder setProcessingMetadata(Map<ChannelProcessingMetadataType, Object> processingMetaData) {

        var sortedList = new TreeMap<>(processingMetaData).entrySet().stream()
          .collect(Collectors.toList());
        return setProcessingMetadata(sortedList);
      }

      Builder setProcessingMetadata(List<Entry<ChannelProcessingMetadataType, Object>> processingMetaData);
