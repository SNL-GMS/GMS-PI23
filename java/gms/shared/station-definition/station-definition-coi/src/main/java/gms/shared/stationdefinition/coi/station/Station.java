      ...
      default Station.Data build() {
        ...
        if (0 == numPresentFields) {
          ...
          Map<Channel, RelativePosition> relativePositionsByChannel = getRelativePositionChannelPairs()
            .orElseThrow()
            .stream()
            .collect(Collectors.toMap(pair -> pair.getChannel().toEntityReference(),
              RelativePositionChannelPair::getRelativePosition));

          getAllRawChannels().orElseThrow().forEach(
            channel -> Validate.isTrue(relativePositionsByChannel.containsKey(channel.toEntityReference()),
              "Station passed in a relative position for a channel it does not manage " + channel.getName()));
          ...
          Validate
            .isTrue(channelGroups.stream().noneMatch(ChannelGroup::isPresent)
