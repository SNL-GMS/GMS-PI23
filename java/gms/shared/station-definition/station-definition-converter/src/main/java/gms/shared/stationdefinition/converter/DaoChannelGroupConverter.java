...
import static gms.shared.stationdefinition.converter.ConverterUtils.CHANNEL_FUNC_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.EFFECTIVE_AT_UNTIL_TIME_STR;
...
import static gms.shared.stationdefinition.converter.ConverterUtils.EFFECTIVE_UNTIL_NOT_NULL;
...
import static gms.shared.stationdefinition.converter.ConverterUtils.SITE_CHANS_NOT_EMPTY;
...
public class DaoChannelGroupConverter implements ChannelGroupConverter {
  ...
  /**
   * Converts a {@link SiteDao}, and a passed list of {@link SiteChanDao} into a
   * {@link ChannelGroup}. {@link Channel}s in the {@link ChannelGroup} will be
   * version references NOTE: The depth is currently ignored and set to "0" as a
   * hardcoded value.
   *
   * @param siteDao The Site for the ChannelGroup
   * @param siteChanDaos The channels in the channel group, as specified by
   * ChannelConverter
   *
  ...
  public ChannelGroup convert(SiteDao siteDao, Collection<SiteChanDao> siteChanDaos,
    ...
    Preconditions.checkNotNull(siteDao, SITE_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR,
      effectiveAt);
    Preconditions.checkNotNull(siteChanDaos, SITE_CHANS_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR, effectiveAt);
    Preconditions.checkNotNull(channelFunction, CHANNEL_FUNC_NOT_NULL + EFFECTIVE_TIME_STR, effectiveAt);

    Preconditions.checkState(effectiveAt.isBefore(effectiveUntil), 
      EFFECTIVE_AT_UNTIL_TIME_STR, effectiveAt, effectiveUntil);
    Preconditions.checkState(!siteChanDaos.isEmpty(), SITE_CHANS_NOT_EMPTY + EFFECTIVE_TIME_STR, effectiveAt);
  ...
  private ChannelGroup getChannelGroup(SiteDao siteDao, Collection<SiteChanDao> siteChanDaos,
    ...
    //this should be done in the jpa converter, but there are side effects of setting it to null, needs to be set to Optional.empty()
  ...
  public ChannelGroup convert(SiteDao siteDao, Collection<SiteChanDao> siteChanDaos,
    ...
    Preconditions.checkState(effectiveAt.isBefore(effectiveUntil), EFFECTIVE_AT_UNTIL_TIME_STR, effectiveAt, effectiveUntil);
    Preconditions.checkState(!siteChanDaos.isEmpty(), SITE_CHANS_NOT_EMPTY + EFFECTIVE_TIME_STR, effectiveAt);
  ...
  /**
   * Converts a Site into a {@link ChannelGroup} version reference. NOTE: The
   * depth is currently ignored and set to "0" as a hardcoded value.
   *
   * @param siteDao The Site for the ChannelGroup
   *
   * @return The {@link ChannelGroup} representing the provided CSS data as a
   * version reference
   */
  @Override
