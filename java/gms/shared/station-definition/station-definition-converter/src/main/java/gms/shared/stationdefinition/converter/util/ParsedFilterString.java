...
/**
 * Value class representing a filter table's filter_string column parsed into their appropriate types
 */
@AutoValue
public abstract class ParsedFilterString {
   ...
   * @param filterString String in the format expected from the filter_string column of the filter table
  ...
  public static ParsedFilterString create(String filterString) {
    ...
    Preconditions.checkArgument(values.length == 5, "filter_string must have 5 space-separated values");
    ...
    switch (causalString) {
      ...
      case "non-causal":
          ...
          break;
      default:
          throw new IllegalArgumentException(String.format("filter_string contains an invalid causal value %s", causalString));
    }

    return new AutoValue_ParsedFilterString(lowFrequencyHz, highFrequencyHz, order, filterType, causal);
  }

  /**
   * Converts the parsed value class back into the raw string represented in the filter table's filter_string column
  ...
  public String toFilterString() {
    ...
    return String.format("%f %f %d %s %s", 
      this.getLowFrequencyHz(), this.getHighFrequencyHz(), 
      this.getOrder(), this.getFilterType().getValue(), causalString);
  }
}
