...
import static gms.shared.stationdefinition.converter.ConverterUtils.WFDISC_NOT_NULL;
...
public class DaoResponseConverter implements ResponseConverter {
  ...
  public Response convert(WfdiscDao wfdiscDao, SensorDao sensorDao, Calibration calibration,
    ...
    Preconditions.checkNotNull(wfdiscDao, WFDISC_NOT_NULL);
    Preconditions.checkNotNull(sensorDao, "SensorDao cannot be null for wfdisc dao %s.%s", 
      wfdiscDao.getStationCode(), wfdiscDao.getChannelCode());
    Preconditions.checkNotNull(calibration, "Calibration cannot be null for wfdisc dao %s.%s", 
      wfdiscDao.getStationCode(), wfdiscDao.getChannelCode());  
    Preconditions.checkNotNull(frequencyAmplitudePhase, "FrequencyAmplitudePhase cannot be null for "
      + "wfdisc dao %s.%s", wfdiscDao.getStationCode(), wfdiscDao.getChannelCode());
