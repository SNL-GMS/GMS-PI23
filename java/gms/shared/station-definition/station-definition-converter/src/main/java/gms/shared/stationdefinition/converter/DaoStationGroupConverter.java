...
public class DaoStationGroupConverter implements StationGroupConverter {
  ...
  @Override
  public StationGroup convert(Instant versionTime, Optional<Instant> effectiveUntil, 
    NetworkDao network, List<Station> stations) {
    Preconditions.checkNotNull(network, "Network cannot be null for version time %s", versionTime);
    Preconditions.checkNotNull(stations, "Stations cannot be null for version time %s", versionTime);
