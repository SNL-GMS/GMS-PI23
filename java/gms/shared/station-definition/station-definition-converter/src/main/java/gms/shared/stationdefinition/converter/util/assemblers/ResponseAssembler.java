...
import static gms.shared.stationdefinition.converter.ConverterUtils.END_TIME_BEFORE_START_TIME_STR;
import static gms.shared.stationdefinition.converter.ConverterUtils.END_TIME_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.INSTRUMENTS_NOT_NULL;
...
public class ResponseAssembler {
  ...
  public List<Response> buildAllForTimeRange(Instant startTime,
    ...
    Preconditions.checkNotNull(instruments, INSTRUMENTS_NOT_NULL + START_END_TIME_STR, startTime, endTime);
    Preconditions.checkState(!endTime.isBefore(startTime), END_TIME_BEFORE_START_TIME_STR, endTime, startTime);
    ...
    return wfdiscs.stream()
      ...
      .map(wfdisc -> {
        ...
        if (!sensorVersionsByStaChan.contains(station, channel)
          ...
          logger.debug(Markers.append(EFFECTIVE_TIME, effectiveTime),
