...
import static gms.shared.stationdefinition.converter.ConverterUtils.BEAM_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.CHANNEL_EFFECTIVE_TIME_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.CHANNEL_END_TIME_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.CHANNEL_START_END_TIME_STR;
import static gms.shared.stationdefinition.converter.ConverterUtils.SITES_MUST_NOT_BE_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.SITE_CHAN_MUST_NOT_BE_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.SITE_CHANS_MUST_NOT_BE_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.EFFECTIVE_TIME_STR;
import static gms.shared.stationdefinition.converter.ConverterUtils.SENSORS_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.SITE_MUST_NOT_BE_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.WFDISCS_NOT_NULL;
import static gms.shared.stationdefinition.converter.ConverterUtils.START_END_BOOLEANS_NOT_NULL;
...
public class ChannelAssembler {
  ...
  public List<Channel> buildAllForTime(Instant effectiveTime,
    ...
    Preconditions.checkNotNull(sites, SITES_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR,
      effectiveTime);
    Preconditions.checkNotNull(siteChans, SITE_CHAN_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR,
      effectiveTime);
    Preconditions.checkNotNull(sensors, SENSORS_NOT_NULL + EFFECTIVE_TIME_STR,
      effectiveTime);
    Preconditions.checkNotNull(wfdiscs, WFDISCS_NOT_NULL + EFFECTIVE_TIME_STR,
      effectiveTime);
    Preconditions.checkNotNull(startEndBooleans, START_END_BOOLEANS_NOT_NULL + EFFECTIVE_TIME_STR,
      effectiveTime);
    ...
    List<Channel> resultList = createChannelTablesAndMaps(Pair.of(effectiveTime, effectiveTime), sites, siteChans,
      sensors, wfdiscs, responses, (Response r) -> r, startEndBooleans);
  ...
  public List<Channel> buildAllForTimeRange(Instant startTime,
    ...
    Preconditions.checkNotNull(sites, SITE_CHAN_MUST_NOT_BE_NULL + TIME_RANGE_STR,
      startTime, endTime);
    Preconditions.checkNotNull(siteChans, SITE_CHANS_MUST_NOT_BE_NULL + TIME_RANGE_STR,
      startTime, endTime);
    Preconditions.checkNotNull(sensors, SENSORS_NOT_NULL + TIME_RANGE_STR,
      startTime, endTime);
    Preconditions.checkNotNull(wfdiscs, WFDISCS_NOT_NULL + TIME_RANGE_STR,
      startTime, endTime);
    Preconditions.checkNotNull(startEndBooleans, START_END_BOOLEANS_NOT_NULL + TIME_RANGE_STR,
      startTime, endTime);
    ...
    return createChannelTablesAndMaps(Pair.of(startTime, endTime), sites, siteChans, sensors, wfdiscs, responses, Response::createEntityReference, startEndBooleans).stream()
  ...
  public Channel buildFromAssociatedRecord(Map<ChannelProcessingMetadataType, Object> processingMetadataMap,
    ...
    Preconditions.checkNotNull(beamDao, BEAM_NOT_NULL + EFFECTIVE_TIME_STR,
      channelEffectiveTime);
    Preconditions.checkNotNull(site, SITES_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR,
      channelEffectiveTime);
    Preconditions.checkNotNull(wfdisc, WFDISCS_NOT_NULL + EFFECTIVE_TIME_STR,
      channelEffectiveTime);
    Preconditions.checkNotNull(siteChan, SITE_CHAN_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR,
      channelEffectiveTime);
    Preconditions.checkNotNull(possibleSensor, SENSORS_NOT_NULL + EFFECTIVE_TIME_STR,
      channelEffectiveTime);

    Preconditions.checkState(channelEffectiveTime.isBefore(channelEndTime),
      CHANNEL_START_END_TIME_STR, channelEffectiveTime, channelEndTime);
    ...
    if (site.getId().getStationCode().equals(site.getReferenceStation())
      ...
      Preconditions.checkState(possibleSensor.isPresent(),
        "Cannot convert raw channel if sensor is not present for %s - %s",
  ...
  public Channel buildRawChannel(SiteDao site,
    ...
    Preconditions.checkNotNull(channelEffectiveTime, CHANNEL_EFFECTIVE_TIME_NOT_NULL);
    Preconditions.checkNotNull(channelEndTime, CHANNEL_END_TIME_NOT_NULL);
    Preconditions.checkNotNull(site, SITE_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR, channelEffectiveTime);
    Preconditions.checkNotNull(wfdisc, WFDISCS_NOT_NULL + EFFECTIVE_TIME_STR, channelEffectiveTime);
    Preconditions.checkNotNull(siteChan, SITE_CHANS_MUST_NOT_BE_NULL + EFFECTIVE_TIME_STR, channelEffectiveTime);
    Preconditions.checkNotNull(possibleSensor, SENSORS_NOT_NULL + EFFECTIVE_TIME_STR, channelEffectiveTime);
    ...
    // Statype is ss (single station) == raw
    Preconditions.checkState(possibleSensor.isPresent(),
      "Cannot convert raw channel if sensor is not present for site chan %s.%s and effective time %s",
      siteChan.getId().getStationCode(), siteChan.getId().getChannelCode(), channelEffectiveTime);
    SensorDao sensor = possibleSensor.get();
    ResponseConverterTransform responseConverterTransform
      = (wfdiscDao, sensorDao, calibration, frequencyAmplitudePhase) -> responseConverter
        .convertToEntity(wfdiscDao);
      ...
      .flatMap(staKey -> {
        ...
        return chanNavmap.entrySet().stream()
          .flatMap(entry
            ...
            idToResponseMap, responseFacet, startEndBooleans).stream());
  ...
  private List<Channel> processChannels(
    ...
    NavigableSet<Instant> possibleVersionTimes = getChangeTimes(siteVersions, siteChanVersions, sensorVersions, wfdiscVersions, idToResponseMap, startEndBooleans);
    var validTimes = AssemblerUtils.getValidTimes(startEndTime, possibleVersionTimes, isRange);

    return processPossibleVersionTimes(startEndTime, validTimes, siteVersions, siteChanVersions, sensorVersions,
  ...
  private List<Channel> processPossibleVersionTimes(
    ...
    for (var i = 0; i < possibleVersionTimes.size() - 1; i++) {
      ...
      Optional<Response> possibleResponse = getPossibleResponse(responses, possibleSensorDao, versionRange, startEndTime);
  ...
  private Optional<Response> getPossibleResponse(TemporalMap<UUID, Response> responses,
    Optional<SensorDao> possibleSensorDao, Range<Instant> currRange,
    Pair<Instant, Instant> startEndTime) {
    ...
    return possibleSensorDao.isPresent()
      ? responses.getVersionFloor(
        UUID.nameUUIDFromBytes((possibleSensorDao.get().getSensorKey().getStation() + possibleSensorDao.get().getSensorKey().getChannel()).getBytes()),
        correctTime)
      : Optional.empty();
  }
  ...
  private Optional<Channel> convertChannel(Range<Instant> versionRange,
    ...
    try {
      ...
      var errMessage = String.format("Could not convert channel with time range %s - %s",
        versionRange.lowerEndpoint(), versionRange.upperEndpoint());
      if (siteChanDao != null) {
        var statChanCode = siteChanDao.getId().getStationCode()
  ...
  private NavigableSet<Instant> getChangeTimes(
    ...
    NavigableSet<Instant> changeTimes = getChangeTimesForWfdiscSensorAndResponse(sensorsForRange, wfdiscsForRange, idToResponseMap, siteChansForRange);
    ...
    AssemblerUtils.addChangeTimesToListForDaosWithDayAccuracy(changeTimes, siteChansForRange, changeOccuredForSiteChan,
      ...
      startEndBooleans::getPrevTimeOverLapForSiteChan, startEndBooleans::getNextTimeOverLapForSiteChan);
    AssemblerUtils.addChangeTimesToListForDaosWithDayAccuracy(changeTimes, sitesForRange, changeOccuredForSite,
      ...
      startEndBooleans::getPrevTimeOverLapForSite, startEndBooleans::getNextTimeOverLapForSite);
  ...
  private NavigableSet<Instant> getChangeTimesForWfdiscSensorAndResponse(
    ...
    NavigableSet<Instant> siteChanTimes
      = getPossibleTimes(siteChansForRange, Functions.compose(SiteChanKey::getOnDate, SiteChanDao::getId), SiteChanDao::getOffDate);
    ...
    while (versionTimeIterator.hasNext()) {
      ...
      if (!prevSampleRate.equals(currSampleRate)
        ...
        || (!currSensor.equals(prevSensor) || !currWfdisc.equals(prevWfdisc)) && AssemblerUtils.changeTimeExistsForDay(currInstant, siteChanTimes)) {
  ...
  private Optional<Double> getCurrSampleRate(Optional<WfdiscDao> currWfdisc, Optional<SensorDao> currSensor) {
