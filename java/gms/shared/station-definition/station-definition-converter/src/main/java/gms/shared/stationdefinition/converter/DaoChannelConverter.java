...
import static gms.shared.stationdefinition.converter.ConverterUtils.COULD_NOT_PARSE_CHANNEL_TYPES_FOR_SITE_CHAN_DAO;
import static gms.shared.stationdefinition.converter.ConverterUtils.EFFECTIVE_TIME_STR;
import static gms.shared.stationdefinition.converter.ConverterUtils.INSTRUMENT_WFDISC_MUST_NOT_BE_NULL;
...
import static gms.shared.stationdefinition.converter.ConverterUtils.VERSION_RANGE_STR;
...
public class DaoChannelConverter implements ChannelConverter {
  ...
  public Channel convert(SiteChanDao siteChanDao,
    ...
    Preconditions.checkNotNull(siteChanDao, SITE_CHAN_MUST_NOT_BE_NULL
      + VERSION_RANGE_STR, versionRange);
    Preconditions.checkNotNull(siteDao, SITE_MUST_NOT_BE_NULL
      + VERSION_RANGE_STR, versionRange);

    Preconditions
      .checkState(instrumentDao != null || wfdiscDao != null, INSTRUMENT_WFDISC_MUST_NOT_BE_NULL
          + VERSION_RANGE_STR, versionRange);
    ...
    Preconditions.checkState(channelTypesOptional.isPresent(),
      COULD_NOT_PARSE_CHANNEL_TYPES_FOR_SITE_CHAN_DAO + " %s.%s", siteChanDao.getId().getStationCode(),
  ...
  public Channel convertToBeamDerived(SiteDao siteDao,
    ...
    Preconditions.checkNotNull(siteChanDao, String.format(SITE_CHAN_MUST_NOT_BE_NULL
      + EFFECTIVE_TIME_STR, channelEffectiveTime));
    Preconditions.checkNotNull(siteDao, String.format(SITE_MUST_NOT_BE_NULL
      + EFFECTIVE_TIME_STR, channelEffectiveTime));
    Preconditions.checkNotNull(wfdiscDao, String.format("Wfdisc cannot be null."
      + EFFECTIVE_TIME_STR, channelEffectiveTime));
    Preconditions.checkNotNull(beamDao, String.format("Beam cannot be null."
      + EFFECTIVE_TIME_STR, channelEffectiveTime));
  ...
  /**
   * Converts the SiteChan and Site objects to a {@link Channel} coi version
   * reference
   *
   * @param siteDao the corresponding site of the channel
   * @param siteChanDao the SiteChan that represents the channels
   * characteristics
   *
  ...
  public Channel convertToVersionReference(SiteDao siteDao, SiteChanDao siteChanDao) {
    Preconditions.checkNotNull(siteDao, SITE_MUST_NOT_BE_NULL);
    Preconditions.checkNotNull(siteChanDao, SITE_CHAN_MUST_NOT_BE_NULL + 
      " For site %s", siteDao.getId().getStationCode());
