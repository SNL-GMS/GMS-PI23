...
public class SiteAndSiteChanUtility {
  ...
  private SiteAndSiteChanUtility() {
    ...
    /**
   * Update whether the start or end of a series of sites for a specific sta 
   * have overlaps and return the sites as a list
   *
   ...
   * @return {@link SiteDao}
   */
  public static List<SiteDao> updateStartEndAndReturnSiteDaos(StartAndEndForSiteAndSiteChan startEnd,
    ...
    for (SiteAndSurroundingDates siteAndSurrounding : siteAndSurroundingDates) {
      ...
      if ((curr != null && curr.getSiteDao().getId().getOnDate().isAfter(siteAndSurrounding.getSiteDao().getId().getOnDate()))
        || curr == null) {
      ...
      if ((curr != null && curr.getSiteDao().getOffDate().isBefore(siteAndSurrounding.getSiteDao().getOffDate()))
        || curr == null) {
    ...
    for (SiteAndSurroundingDates surrounding : staCodeToMinTime.values()) {
      ...
      startEnd.setPrevTimeOverLapForSite(surrounding.getSiteDao(), 
        surrounding.getPreviousOffDate().isPresent());
    }
    for (SiteAndSurroundingDates surrounding : staCodeToMaxTime.values()) {
      
      startEnd.setNextTimeOverLapForSite(surrounding.getSiteDao(),
        surrounding.getNextOnDate().isPresent());
    ...
    return siteDaos;
      ...
      /**
   * Update whether the start or end of a series of sitechans for a specific sta + chan
   * have overlaps and return the sitechan as a list
   *
   ...
   * @return {@link SiteDao}
   */
  public static List<SiteChanDao> updateStartEndAndReturnSiteChanDaos(StartAndEndForSiteAndSiteChan startEnd,
    ...
    for (SiteChanAndSurroundingDates siteChanAndSurrounding : siteChanAndSurroundingDates) {
      ...
      if ((curr != null && curr.getSiteChanDao().getId().getOnDate().isAfter(siteChanAndSurrounding.getSiteChanDao().getId().getOnDate()))
        || curr == null) {
      ...
      if ((curr != null && curr.getSiteChanDao().getOffDate().isBefore(siteChanAndSurrounding.getSiteChanDao().getOffDate()))
        || curr == null) {
    ...
    for (SiteChanAndSurroundingDates surrounding : staChanCodeToMinTime.values()) {
      ...
      startEnd.setPrevTimeOverLapForSiteChan(surrounding.getSiteChanDao(),
        surrounding.getPreviousOffDate().isPresent());
    }
    for (SiteChanAndSurroundingDates surrounding : staChanCodeToMaxTime.values()) {

      startEnd.setNextTimeOverLapForSiteChan(surrounding.getSiteChanDao(),
        surrounding.getNextOnDate().isPresent());
    }
