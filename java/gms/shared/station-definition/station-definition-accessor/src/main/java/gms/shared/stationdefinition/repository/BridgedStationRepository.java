...
public class BridgedStationRepository implements StationRepositoryInterface {
  ...
  public List<Station> findStationsByNameAndTime(List<String> stationNames, Instant effectiveAt) {
    ...
    List<SiteDao> siteDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteDaos(startEndBooleans, sitesAndSurroundingDates);
    ...
    List<SiteChanAndSurroundingDates> siteChanAndSurroundingDates = siteChanDatabaseConnector
      .findSiteChansAndSurroundingDatesByStationCodeAndTimeRange(stationCodes,
        minMaxPair.getLeft(), minMaxPair.getRight());

    List<SiteChanDao> siteChanDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteChanDaos(startEndBooleans, siteChanAndSurroundingDates);
    ...
    var stationDefinitionDataHolder = new StationDefinitionDataHolder(siteDaos,
      siteChanDaos, null, null, null, startEndBooleans);

    List<Channel> channels = channelUtil.findChannelsGivenSiteAndSiteChan(
    ...
    List<ChannelGroup> channelGroups = channelGroupRepository
      .findChannelGroupsGivenChannelsSitesAndSiteChansAndTimeRange(stationDefinitionDataHolder,
        channels, minMaxPair.getLeft(), minMaxPair.getRight());
    ...
    return stationAssembler.buildAllForTime(siteDaos, siteChanDaos,
      channelGroups, channels, effectiveAt, channelNameSiteChanKeyMap, startEndBooleans);
  }
  ...
  public List<Station> findStationsByNameAndTimeRange(List<String> stationNames,
    ...
    var siteDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteDaos(startEndBooleans, sitesAndSurroundingDates);
    ...
    var siteChanDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteChanDaos(startEndBooleans, siteChanAndSurroundingDates);
    ...
    List<ChannelGroup> channelGroups = channelGroupRepository
      .findChannelGroupsGivenChannelsSitesAndSiteChansAndTimeRange(stationDefinitionDataHolder, channels, minMaxPair.getLeft(), minMaxPair.getRight());
    ...
    return stationAssembler.buildAllForTimeRange(siteDaos, siteChanDaos,
      ...
      startTime, endTime, channelNameSiteChanKeyMap, startEndBooleans);
  }
