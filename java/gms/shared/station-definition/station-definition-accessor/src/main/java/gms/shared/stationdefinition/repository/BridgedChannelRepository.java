...
public class BridgedChannelRepository implements ChannelRepositoryInterface {
  ...
  public List<Channel> findChannelsByNameAndTime(List<String> channelNames, Instant effectiveAt) {
    ...
    channels.addAll(channelAssembler.buildAllForTime(
      effectiveAt, data.getSiteDaos(), data.getSiteChanDaos(), data.getSensorDaos(),
      data.getWfdiscVersions(), responses, data.getStartEndBoolean()));
    ...
    Map<String, Station> stationNameMap = stationRepository.findStationsByNameAndTime(stationNames, effectiveAt).stream()
    ...
    channels = channels.stream()
      .map(channel -> {
        ...
        if (posStation != null && posStation.getEffectiveAt().isPresent()) {
          newchan = channel.toBuilder().setData(channel.getData().get().toBuilder()
            .setStation(Station.createVersionReference(posStation.getName(), posStation.getEffectiveAt().get())).build()).build();
        }
  ...
  public List<Channel> findChannelsByNameAndTimeRange(List<String> channelNames, Instant startTime, Instant endTime) {
    ...
    channels.addAll(channelAssembler.buildAllForTimeRange(
      startTime, endTime, data.getSiteDaos(), data.getSiteChanDaos(), data.getSensorDaos(),
      data.getWfdiscVersions(), responses, data.getStartEndBoolean()));
    ...
    return channelNames.stream()
      ...
      .filter(object -> {
        ...
        logger.debug("Attempting to retrieve entities of type {} from the cache, but "
          + "retrieved a null value, ignoring", Channel.class.toString());
        return false;
      })
