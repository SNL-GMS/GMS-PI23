...
public class BridgedChannelGroupRepository implements ChannelGroupRepositoryInterface {
  ...
  public List<ChannelGroup> findChannelGroupsByNameAndTime(List<String> channelGroupNames, Instant effectiveAt) {
    ...
    List<SiteDao> siteDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteDaos(startAndEnd, sitesAndSurroundingDates);
    ...
    var siteChanDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteChanDaos(startAndEnd, siteChanAndSurroundingDates);
    ...
    var stationDefinitiondataHolder = new StationDefinitionDataHolder(siteDaos, siteChanDaos,
      null, null, null, startAndEnd);

    List<Channel> channels = bridgedChannelUtil.findChannelsGivenSiteAndSiteChan(
    ...
    return channelGroupAssembler.buildAllForTime(siteDaos, siteChanDaos, effectiveAt, channelsByStaChan, startAndEnd);
  }

  public List<ChannelGroup> findChannelGroupsGivenChannelsSitesAndSiteChans(StationDefinitionDataHolder stationDefinitionDataHolder, Instant effectiveAt, List<Channel> channels) {

    var siteDaosForChannelGroup = stationDefinitionDataHolder.getSiteDaos().stream()
      .filter(siteDao -> !(siteDao.getId().getStationCode().equalsIgnoreCase(siteDao.getReferenceStation())
      && siteDao.getStaType() == StaType.ARRAY_STATION))
      .collect(Collectors.toList());
    ...
    return channelGroupAssembler.buildAllForTime(siteDaosForChannelGroup, stationDefinitionDataHolder.getSiteChanDaos(),
      effectiveAt, channelsByStaChan, stationDefinitionDataHolder.getStartEndBoolean());
  }
  ...
  public List<ChannelGroup> findChannelGroupsByNameAndTimeRange(List<String> channelGroupNames,
    ...
    var siteDaos = SiteAndSiteChanUtility.updateStartEndAndReturnSiteDaos(startAndEnd, sitesAndSurroundingDates).stream()
      .filter(siteDao -> !(siteDao.getId().getStationCode().equalsIgnoreCase(siteDao.getReferenceStation())
      && siteDao.getStaType() == StaType.ARRAY_STATION))
      .collect(Collectors.toList());
    ...
    var siteChans = SiteAndSiteChanUtility.updateStartEndAndReturnSiteChanDaos(startAndEnd, siteChanAndSurroundingDates);
    ...
    List<Channel> channels = bridgedChannelUtil.findChannelsGivenSiteAndSiteChan(stationDefinitiondataHolder,
      minMaxPair.getLeft(), minMaxPair.getRight());
    Table<String, String, NavigableMap<Instant, Channel>> channelsByStaChan
      = AssemblerUtils.buildVersionTable(
        Functions.compose(StationDefinitionIdUtility::getCssKeyFromName, Channel::getName).andThen(SiteChanKey::getStationCode),
        ...
        Functions.compose(Optional::get, Channel::getEffectiveAt),
        channels);
    return channelGroupAssembler.buildAllForTimeRange(siteDaos, siteChans, startTime, endTime, channelsByStaChan, startAndEnd);
  }

  public List<ChannelGroup> findChannelGroupsGivenChannelsSitesAndSiteChansAndTimeRange(StationDefinitionDataHolder stationDefinitionDataHolder,
    ...
    return channelGroupAssembler.buildAllForTimeRange(siteDaosForChannelGroup, stationDefinitionDataHolder.getSiteChanDaos(),
      startTime, endTime, channelsByStaChan, stationDefinitionDataHolder.getStartEndBoolean());
  }
}
