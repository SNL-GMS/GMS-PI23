...
public class NetworkDatabaseConnector extends DatabaseConnector {
  ...
  @Autowired
  public NetworkDatabaseConnector(@Qualifier("entityManagerFactory")
  ...
  public Optional<NetworkDao> findNetwork(String net) {
    ...
    if (net == null || net.isEmpty()) {
      ...
      EntitySingleResultFunction<NetworkDao> delegateFunc = entityManager -> {
      ...
      return runWithEntityManagerSingleResultFunction(delegateFunc,
        NETWORK_ERROR, String.format(NETWORK_MESSAGE, net));
    }
  }
  ...
  public List<NetworkDao> findNetworks(Collection<String> nets) {
    ...
    if (nets.isEmpty()) {
      ...
      return runPartitionedQuery(nets, 950, partition -> {
        EntityResultListFunction<NetworkDao> delegateFunc = entityManager -> {
          CriteriaBuilder cb = entityManager.getCriteriaBuilder();
          CriteriaQuery<NetworkDao> query = cb.createQuery(NetworkDao.class);
          Root<NetworkDao> fromNetwork = query.from(NetworkDao.class);
          query.select(fromNetwork);
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          NETWORK_IDS_ERROR, String.format("network ids size %s", nets.size()));
      });
    }
  }
}
