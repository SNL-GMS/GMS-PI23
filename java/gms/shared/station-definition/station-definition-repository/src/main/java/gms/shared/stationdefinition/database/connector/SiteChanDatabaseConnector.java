...
public class SiteChanDatabaseConnector extends DatabaseConnector {
  ...
  static final String SITE_CHAN_CODES_EFFECTIVE_TIME_ERROR = "Site chan by codes and time exception";
  static final String SITE_CHAN_KEYS_TIME_RANGE_ERROR = "Site chan by keys and time range exception";
  static final String SITE_CHAN_CODES_TIME_RANGE_ERROR = "Site chan by codes and time range exception";

  static final String SITE_CHAN_SURROUNDING_DATES_CODES_TIME_ERROR = "Site chan surround dates by codes and time exception";
  static final String SITE_CHAN_SURROUNDING_DATES_CODES_TIME_RANGE_ERROR = "Site chan surround dates by codes and time rnage exception";
  static final String SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_ERROR = "Site chan surround dates by keys and time exception";
  static final String SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_RANGE_ERROR = "Site chan surround dates by keys and time range exception";
  ...
  static final String SITE_CHAN_SURROUNDING_DATES_CODES_TIME_RANGE_MESSAGE = "codes %s and time range %s - %s";
  static final String SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_MESSAGE = "keys %s and time %s";
  static final String SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_RANGE_MESSAGE = "keys %s and time range %s - %s";

  @Autowired
  public SiteChanDatabaseConnector(@Qualifier("entityManagerFactory")
  ...
  public Optional<SiteChanDao> findSiteChan(SiteChanKey siteChanKey) {
    ...
    EntitySingleResultFunction<SiteChanDao> delegateFunc = entityManager -> {
    ...
    return runWithEntityManagerSingleResultFunction(delegateFunc,
      SITE_CHAN_KEY_ERROR, errMessage);
  }
  ...
  public List<SiteChanDao> findSiteChansByStationCodeAndTime(Collection<String> stationCodes,
    ...
    var errMessage = String.format(SITE_CHAN_CODES_TIME_MESSAGE, stationCodes.size(), effectiveTime);
    if (stationCodes.isEmpty()) {
      ...
      return runPartitionedQuery(stationCodes, 950, partition -> {
        EntityResultListFunction<SiteChanDao> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITE_CHAN_CODES_EFFECTIVE_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteChanDao> findSiteChansByStationCodeAndTimeRange(Collection<String> stationCodes,
    ...
    var errMessage = String.format(SITE_CHAN_CODES_TIME_RANGE_MESSAGE, stationCodes.size(),
      startTime, endTime);
    if (stationCodes.isEmpty()) {
      logger.debug(MISSING_STATION_CODES_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(stationCodes, 400, partition -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITE_CHAN_CODES_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteChanDao> findSiteChansByNameAndTimeRange(Collection<SiteChanKey> siteChanKeys,
    ...
    var errMessage = String.format(SITE_CHAN_KEYS_TIME_RANGE_MESSAGE,
    ...
    if (siteChanKeys.isEmpty()) {
      ...
      return runPartitionedQuery(siteChanKeys, 250, keySubList -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITE_CHAN_KEYS_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteChanDao> findSiteChansByTimeRange(Instant startTime, Instant endTime) {
    ...
    return runWithEntityManagerResultListFunction(delegateFunc,
      SITE_CHAN_TIME_RANGE_ERROR, errMessage);
  }
  ...
  public List<SiteChanDao> findSiteChansByKeyAndTime(List<SiteChanKey> siteChanKeys, Instant effectiveAt) {
    ...
    var errMessage = String.format(SITE_CHAN_KEYS_EFFECTIVE_TIME_MESSAGE, siteChanKeys.size(), effectiveAt);
    return runPartitionedQuery(siteChanKeys, 500, keySublist -> {
      ...
      return runWithEntityManagerResultListFunction(delegateFunc,
        SITE_CHAN_KEYS_EFFECTIVE_TIME_ERROR, errMessage);
    });
  }
  ...
  public List<SiteChanAndSurroundingDates> findSiteChansAndSurroundingDatesByStationCodeAndTime(
    ...
    var errMessage = String.format(SITE_CHAN_SURROUNDING_DATES_CODES_TIME_MESSAGE,
    ...
    if (stationCodes.isEmpty()) {
      ...
      return runPartitionedQuery(stationCodes, 950, partition -> {
        EntityResultListFunction<SiteChanAndSurroundingDates> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITE_CHAN_SURROUNDING_DATES_CODES_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteChanAndSurroundingDates> findSiteChansAndSurroundingDatesByStationCodeAndTimeRange(
    ...
    var errMessage = String.format(SITE_CHAN_SURROUNDING_DATES_CODES_TIME_RANGE_MESSAGE,
    ...
    if (stationCodes.isEmpty()) {
      ...
      return runPartitionedQuery(stationCodes, 950, partition -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITE_CHAN_SURROUNDING_DATES_CODES_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteChanAndSurroundingDates> findSiteChansAndSurroundingDatesByKeysAndTime(
    ...
    var errMessage = String.format(SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_MESSAGE,
      siteChanKeys.size(), effectiveAt);
    return runPartitionedQuery(siteChanKeys, 950, partition -> {
      EntityResultListFunction<SiteChanAndSurroundingDates> delegateFunc = entityManager -> {
        var cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<SiteChanAndSurroundingDates> siteChanQuery = cb
          .createQuery(SiteChanAndSurroundingDates.class);
      ...
      return runWithEntityManagerResultListFunction(delegateFunc,
        SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_ERROR, errMessage);
    });
  }
  ...
  public List<SiteChanAndSurroundingDates> findSiteChansAndSurroundingDatesByKeysAndTimeRange(
    ...
    var errMessage = String.format(SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_RANGE_MESSAGE,
      siteChanKeys.size(), startTime, endTime);
    return runPartitionedQuery(siteChanKeys, 950, partition -> {
      EntityResultListFunction<SiteChanAndSurroundingDates> delegateFunc = entityManager -> {
        var cb = entityManager.getCriteriaBuilder();
      ...
      return runWithEntityManagerResultListFunction(delegateFunc,
        SITE_CHAN_SURROUNDING_DATES_KEYS_TIME_RANGE_ERROR, errMessage);
    });
  }
