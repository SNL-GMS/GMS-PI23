...
public class WftagDatabaseConnector extends DatabaseConnector {
  ...
  @Autowired
  public WftagDatabaseConnector(@Qualifier("entityManagerFactory")
  ...
  public List<WfTagDao> findWftagsByTagIds(Collection<Long> tagIds) {
    ...
    if (tagIds.isEmpty()) {
      ...
      return runPartitionedQuery(tagIds, 250, partitionedTagIds -> {
        EntityResultListFunction<WfTagDao> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          WFTAGS_BY_IDS_ERROR, errMessage);
      });
    }
  }
}
