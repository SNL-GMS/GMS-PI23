...
public class FilterGroupDatabaseConnector extends DatabaseConnector {
  ...
  public FilterGroupDatabaseConnector(@Qualifier("entityManagerFactory") EntityManagerFactory entityManagerFactory) {

    super(entityManagerFactory);
  }

  /**
   * Finds and returns all FilterGroupDaos based on the passed in Parent FIlter
   * Ids
   *
  ...
  public List<FilterGroupDao> findFilterGroupsByIds(Collection<Long> parentFilterIds) {
    ...
    if (parentFilterIds == null || parentFilterIds.isEmpty()) {
      logger.debug("Request for Filter Group was given with empty/null list of parent filterIds ids");
      return new ArrayList<>();
    } else {

      var errorMessage = String.format(PARENT_FILTER_ID_SIZE_MESSAGE, parentFilterIds.size());
      return runPartitionedQuery(parentFilterIds, 950, partitioneParentFilterIds -> {
        EntityResultListFunction<FilterGroupDao> delegateFunction = entityManager -> {
          ...
          query.select(fromFilterGroup);
          query.where(fromFilterGroup.get(FILTER_GROUP_KEY).get(PARENT_FILTER_ID).in(partitioneParentFilterIds));
          return entityManager.createQuery(query).getResultList();
        };

        return runWithEntityManagerResultListFunction(delegateFunction,
          FILTER_GROUP_ID_ERROR, errorMessage);
      });
    }
  }
}
