...
public class InstrumentDatabaseConnector extends DatabaseConnector {
  ...
  @Autowired
  public InstrumentDatabaseConnector(@Qualifier("entityManagerFactory") 
  ...
  public List<InstrumentDao> findInstruments(Collection<Long> instrumentIds) {
    ...
    if (instrumentIds == null || instrumentIds.isEmpty()) {
      ...
      return runPartitionedQuery(instrumentIds, 950, partition -> {
        EntityResultListFunction<InstrumentDao> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          INSTRUMENTS_ERROR, String.format("instrument ids size = %s", instrumentIds.size()));
      });
    }
  }
}
