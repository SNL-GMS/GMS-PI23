...
public class SiteDatabaseConnector extends DatabaseConnector {
  ...
  static final String SITES_BY_REF_STATION_AND_TIME_ERROR = "Sites by ref stations and time exception";
  static final String SITES_BY_REF_STATION_AND_TIME_RANGE_ERROR = "Sites by ref stations and time range exception";
  static final String SITES_BY_STATIONS_AND_TIME_RANGE_ERROR = "Sites by station names and time range exception";
  static final String SITES_BY_TIME_RANGE_ERROR = "Sites by time range exception";

  static final String SITES_SURROUNDING_DATES_BY_REF_STATION_AND_TIME_ERROR = "Sites and surrounding dates by ref stations and time exception";
  static final String SITES_SURROUNDING_DATES_BY_REF_STATION_AND_TIME_RANGE_ERROR = "Sites and surrounding dates by ref stations and time range exception";
  static final String SITES_SURROUNDING_DATES_BY_STATIONS_AND_TIME_ERROR = "Sites and surrounding dates by stations and time exception";
  static final String SITES_SURROUNDING_DATES_BY_STATIONS_AND_TIME_RANGE_ERROR = "Sites and surrounding dates by stations and time range exception";
  ...
  static final String SITES_BY_REF_STATION_TIME_RANGE_MESSAGE = "ref stations size %s, time range %s - %s";
  ...
  static final String SITES_SURROUNDING_DATES_BY_REF_STATION_TIME_MESSAGE = "ref stations size %s, time %s";
  static final String SITES_SURROUNDING_DATES_BY_REF_STATION_TIME_RANGE_MESSAGE = "ref stations size %s, time range %s - %s";
  static final String SITES_SURROUNDING_DATES_BY_STATIONS_TIME_MESSAGE = "stations size %s, time %s";
  static final String SITES_SURROUNDING_DATES_BY_STATIONS_TIME_RANGE_MESSAGE = "ref stations size %s, time range %s - %s";

  @Autowired
  public SiteDatabaseConnector(@Qualifier("entityManagerFactory")
  ...
  public List<SiteDao> findSitesByStationCodes(Collection<String> stationNames) {
    ...
    if (stationNames.isEmpty()) {
      ...
      return runPartitionedQuery(stationNames, 950, partition -> {
        EntityResultListFunction<SiteDao> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_BY_CODES_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteDao> findSitesByRefStation(Collection<String> refStationNames) {
    ...
    if (refStationNames.isEmpty()) {
      ...
      return runPartitionedQuery(refStationNames, 950, partition -> {
        EntityResultListFunction<SiteDao> delegateFunction = entityManager -> {
          CriteriaBuilder cb = entityManager.getCriteriaBuilder();
          CriteriaQuery<SiteDao> query = cb.createQuery(SiteDao.class);
          Root<SiteDao> fromSite = query.from(SiteDao.class);
          query.select(fromSite);
        ...
        return runWithEntityManagerResultListFunction(delegateFunction,
          SITES_BY_REF_STATION_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteDao> findSitesByStationCodesAndStartTime(Collection<String> stationNames,
    ...
    var errMessage = String.format(SITES_BY_STATION_CODES_TIME_MESSAGE,
      stationNames.size(), start);
    if (stationNames.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(stationNames, 950, partition -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_BY_CODES_AND_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteDao> findSitesByRefStationAndStartTime(Collection<String> stationNames,
    ...
    var errMessage = String.format(SITES_BY_REF_STATION_TIME_MESSAGE, stationNames.size(),
      start);
    if (stationNames.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(stationNames, 200, partition -> {
        EntityResultListFunction<SiteDao> delegateFunc = entityManager -> {
          CriteriaBuilder cb = entityManager.getCriteriaBuilder();
          CriteriaQuery<SiteDao> query = cb.createQuery(SiteDao.class);
          Root<SiteDao> fromSite = query.from(SiteDao.class);
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_BY_REF_STATION_AND_TIME_ERROR, errMessage);
      });
    }
  ...
  public List<SiteDao> findSitesByReferenceStationAndTimeRange(List<String> referenceStations,
    ...
    var errMessage = String.format(SITES_BY_REF_STATION_TIME_RANGE_MESSAGE,
      referenceStations.size(), startTime, endTime);
    if (referenceStations.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return List.of();
    } else {
      return runPartitionedQuery(referenceStations, 900, referenceStationsSubList -> {
        EntityResultListFunction<SiteDao> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_BY_REF_STATION_AND_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteDao> findSitesByNamesAndTimeRange(Collection<String> stationNames,
    ...
    var errMessage = String.format(SITES_BY_STATIONS_TIME_RANGE_MESSAGE, stationNames.size(),
      startTime, endTime);
    if (stationNames.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
      ...
      return runPartitionedQuery(stationNames, 950, partition -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_BY_STATIONS_AND_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteDao> findSitesByTimeRange(Instant startTime, Instant endTime) {
    ...
    return runWithEntityManagerResultListFunction(delegateFunc,
      SITES_BY_TIME_RANGE_ERROR, errMessage);
  }
  ...
  public List<SiteAndSurroundingDates> findSitesAndSurroundingDatesByRefStaAndTime(
    ...
    var errMessage = String.format(SITES_SURROUNDING_DATES_BY_REF_STATION_TIME_MESSAGE,
      refStas.size(), effectiveAt);
    if (refStas.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(refStas, 950, partition -> {
        EntityResultListFunction<SiteAndSurroundingDates> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_SURROUNDING_DATES_BY_REF_STATION_AND_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteAndSurroundingDates> findSitesAndSurroundingDatesByRefStaAndTimeRange(
    ...
    var errMessage = String.format(SITES_SURROUNDING_DATES_BY_REF_STATION_TIME_RANGE_MESSAGE,
      refStas.size(), startTime, endTime);
    if (refStas.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(refStas, 950, partition -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_SURROUNDING_DATES_BY_REF_STATION_AND_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteAndSurroundingDates> findSitesAndSurroundingDatesByStaCodeAndTime(
    ...
    var errMessage = String.format(SITES_SURROUNDING_DATES_BY_STATIONS_TIME_MESSAGE,
      stationCodes.size(), effectiveAt);
    if (stationCodes.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(stationCodes, 950, partition -> {
        EntityResultListFunction<SiteAndSurroundingDates> delegateFunc = entityManager -> {
          var cb = entityManager.getCriteriaBuilder();
          CriteriaQuery<SiteAndSurroundingDates> siteQuery = cb
            .createQuery(SiteAndSurroundingDates.class);
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SITES_SURROUNDING_DATES_BY_STATIONS_AND_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SiteAndSurroundingDates> findSitesAndSurroundingDatesByStaCodeAndTimeRange(
    ...
    var errMessage = String.format(SITES_SURROUNDING_DATES_BY_STATIONS_TIME_RANGE_MESSAGE,
      stationCodes.size(), startTime, endTime);
    if (stationCodes.isEmpty()) {
      logger.debug(MISSING_STATION_NAME_LIST_ERROR);
      return new ArrayList<>();
    } else {
  ...
  private CriteriaQuery<SiteAndSurroundingDates> getSiteAndSurroundingDatesQuery(
    ...
    previousOffDateQuery.select(subFromSiteOffDate.<Instant>get(OFF_DATE))
      .where(cb.and(
    ...
    nextOnDateQuery.select(subFromSiteOnDate.get(ID).<Instant>get(ON_DATE))
      .where(cb.and(
