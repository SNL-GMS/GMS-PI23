...
public class SensorDatabaseConnector extends DatabaseConnector {
  ...
  public Optional<SensorDao> findSensorByKeyInRange(String station,
    ...
    EntitySingleResultFunction<SensorDao> delegateFunc = entityManager -> {
    ...
    return runWithEntityManagerSingleResultFunction(delegateFunc, SENSORS_KEY_RANGE_ERROR, errMessage);
  }

  public List<SensorDao> findSensorsByChannelIdAndTimeRange(Collection<Long> channelIds, 
    ...
    if (channelIds.isEmpty()) {
      ...
      return runPartitionedQuery(channelIds, 950, channelIdSublist -> {
        EntityResultListFunction<SensorDao> delegateFunc = entityManager -> {
          var builder = entityManager.getCriteriaBuilder();
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          SENSORS_IDS_TIME_RANGE_ERROR, errMessage);
      });
    }
  }

  /**
   * Finds all {@link SensorDao}s for siteChanKeys, starting at effectiveTime
   ...
   * @param effectiveTime
   *
   * @return List of all Sensor for the supplied siteChanKeys that create a
  ...
  public List<SensorDao> findSensorVersionsByNameAndTime(List<SiteChanKey> siteChanKeys,
    ...
    if (siteChanKeys.isEmpty()) {
      ...
      return runPartitionedQuery(siteChanKeys, 500, keySubList -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc, SENSORS_IDS_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  public List<SensorDao> findSensorsByKeyAndTime(List<SiteChanKey> siteChanKeys, Instant effectiveAt) {
    ...
    return runPartitionedQuery(siteChanKeys, 500, keySublist -> {
      ...
      return runWithEntityManagerResultListFunction(delegateFunc,
        SENSORS_KEY_TIME_ERROR, errMessage);
    });
  }
  ...
  public List<SensorDao> findSensorsByKeyAndTimeRange(List<SiteChanKey> siteChanKeys,
    ...
    var errMessage = String.format(IDS_TIME_RANGE_MESSAGE, siteChanKeys.size(),
      startTime, endTime);
    if (siteChanKeys.isEmpty()) {
  ...
  /**
   * subquery to retreive the start time of the next version. For Sensor, that
   * is defined as a change in tShift value
   *
   * @param cb
   * @param query
   * @param k
