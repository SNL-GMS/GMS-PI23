...
public class WfdiscDatabaseConnector extends DatabaseConnector {
  ...
  static final String WFDISCS_BY_SITECHAN_KEYS_TIME_ERROR = "Wfdiscs by sitechan keys and time exception";
  ...
  static final String WFDISCS_BY_TIME_RANGE_CREATION_TIME_ERROR = "Wfdiscs by time range and creation time exception";
  static final String WFDISCS_BY_NAMES_TIME_ERROR = "Wfdiscs by station names and time exception";
  static final String WFDISCS_BY_SITECHAN_KEYS_TIME_RANGE_ERROR = "Wfdiscs by sitechan keys and time range exception";
  ...
  static final String WFDISCS_BY_TIME_RANGE_CREATION_TIME_MESSAGE = "time range %s - %s, creation time %s";
  static final String WFDISCS_BY_NAMES_TIME_MESSAGE = "station names %s, time %s";
  static final String WFDISCS_BY_SITECHAN_KEYS_TIME_RANGE_MESSAGE = "sitechan keys %s, time range %s - %s";
  ...
  public List<WfdiscDao> findWfdiscsByWfids(Collection<Long> wfids) {
    ...
    if (wfids.isEmpty()) {
      ...
      return runPartitionedQuery(wfids, 250, partitionedWfids -> {
        EntityResultListFunction<WfdiscDao> delegateFunc = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          WFDISCS_BY_WFIDS_ERROR, errMessage);
      });
    }
  }

  /**
   * Finds all {@link WfdiscDao}s for siteChanKeys, starting at effectiveTime
   * that have the same Response version info. Response Versions look at calib
   * and calper fields
   *
   * @param siteChanKeys
   * @param effectiveTime
   *
   * @return List of all WfDisc for the supplied siteChanKeys that create a
   * version starting at effectiveTime
   */
  public List<WfdiscDao> findWfdiscVersionsByNameAndTime(Collection<SiteChanKey> siteChanKeys,
    ...
    var errMessage = String.format(WFDISCS_BY_SITECHAN_KEYS_TIME_MESSAGE, siteChanKeys.size(),
      effectiveTime);
    if (siteChanKeys.isEmpty()) {
      logger.debug(EMPTY_LIST_OF_KEYS_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(siteChanKeys, 500, keySubList -> {
        EntityResultListFunction<WfdiscDao> delegateFunc = entityManager -> {
          var cb = entityManager.getCriteriaBuilder();
          CriteriaQuery<WfdiscDao> query = cb.createQuery(WfdiscDao.class);
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          WFDISCS_BY_SITECHAN_KEYS_TIME_ERROR, errMessage);
      });
    }
  }

  /**
   * Finds all {@link WfdiscDao}s for the given station code and channel code
   * pairs at the specified effective time.
   * <p>
   * select * from wfdisc where wfdisc.endtime >= eff_time and wfdisc.time <=
   * eff_time and segtype = 'o' and ((sta = 'AS01' and chan='BHZ') or (sta =
   * 'MK01' and chan = 'SHZ')) <p>
   * AS01/BHZ MK01/SHZ @param siteChanKeys @param effectiveTime @return
   */
  public List<WfdiscDao> findWfdiscsByNameAndTime(Collection<SiteChanKey> siteChanKeys,
    ...
    var errMessage = String.format(WFDISCS_BY_NAMES_TIME_MESSAGE, siteChanKeys.size(),
      effectiveTime);
    if (siteChanKeys.isEmpty()) {
      logger.debug(EMPTY_LIST_OF_KEYS_ERROR);
      return new ArrayList<>();
      ...
      return runPartitionedQuery(siteChanKeys, 500, keySubList -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunc,
          WFDISCS_BY_NAMES_TIME_ERROR, errMessage);
      });
    }
  }
  ...
  /**
   * Finds all {@link WfdiscDao}s for the given station code and channel code
   * pairs that have a {@link StationChannelTimeKey#getTime()} and
   * {@link WfdiscDao#getEndTime()} that falls within the queried time range as
   * well as the greatest wfdisc before the query sstart time specified by the
   * inputs such that:
   * <p>
  ...
  public List<WfdiscDao> findWfdiscsByNameAndTimeRange(Collection<SiteChanKey> siteChanKeys,
    ...
    var errMessage = String.format(WFDISCS_BY_SITECHAN_KEYS_TIME_RANGE_MESSAGE,
      siteChanKeys.size(), startTime, endTime);
    if (siteChanKeys.isEmpty()) {
      logger.debug(EMPTY_LIST_OF_KEYS_ERROR);
      return new ArrayList<>();
    } else {
      return runPartitionedQuery(siteChanKeys, 250, keySubList -> {
        EntityResultListFunction<WfdiscDao> delegateFunc = entityManager -> {
          var cb = entityManager.getCriteriaBuilder();
          CriteriaQuery<WfdiscDao> query = cb.createQuery(WfdiscDao.class);
          Root<WfdiscDao> fromWfdisc = query.from(WfdiscDao.class);

          Predicate[] keyPredicates = keySubList.stream()
            .map(k -> cb.and(
            cb.equal(fromWfdisc.get(STATION_CODE), k.getStationCode()),
            cb.equal(fromWfdisc.get(CHANNEL_CODE), k.getChannelCode())
          )).
          ...
          var finalQuery = buildWfdiscsQuery(keySubList,
            timeRangePredicate, segTypePredicate, cb, query,
            fromWfdisc, startTime);

          return entityManager.createQuery(finalQuery).getResultList();
        };

        return runWithEntityManagerResultListFunction(delegateFunc,
          WFDISCS_BY_SITECHAN_KEYS_TIME_RANGE_ERROR, errMessage);
      });
    }
  }
   ...
   * @return
   */
  private CriteriaQuery<WfdiscDao> buildWfdiscsQuery(Collection<SiteChanKey> keySubList,
    ...
    Subquery<Float> prevWfdiscSubQuery = wfdiscQuery.subquery(Float.class);
    Root<WfdiscDao> subFromWfdisc = prevWfdiscSubQuery.from(WfdiscDao.class);
    Expression<Float> expr = cb.max(subFromWfdisc.get(TIME));
    ...
    prevWfdiscSubQuery.select(expr)
      .where(cb.and(
        cb.or(keyPredicates),
        cb.lessThan(subFromWfdisc.get(TIME), startTime)
      ));

    var maxTimeSubPredicate = cb.equal(fromWfdisc.get(TIME),
      prevWfdiscSubQuery.getSelection());

    return wfdiscQuery.select(fromWfdisc)
      .where(cb.or(maxTimeSubPredicate,
        timeRangePredicate),
        segTypePredicate)
  ...
  public List<WfdiscDao> findWfDiscVersionAfterEffectiveTime(Collection<SiteChanKey> siteChanKeys) {
    ...
    List<Tuple> resultSet = runPartitionedQuery(siteChanKeys, 250, partitionedSiteChanKeys -> {
      EntityResultListFunction<Tuple> delegateFunc = entityManager -> {
        var cb = entityManager.getCriteriaBuilder();
      ...
      return runWithEntityManagerResultListFunction(delegateFunc,
        WFDISCS_BY_SITECHAN_KEYS_ERROR, errMessage);
    });

    return runPartitionedQuery(resultSet, 250, partitionedResultSet -> {
      EntityResultListFunction<WfdiscDao> delegateFunc = entityManager -> {
        var cb = entityManager.getCriteriaBuilder();
        CriteriaQuery<WfdiscDao> query = cb.createQuery(WfdiscDao.class);
        Root<WfdiscDao> fromWfdisc = query.from(WfdiscDao.class);
  ...
  public List<WfdiscDao> findWfdiscsByTimeRange(Instant startTime, Instant endTime) {
    ...
    EntityResultListFunction<WfdiscDao> delegateFunction = entityManager -> {
    ...
    return runWithEntityManagerResultListFunction(delegateFunction,
      WFDISCS_BY_TIME_RANGE_ERROR, errMessage);
  }

  /**
   * Finds all {@link WfdiscDao}s for the given station code and channel code
   * pairs that have a {@link StationChannelTimeKey#getTime()} and
   * {@link WfdiscDao#getEndTime()} that falls within the queried time range and
   * Segment type specified by the inputs such that:
   * <p>
   * {@link StationChannelTimeKey#getTime()} {@literal <=} endTime && {@link WfdiscDao#getEndTime()}
   ...
   * @param creationTime the maximum creation time (load date) allowed for the
   * wfdisc
   *
  ...
  public List<WfdiscDao> findWfdiscsByNameTimeRangeAndCreationTime(
    ...
    var errMessage = String.format(WFDISCS_BY_TIME_RANGE_CREATION_TIME_MESSAGE,
    ...
    return runWithEntityManagerResultListFunction(delegateFunc,
      WFDISCS_BY_TIME_RANGE_CREATION_TIME_ERROR, errMessage);
  }

  /**
   * subquery to retreive the start time of the next version. For Wfdisc, that
   * is defined as a change in calper or calib values
   *
   * @param cb
   * @param query
   * @param k
