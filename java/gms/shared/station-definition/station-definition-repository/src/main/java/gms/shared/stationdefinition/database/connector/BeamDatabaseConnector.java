...
public class BeamDatabaseConnector extends DatabaseConnector {
  ...
  @Autowired
  public BeamDatabaseConnector(@Qualifier("entityManagerFactory")
    EntityManagerFactory entityManagerFactory) {
    super(entityManagerFactory);
  }

  public Optional<BeamDao> findBeamForWfid(long wfid) {
    EntitySingleResultFunction<BeamDao> delegateFunction = entityManager -> 
      entityManager.find(BeamDao.class, wfid);

    return runWithEntityManagerSingleResultFunction(delegateFunction,
      BEAM_WFID_ERROR, String.format(WFID_MESSAGE, wfid));
  }

  /**
   * Uses the provided primary key representation for a {@link BeamDao} and uses
   * it to search the database for a match.
   *
   * @param wfids List of wfdisc ids to search for (also the pk in the Beam
   * table)
   *
   * @return a {@link List<BeamDao>} of all matching records. The list is empty
   * if no match is found.
   */
  public List<BeamDao> findBeamsByWfid(List<Long> wfids) {

    return runPartitionedQuery(wfids, 500, channelIdSublist -> {
      EntityResultListFunction<BeamDao> delegateFunc = entityManager -> {
      ...
      return runWithEntityManagerResultListFunction(delegateFunc,
        BEAM_WFIDS_ERROR, String.format(WFIDS_MESSAGE, wfids.size()));
    });
  }
}
