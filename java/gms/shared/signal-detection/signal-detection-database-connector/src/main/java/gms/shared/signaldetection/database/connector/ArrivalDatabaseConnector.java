...
public class ArrivalDatabaseConnector extends DatabaseConnector {
  ...
  static final String STATION_NAMES_EXCLUDED_ARIDS_ERROR = "Arrivals by stations names and excluded arids exception";
  ...
  public Optional<ArrivalDao> findArrivalByArid(long arid) {
    ...
    EntitySingleResultFunction<ArrivalDao> delegateFunction = entityManager -> {
    ...
    return runWithEntityManagerSingleResultFunction(delegateFunction,
      ARRIVAL_ARID_ERROR, errorMessage);
  }
   ...
   *
   * @return list of {@link ArrivalDao}s {@link Optional} if no such
   * {@link ArrivalDao} exists
   */
  public List<ArrivalDao> findArrivalsByArids(Collection<Long> arids) {
    ...
    if (arids.isEmpty()) {
      ...
      return runPartitionedQuery(arids, 250, partitionedArids -> {
        EntityResultListFunction<ArrivalDao> delegateFunction = entityManager -> {
        ...
        return runWithEntityManagerResultListFunction(delegateFunction,
          ARRIVALS_ARIDS_ERROR, errorMessage);
      });
    }
  }

  /**
   * Find all {@link ArrivalDao}s for the given station names, excluded arids
   * and time range
   *
  ...
  public List<ArrivalDao> findArrivals(Collection<String> stationNames,
    ...
    var errorMessage = String.format(START_END_LAG_MESSAGE, startTime, endTime,
      leadDelta, lagDelta);
    ...
    if (stationNames.isEmpty()) {
      ...
      return runPartitionedQuery(stationNames, 250, partitionedStations -> {
        EntityResultListFunction<ArrivalDao> delegateFunction = entityManager -> {
          var cb = entityManager.getCriteriaBuilder();
        ...
        return runWithEntityManagerResultListFunction(delegateFunction,
          STATION_NAMES_EXCLUDED_ARIDS_ERROR, errorMessage);
      });
    }
  }
  ...
  public List<ArrivalDao> findArrivalsByTimeRange(Instant startTime, Instant endTime) {
    ...
    return runWithEntityManagerResultListFunction(delegateFunction, 
      ARRIVALS_TIME_RANGE_ERROR, errorMessage);
  }
}
