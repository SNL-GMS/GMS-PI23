...
class SignalDetectionManagerTest extends SpringTestBase {
  ...
  void testFindFilterDefinitionsForSignalDetections() throws Exception {
    var signalDetections = ImmutableList.of(SignalDetection.createEntityReference(UUID.randomUUID()));
    ...
    var response = FilterDefinitionByUsageBySignalDetectionHypothesis.from(Collections.emptyList());

    Mockito.when(signalDetectionAccessorImpl.findFilterDefinitionsForSignalDetections(signalDetections, stageId))
      .thenReturn(Mono.just(response));

    postResult("/signal-detection/filter-definitions-by-usage/query/signal-detections/", request, HttpStatus.OK, altMockMvc);

    Mockito.verify(signalDetectionAccessorImpl, times(1))
      .findFilterDefinitionsForSignalDetections(request.getSignalDetections(), request.getStageId());
  }
}
