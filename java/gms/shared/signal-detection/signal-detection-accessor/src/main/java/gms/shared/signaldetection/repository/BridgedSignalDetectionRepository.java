...
public class BridgedSignalDetectionRepository implements SignalDetectionRepositoryInterface {
  ...
  /**
   * Retrieve related filter records, mapped by their usage, and associate them with the provided hypotheses
   * @param hypotheses A collection of {@link SignalDetectionHypothesis} to retrieve associated filter records for
   * @return Filter records mapped by usage associated with the provided hypotheses
   */
  public List<FilterRecordIdsByUsage> findFilterRecordsForSignalDetectionHypotheses(Collection<SignalDetectionHypothesis> hypotheses) {
    checkNotNull(hypotheses);

    var sdhByLegacyId = hypotheses.stream()
      .map(SignalDetectionHypothesis::toEntityReference)
      .map(sdh -> getLegacyId(sdh)
        .map(legacyId -> Map.entry(legacyId, sdh)))
      .flatMap(Optional::stream)
      .collect(toMap(Map.Entry::getKey, Map.Entry::getValue));

    var aridsByAccount = sdhByLegacyId.keySet().stream()
    ...
    var filterIdsByUsageByLegacyId = aridsByAccount.entrySet().stream()
      .map(aridsByAcctEntry -> getFilterIdsByUsageByLegacyId(aridsByAcctEntry.getKey(), aridsByAcctEntry.getValue()))
      .flatMap(Optional::stream)
      .flatMap(filterIdsByUsageByLegacyIdSubmap -> filterIdsByUsageByLegacyIdSubmap.entrySet().stream())
      .collect(toMap(Map.Entry::getKey, Map.Entry::getValue));

    return sdhByLegacyId.entrySet().stream()
      .map(legIdBySdhEntry -> Optional.ofNullable(filterIdsByUsageByLegacyId.get(legIdBySdhEntry.getKey()))
        .map(filterIdsByUsage -> FilterRecordIdsByUsage.create(filterIdsByUsage, legIdBySdhEntry.getValue())))
      .flatMap(Optional::stream)
      .filter(filterRecordIdsByUsage -> {
        if (filterRecordIdsByUsage.getIdsByUsage().isEmpty()) {
          logger.debug("Usage map for hypothesis {} has no entries. Excluding hypothesis from results.", 
  ...
  private Optional<SignalDetectionHypothesisArrivalIdComponents> getLegacyId(SignalDetectionHypothesis hypothesis) {
    var legacyId = Optional.ofNullable(
      signalDetectionIdUtility.getArrivalIdComponentsFromSignalDetectionHypothesisId(hypothesis.getId().getSignalDetectionId()));

    legacyId.ifPresentOrElse(id -> {}, () -> logger.debug("Legacy ID not found for hypothesis {}", hypothesis.getId()));
    return legacyId;
  }

  private Optional<Map<SignalDetectionHypothesisArrivalIdComponents, Map<FilterDefinitionUsage, Long>>> getFilterIdsByUsageByLegacyId(String legacyAccountName, List<Long> arids) {
    var arrDPIConnector = signalDetectionBridgeDatabaseConnectors.getConnectorForCurrentStage(legacyAccountName, ARRIVAL_DYN_PARS_INT_CONNECTOR_TYPE);

    if (arrDPIConnector.isEmpty()) {
      logger.debug("Legacy Account {} does not map to a known stage. Unable to acquire ADPI Connector.", legacyAccountName);
    }

    return arrDPIConnector
      .map(connector -> connector.findFilterAdpisByIds(arids).stream()
        .collect(groupingBy(adpi -> adpi.getArrivalDynParsIntKey().getArid())))
      .map(adpisByArid -> adpisByArid.entrySet().stream()
        .collect(toMap(Map.Entry::getKey, entry -> getFilterIdsByUsage(entry.getValue()))))
      .map(filterIdsByUsageByArid -> remapAridToLegacyId(legacyAccountName, filterIdsByUsageByArid));
  }

  private Map<FilterDefinitionUsage, Long> getFilterIdsByUsage(List<ArrivalDynParsIntDao> adpis) {
    return adpis.stream()
      .collect(toMap(
        adpi -> FilterDefinitionUsage.fromString(adpi.getArrivalDynParsIntKey().getGroupName()), 
        ArrivalDynParsIntDao::getIvalue));
  }

  private static Map<SignalDetectionHypothesisArrivalIdComponents, Map<FilterDefinitionUsage, Long>> remapAridToLegacyId(String legacyAccountName, Map<Long, Map<FilterDefinitionUsage, Long>> filterIdsByUsageByArid) {
    return filterIdsByUsageByArid.entrySet().stream()
      .map(fidByUsageByAridEntry -> Map.entry(
        SignalDetectionHypothesisArrivalIdComponents.create(legacyAccountName, fidByUsageByAridEntry.getKey()),
