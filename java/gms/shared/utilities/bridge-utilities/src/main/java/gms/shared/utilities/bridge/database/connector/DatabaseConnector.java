...
public abstract class DatabaseConnector {
  ...
  /**
   * Entity manager run method which takes in {@link EntitySingleResultFunction}
   * and returns a given object of type <T>
   *
      ...
      .get(() -> {
        ...
        try {
          ...
          if (e instanceof NoResultException || e instanceof NonUniqueResultException
            || e instanceof QueryTimeoutException || e instanceof IllegalStateException) {
            // handle entity manager error and return empty
            handleEntityManagerErrors(e, queryErrorMessage, errorMessageArgs, EMPTY_RESULT_MESSAGE);
  ...
  /**
   * Entity manager run method which takes in {@link EntityResultListFunction}
   * and returns a list of given objects of type <T>
   *
   * @param <T> - returned object type
      ...
      .get(() -> {
        ...
        try {
          ...
          if (e instanceof QueryTimeoutException || e instanceof IllegalStateException) {
            ...
            handleEntityManagerErrors(e, queryErrorMessage, errorMessageArgs, EMPTY_LIST_MESSAGE);
            return List.of();
          } else {
            // wrap recoverable exception and send to retry policy
            throw new DatabaseConnectorException(queryErrorMessage, e);
  ...
  /**
   * Entity manager run method which takes in (@link EntityVoidFunction} and
   * commits the transaction without returning
   *
      ...
      .run(() -> {
        ...
        try {
          ...
          if (e instanceof NullPointerException || e instanceof IllegalStateException 
            ...
            logger.warn("Unrecoverable exception for {} Rolling back transaction...", errorMessage);
  ...
  /**
   * Breaks the query into partitionSize batches and makes individual Oracle
   * queries for each partition
   *
   ...
   */
  private RetryPolicy<Object> createVoidRetryPolicy() {
    ...
    return new RetryPolicy<>()
      ...
      .onFailedAttempt(e -> logger.warn("Entity manager query failed with "
          + "error - {}. " + RETRYING_MESSAGE, e.getLastFailure().getMessage())
      )
      .onRetry(e -> logger.warn("Entity manager query failed attempt #{}. "
        + RETRYING_MESSAGE, e.getAttemptCount()))
      .onRetriesExceeded(e -> logger.error("Entity manager query max retries exceeded",
        e.getFailure()));
  }
   ...
   */
  private RetryPolicy<Object> createRetryPolicy(String queryErrorMessage,
    ...
    return new RetryPolicy<>()
      ...
      .withMaxAttempts(MAX_RETRY_ATTEMPTS)
      .handle(DatabaseConnectorException.class)
      .onFailedAttempt(e -> handleEntityManagerErrors(e.getLastFailure(), 
          queryErrorMessage, errorMessageArgs, RETRYING_MESSAGE)
      )
      .onRetry(e -> logger.warn("Entity manager query failed attempt #{}. "
        + RETRYING_MESSAGE, e.getAttemptCount()))
      .onRetriesExceeded(e -> logger.error("Entity manager query max retries exceeded.",
   ...
   */
  private void handleEntityManagerErrors(Throwable e, 
    String queryErrorMessage, String errorMessageArgs, String endMessage) {
    // get the error message for the given exception type
    var entityErrorMessage = EntityErrorMessage.get(
      e.getClass().getSimpleName());
    var mainErrorMessage = e.getMessage();
    String errorTypeMessage = entityErrorMessage.getMessage(queryErrorMessage,
      errorMessageArgs, mainErrorMessage);
    var errorMessage = "Entity manager query failed with "
      + "error - {}. " + endMessage;
