...
public class BicubicSplineFeaturePredictor implements FeaturePredictorPlugin {
  ...
  public <T extends FeaturePredictionValue<?, ?, ?>> Optional<FeaturePrediction<T>> predict(
    ...
    try {
      ...
      // here.
      //
      travelTimeAndDerivatives  = utility.interpolateEarthModel(
      ...
      logger.info(
        "Interpolator threw an InsufficientDataException or ArrayIndexOutOfBoundsException for phase {}, prediction type {}, source location {}, reciever location {}"
          + "\nThis usually indicates that table data could not be extrapolated."
        ,
        phaseType, predictionType, sourceLocation, receiverLocation
      );
    ...
    if (predictionType == FeaturePredictionType.ARRIVAL_TIME_PREDICTION_TYPE) {
      ...
      if (predictedValue.getClass() == predictionType.getTypeValueClass()) {
        return Optional.of(FeaturePrediction.<T>builder()
