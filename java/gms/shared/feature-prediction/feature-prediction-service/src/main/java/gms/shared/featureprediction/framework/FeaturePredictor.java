...
public class FeaturePredictor {
  ...
  /**
   * Calculate some predictions. This version of the nethod takes pure location
   * data.
   *
   ...
   * @param featurePredictionCorrectionDefinitions List of correction
   * definitions specifying which corrections to perform and how to perform
   * them.
   *
   * @return A set of feature prediction, wrapped in a
  ...
  public FeaturePredictionContainer predict(
    ...
    if (featurePredictionList.isEmpty()) {
      throw new ZeroFeaturePredictionsException();
    }
  ...
  /**
   * Calculates some feature predictions, that places them inside the provided
   * LocationSolution (that is, makes a copy of the LocationSolution with the
   * new feature predictions added.)
   * <p>
   * If the provided LocationSolution already contains a FeaturePrediction where
   * the prediction type matches one of the provided prediction types AND the
   * channel matches one of the provided channels, no prediction is calculated
   * for that type/channel combination. In other words,this will only calculate
   * predictions for prediction type/channel pairs that don't exist in the
   * location solution.
   *
   * @param predictionTypes What type of predictions to calculate
   * @param sourceLocationSolution The LocationSolution to update with new
   * predictions. Contains the event location
   * @param receivingChannels The channels whose location to calculate the
   * prediction for
   * @param phaseTypes Which phases to predict for
   * @param earthModel Which model to use
   * @param featurePredictionCorrectionDefinitions List of correction
   ...
   * them.
   *
   * @return a copy of the sourceLocationSolution, updated with the new feature
   * predictions.
   */
  public LocationSolution predict(List<FeaturePredictionType<?>> predictionTypes,
    ...
    var mapTypeEntryList = predictionTypes.stream()
          //
          ...
          //
          .filter(entry
            -> !oldFeaturePredictionContainer.anyMatch(featurePrediction
            -> entry.getValue().equals(featurePrediction.getChannel().orElse(null)) && entry.getKey() == featurePrediction.getPredictionType()))
      .collect(Collectors.toList());
    
    if (mapTypeEntryList.isEmpty()) {
      featurePredictionList = List.of();
    } else {
      featurePredictionList = mapTypeEntryList.stream()
        .map(entry
          -> phaseTypes.stream().map(phaseType -> {
          //
          ...
          return registry.getPlugin(
            ...
            earthModel,
            featurePredictionCorrectionDefinitions
          ).map(channel -> channel.toBuilder().setChannel(Optional.of(entry.getValue())).build());
        }))
