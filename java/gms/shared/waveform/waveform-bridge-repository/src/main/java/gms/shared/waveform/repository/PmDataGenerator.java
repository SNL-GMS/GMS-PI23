...
public class PmDataGenerator {
   ...
   *
   * @param data {@link PMData} containing the data necessary to generate the
   * channel name.
   *
  ...
  public List<QcSegment> convertToQcSegment(List<QcData> qcDataList, Instant originTime) {
    ...
    return qcDataList.stream().map(data -> {
      if ((data.getMaskType() == 600) && "ASAR.AS01".equals(data.getSta()) && "SHZ".equals(data.getChan())) {
        return qcDataGenerator.createCannedQcSegmentWithVersions(data, originTime, 9);
   ...
   *
   * @param pmDataList List of relevant PMData items to convert to
   * ProcessingMasks
   ...
   */
  public List<ProcessingMask> createProcessingMasks(List<PMData> pmDataList, List<QcData> qcDataList, Instant originTime) {
    ...
    var processingMaskList = pmGrouping.entrySet().stream().map(mapper -> {
      logger.debug("Looking at Group: {} Count:({}) {}", mapper.getKey(), mapper.getValue().size(), mapper.getValue());
      return new PmDataGenerator().convertGroupToProcessingMask(mapper.getValue(), qcDataList, originTime);
    })
      .collect(Collectors.toList());

    if (pmGrouping.size() != processingMaskList.size()) {
      logger.debug("Invalid number of processing masks created.  Expected:[{}] Actual:[{}]", pmGrouping.size(), processingMaskList.size());
    }
   ...
   */
  public Map<Long, List<PMData>> convertPmDataToGroup(List<PMData> pmDataList) {

    return pmDataList.stream().collect(Collectors.groupingBy(PMData::getPmGroup, Collectors.toList()));
   ...
   *
   * @param pmDataList List of relevant PMData items of the same grouping to
   * convert to ProcessingMask
   * @param qcDataList List of QcData items associated with PMData items
   * @param originTime Time to map ProcessingMask and QcSegments
   *
   * @return
   */
  public ProcessingMask convertGroupToProcessingMask(List<PMData> pmDataList, List<QcData> qcDataList, Instant originTime) {
    ...
    Preconditions.checkArgument(!qcDataList.isEmpty(), QCLIST_EMPTY_MSG);
    Preconditions.checkNotNull(originTime, TIME_CHECK_MSG);
    var matchingConditions = pmDataList.stream().map(match
      -> List.of(match.getChan(), match.getSta(), match.getPmGroup(), match.getProcessingOperation()))
      .distinct()
      .collect(Collectors.toList());//could try all match
    Preconditions.checkArgument(matchingConditions.size() == 1);

    var localQcSegments = pmDataList.stream().map(pmField -> {
      var qcDataRef = qcDataList
        .stream().filter(predicate
          -> (predicate.getChan().equals(pmField.getChan())
        && predicate.getSta().equals(pmField.getSta())
        && predicate.getStartSample() == pmField.getStartSample()
        && predicate.getQcmaskid() == pmField.getQcmaskid()
        && predicate.getMaskType() == pmField.getQcMaskType())
        ).collect(Collectors.toList());
      if (qcDataRef.size() != 1) {
        logger.debug("Expected Qc data to match pm data {} {} {} {} {}",
          pmField.getChan(), pmField.getSta(), pmField.getStartSample(),
          pmField.getQcmaskid(), pmField.getQcMaskType()
        );
        throw new IllegalStateException("One QcSegment expected, found " + qcDataRef.size());
      }
    ...
    var localQcVersionsList = localQcSegments.stream().map(mapper
      -> mapper.getData().get().getVersionHistory().last()).collect(Collectors.toList());
    //create qc segments
    var pmProperties = pmDataList.get(0);

    var startTime = localQcVersionsList.stream()
      .map(QcSegmentVersion::getData)
      .filter(Optional::isPresent)
      .map(Optional::get)
      .map(QcSegmentVersion.Data::getStartTime)
      .min(Instant::compareTo).orElseThrow(() -> new IllegalArgumentException(QC_DATA_MISSING_MSG));

    var endTime = localQcVersionsList.stream()
      .map(QcSegmentVersion::getData)
      .filter(Optional::isPresent)
      .map(Optional::get)
      .map(QcSegmentVersion.Data::getEndTime)
      .max(Instant::compareTo).orElseThrow(() -> new IllegalArgumentException(QC_DATA_MISSING_MSG));

    var pmData = ProcessingMask.Data.instanceBuilder()
      .setEffectiveAt(endTime)
      //set effective at to match start time (or should it be end time - match Qc methodology??
    ...
    var idArray
      ...
      + pmProperties.getProcessingOperation() + createChannelName(pmProperties);
