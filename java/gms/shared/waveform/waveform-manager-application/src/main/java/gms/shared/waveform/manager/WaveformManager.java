...
public class WaveformManager {
  ...
  /**
   * Returns a collection of {@link ChannelSegment}s for each Channel entity
   * provided in the query parameters (since Channel is faceted, the provided
   * objects may be fully populated or contain only references).
   * <p>
   * The response has a collection of ChannelSegments for each Channel entity
   * since a ChannelSegment is associated to a single Channel object but there
   * may be multiple versions of each Channel entity within the queried time
   * interval.
   * <p>
   * Each ChannelSegment may contain multiple Waveforms to account for gaps in
   * available waveform samples or changes in sample rate, but each Waveform is
   * as long as possible. This operation always returns calibrated waveform
   * samples.
   *
   * @param channelTimeRangeRequest List of channels to and time ranges to query
   * over.
   *
   * @return list of all {@link ChannelSegment} objects for each Channel entity
  ...
  /**
   * Returns a collection of {@link ChannelSegment}s as it existed at the
   * creation time listed in ChannelSegmentDescriptor, even if newer data
   * samples have since been stored in this WaveformRepository. (since Channel
   * is faceted, the provided objects may be fully populated or contain only
   * references).
   * <p>
   * All of the samples returned for a ChannelSegmentDescriptor must be for the
   * exact Channel version provided in that ChannelSegmentDescriptor. Each
   * returned ChannelSegment may contain multiple Waveforms to account for gaps
   * in available waveform samples or changes in sample rate, but each Waveform
   * is as long as possible.
   *
   * @param channelSegmentDescriptorRequest ChannelName, time ranges, and
   * creation time to query over.
   *
   * @return list of all {@link ChannelSegment} objects for each Channel entity
   * within the queried time interval
   */
   ...
   *
   * @param channelTimeRangeRequest A List of Channels, start time, and end time
   * to query for QcSegments
   *
   * @return A Collection of {@link QcSegment}s associated with the channels
   * during the requested interval.
   */
  @PostMapping(value = "/qc-segment/query/channel-timerange/canned")
  @Operation(summary = "Loads and returns QcSegmentVersion based on channel name and segment start, end, and creation times")
  public Collection<QcSegment> findQcSegmentsByChannelsandTimeRangeCanned(
    @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "List of channels and time range used to query QC Segments")
    @RequestBody ChannelTimeRangeRequest channelTimeRangeRequest) {
    /*
     * Note canned endpoint does not contain the faceting definition
     */
