...
/**
 * Accessor for retrieving Waveforms from the backing store. This contains a
 * cache of previously retrieved channels, and a class to a retrieve any
 * channels not in the cache from the backing store.
 */
@Service
public class WaveformAccessor implements WaveformAccessorInterface {
  ...
  @Override
  public Collection<QcSegment> findQcSegmentsByChannelsandTimeRangeCanned(Collection<Channel> channels, Instant startTime, Instant endTime) {
    try {

      logger.info("Retrieving findQcSegmentsByChannelsandTimeRangeCanned");

      var jsonFile = Thread.currentThread().getContextClassLoader().getResourceAsStream("/QcDataSet.json");

      var jsonData = ObjectMapperFactory.getJsonObjectMapper().readValue(jsonFile, QcDataList.class);

      return jsonData.getQcList().stream().map(data -> {
        if ((data.getMaskType() == 600) && "ASAR.AS01".equals(data.getSta()) && "SHZ".equals(data.getChan())) {
          return qcDataGenerator.createCannedQcSegmentWithVersions(data, startTime, 9);

        } else {
          return qcDataGenerator.createCannedQcSegmentWithVersions(data, startTime, 0);
        }
      }).collect(Collectors.toList());
    }
