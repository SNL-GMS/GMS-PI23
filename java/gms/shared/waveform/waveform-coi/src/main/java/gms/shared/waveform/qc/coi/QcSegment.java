...
public abstract class QcSegment {
  ...
  /**
   * Return set of vQcSegmentVersions, where the latest is fully populated and
   * the rest are entity references.
   *
   * @return QcSegment with same identity but with the "proper" history of
   * QcSegmentVersions as entity references.
   */
  public QcSegment withPreviousHistoryAsEntityReference() {
    ...
    return getData().map(data -> {
      ...
      var facetedSet = set.stream()
        .map(QcSegmentVersion::toEntityReference)
        .collect(Collectors.toSet());

      facetedSet.add(lastVersion);

      return toBuilder().setData(
        data.toBuilder()
          .setVersionHistory(ImmutableSortedSet.copyOf(facetedSet))
          .build()
      ).build();
    }).orElseThrow(
      () -> new IllegalStateException("Calling withPreviousHistoryAsEntityReference on an entity reference"));
  ...
  public abstract static class Data {
    ...
    public abstract static class Builder {
      ...
      public Data build() {
        ...
        var versionChannels = tentativeData.getVersionHistory().stream()
        ...
        Preconditions.checkState(
          versionChannels.stream().allMatch(channelName -> channelName.equals(tentativeData.getChannel().getName())),
          "Channel %s does not match all of the channel names: %s",
          tentativeData.getChannel(), versionChannels);
