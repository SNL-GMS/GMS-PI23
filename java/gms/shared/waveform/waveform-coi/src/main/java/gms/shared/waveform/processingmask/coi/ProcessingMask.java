...
public abstract class ProcessingMask {
  ...
  public abstract static class Data {
    ...
    public static interface BuilderInterface {
      ...
      BuilderInterface setAppliedToRawChannel(Channel applyToRawChannel);

      BuilderInterface setEffectiveAt(Instant effectiveAt);

      BuilderInterface setStartTime(Instant startTime);

      BuilderInterface setEndTime(Instant endTime);

      BuilderInterface setProcessingOperation(ProcessingOperation processingOperation);

      BuilderInterface setMaskedQcSegmentVersions(Collection<QcSegmentVersion> maskedVersions);
    ...
    public abstract static class Builder implements BuilderInterface {
      ...
      public Data build() {
        ...
        checkState(tentativeData.getStartTime().equals(tentativeData.getEndTime())
          || tentativeData.getStartTime().isBefore(tentativeData.getEndTime()),
          "Start time [" + tentativeData.getStartTime() + "] must equal or preceed end time [" + tentativeData.getEndTime() + "]");
        checkState(!tentativeData.getMaskedQcSegmentVersions().isEmpty(), "MaskedQcSegmentVersions must not be empty");

        var versionChannels = tentativeData.getMaskedQcSegmentVersions().stream()
        ...
        checkState(
          versionChannels.stream()
            .allMatch(channelName -> channelName.equals(tentativeData.getAppliedToRawChannel().getName())),
          "Channel %s does not match all of the channel names: %s",
          tentativeData.getAppliedToRawChannel(), versionChannels);
    ...
    private static class JacksonBuilder implements BuilderInterface {
      ...
      private final AutoValue_ProcessingMask_Data.Builder delgateBuilder = new AutoValue_ProcessingMask_Data.Builder();
      private int invokedSetterCount;

      @Override
      public BuilderInterface setAppliedToRawChannel(Channel applyToRawChannel) {
        invokedSetterCount++;
      ...
      @Override
      public BuilderInterface setEffectiveAt(Instant effectiveAt) {
        invokedSetterCount++;
      ...
      @Override
      public BuilderInterface setProcessingOperation(ProcessingOperation processingOperation) {
        invokedSetterCount++;
      ...
      @Override
      public BuilderInterface setMaskedQcSegmentVersions(Collection<QcSegmentVersion> maskedVersions) {
        invokedSetterCount++;
