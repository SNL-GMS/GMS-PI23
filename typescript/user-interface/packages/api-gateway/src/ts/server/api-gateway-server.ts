...
import {
  publishAcknowledgedChange,
  publishDerivedChannel,
  publishQuietedChange
} from '../kafka/kafka-publishing';
...
export const configureAdditionalRoutesDerivedChannel = (): void => {
  const ackHandler = async (req, res) => {
    const derivedChannel: ChannelTypes.Channel = req.body;

    await publishDerivedChannel(derivedChannel);
    const response = {
      status: 'complete'
    };
    res.send(JSON.stringify(response));
  ...
  const clientLogRoute = '/interactive-analysis-api-gateway/publish-derived-channel';
  logger.info(`register ${clientLogRoute}`);
  app.post(clientLogRoute, ackHandler);
};
