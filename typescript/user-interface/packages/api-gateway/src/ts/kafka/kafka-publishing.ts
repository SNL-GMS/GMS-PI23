 ...
 *
 * @param derivedChannel the Channel to be sent
 */
export const publishDerivedChannel = async (
  derivedChannel: ChannelTypes.Channel
): Promise<void> => {
  logger.info(`Publishing derived channel: ${derivedChannel.name}`);
  await KafkaProducer.Instance().send(kafkaSettings.producerTopics.derivedChannel, [
    { value: JSON.stringify(derivedChannel) }
  ]);
};
