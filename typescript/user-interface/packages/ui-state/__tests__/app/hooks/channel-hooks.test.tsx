...
describe('channel hooks', () => {
    ...
    it('useGetChannelsQuery requests derived channels from uiChannelSegments', () => {
      ...
      const channelNames = Object.values(uiChannelSegmentRecord).flatMap(filter =>
        Object.values(filter).flatMap(channelSegments =>
          channelSegments.map(({ channelSegment: { channelName } }) => channelName)
        )
      );
  ...
  describe('useDerivedChannels', () => {
    it('useDerivedChannels returns an array of only raw channels', () => {
      const mockStoreCreator: MockStoreCreator<AppState, AnyAction> = createMockStore([thunk]);
      const mockAppState: AppState = {
        ...appState,
        data: {
      ...
      const { result } = renderHook(() => useDerivedChannels(), {
        wrapper: Wrapper
      });
  ...
  describe('useChannelsRecord', () => {
    it('useChannels returns an array of channels', () => {
      const mockStoreCreator: MockStoreCreator<AppState, AnyAction> = createMockStore([thunk]);
      const mockAppState: AppState = {
        ...appState,
        data: {
      ...
      const { result } = renderHook(() => useChannelsRecord(), {
        wrapper: Wrapper
      });
