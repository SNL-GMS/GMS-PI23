import type { ChannelTypes, CommonTypes, SohTypes } from '@gms/common-model';
import {
...
import {
  ...
  usePublishDerivedChannelMutation,
 ...
 */
export const useGetPublishDerivedChannelMutation = (): MutationTrigger<
  MutationDefinition<
  ...
  >
> => {
  const [publishDerivedChannelMutation] = usePublishDerivedChannelMutation();
  return publishDerivedChannelMutation;
};
...
export const usePublishDerivedChannel = (): ((
  channelToPublish: ChannelTypes.Channel
) => Promise<void>) => {
  const callMutation = useGetPublishDerivedChannelMutation();
  return async channelToPublish => {
    logger.info(`Publishing derived channel: ${channelToPublish.name}`);

    await callMutation(channelToPublish);
  };
};
