...
} from '../api/data/event/find-events-by-assoc-sd-hypotheses';
import {
...
export const useQueryArgsForFindEventsByAssociatedSignalDetectionHypotheses = (
  interval: CommonTypes.TimeRange
): FindEventsByAssociatedSignalDetectionHypothesesArgs => {
  const openIntervalName = useAppSelector(state => state.app.workflow.openIntervalName);
  ...
  const signalDetectionHypotheses = React.useMemo(() => {
    ...
    return Array.from(hypothesesSet);
...
export const useFindEventsByAssociatedSignalDetectionHypothesesHistory = (): EventsByAssociatedSignalDetectionHypothesesHistoryFetchResult => {
  const history = useAppSelector(
...
const useFindEventsByAssociatedSignalDetectionHypothesesSkippedResult = (): EventsFetchResult => {
  const associatedResult = React.useRef({
    data: [],
    pending: 0,
    fulfilled: 0,
    rejected: 0,
  ...
  return associatedResult.current;
};
...
export const useFindEventsByAssociatedSignalDetectionHypotheses = (
  args: FindEventsByAssociatedSignalDetectionHypothesesArgs
): EventsFetchResult => {
  const history = useFindEventsByAssociatedSignalDetectionHypothesesHistory();

  useFetchEventsByAssociatedSignalDetectionHypotheses(args);

  const associatedEvents = useAppSelector(selectAssociatedEvents);

  const skippedReturnValue = useFindEventsByAssociatedSignalDetectionHypothesesSkippedResult();

  const shouldSkip = shouldSkipFindEventsByAssociatedSignalDetectionHypotheses(args);

  const emptyArray = React.useRef<EventTypes.Event[]>([]);

  const data = React.useMemo(
    () => (shouldSkip ? emptyArray.current : Object.values(associatedEvents).map(e => e)),
    [associatedEvents, shouldSkip]
  );

  return React.useMemo(() => {
    if (shouldSkipFindEventsByAssociatedSignalDetectionHypotheses(args)) {
      return skippedReturnValue;
    }
    return { ...history, data };
  }, [args, history, data, skippedReturnValue]);
};

/**
 * Get the events with associated signal detection hypotheses
 * Collects results inside and outside viewable interval, used to supplement SD type determination/coloring in displays
 * !! Do not use to display events in UI
 */
export const useGetAssociatedEvents = (): EventsFetchResult => {
  const [viewableInterval] = useViewableInterval();
  const args = useQueryArgsForFindEventsByAssociatedSignalDetectionHypotheses(viewableInterval);
  return useFindEventsByAssociatedSignalDetectionHypotheses(args);
};
...
export const useEventStatusQuery = (): FindEventStatusInfoByStageIdAndEventIdsQuery => {
  ...
  const eventData = [...eventResults.data, ...associatedEventResults.data];
  const data: FindEventStatusInfoByStageIdAndEventIdsProps = {
    stageId: { name: stageName },
    eventIds: eventData?.map(event => event.id)
  };
  const skip =
    ...
    associatedEventResults.pending > 0;
