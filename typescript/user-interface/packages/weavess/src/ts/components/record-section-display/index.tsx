...
export const workerRpc = (() => {
  ...
  const rpc = new RpcProvider((message, transfer) => worker.postMessage(message, transfer));
...
export class RecordSection extends React.Component<unknown, RecordSectionState> {
  ...
  public displayName = 'RecordSection';
  ...
  public kilometersToDegrees = (km: number): number =>
    ...
    (km / (Math.PI * 6371e3)) * this.ONE_HUNDRED_EIGHTY_DEGREES;
  ...
  public async loadData(waveformArray: any[]): Promise<void> {
    ...
    for (const waveform of waveformArray) {
      ...
      signalDetectGeometry.vertices.push(
    ...
    phases.forEach(val => {
      ...
      val.percentY = (1 - (val.detectY - minY) / (maxY - minY)) * 100;
    });
