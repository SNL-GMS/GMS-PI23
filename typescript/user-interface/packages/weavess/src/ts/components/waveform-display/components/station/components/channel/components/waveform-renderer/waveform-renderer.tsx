...
> {
  ...
  private renderedMaskPointRefs: THREE.Mesh[] = [];
  ...
  private readonly tempRenderedMaskPointRefs: THREE.Mesh[] = [];
  ...
  private readonly MASK_SPRITE = new THREE.TextureLoader().load('./resources/images/disc.png');
  ...
  public updateAmplitude = debounce(
    async (timeRange: WeavessTypes.TimeRange): Promise<void> => {
      ...
      if (this.props.masks && this.renderedMaskRefs.length === 0) {
        await this.renderChannelMasks(this.props.masks);
      }
  ...
  private readonly updateBoundaryMarkers = () => {
    ...
    // Left marker
    const leftStartSeconds = this.props.displayInterval.startTimeSecs;
    ...
    if (leftWidth > 0) {
      const leftMarkerGeometry = new THREE.PlaneBufferGeometry(leftWidth, height);
    ...
    const rightEndSeconds = this.props.displayInterval.endTimeSecs;

    const rightWidth = timeToGlScale(rightEndSeconds) - timeToGlScale(rightStartSeconds);

    // Do not create and add to scene if there is no width
    if (rightWidth > 0) {
      const rightMarkerGeometry = new THREE.PlaneBufferGeometry(rightWidth, height);
      this.rightBoundaryMarker = new THREE.Mesh(rightMarkerGeometry, material);

      const rightMidpoint = timeToGlScale(
        rightEndSeconds + (rightEndSeconds - rightStartSeconds) / 2
      );
  ...
  private readonly setupThreeJSFromVertices = () => {
    ...
    if (processedData) {
      ...
      this.props.setError(
        true,
        `No waveforms found for channel segment ${this.props.channelSegmentId}`
      );
    }
    this.updateAmplitudeFromBounds();
    this.updateBoundaryMarkers();
  ...
  private readonly convertDataSegmentDataFloat32 = async (
    ...
    if (float32Array == null) {
      throw new Error('No waveform was found/generated. Cannot render waveform.');
    }
   ...
   */
  private readonly renderChannelMasks = async (masks: WeavessTypes.Mask[]) => {
    ...
    await Promise.all(
      sortBy(
        ...
        await this.buildMaskPointsObject(mask, timeToGlScale);
      })
    );
  ...
  private readonly buildMaskPlaneObject = (
    ...
    timeToGlScale: d3.ScaleLinear<number, number, number>
  ): void => {
    ...
    const planeGeometry = new THREE.PlaneBufferGeometry(width, this.maskHeight * 2);
   ...
   */
  private readonly buildMaskPointsObject = async (
    mask: WeavessTypes.Mask,
    timeToGlScale: d3.ScaleLinear<number, number, number>
  ): Promise<void> => {
    ...
    if (
      ...
      pointsArray.push(await this.findAmplitudeAtTime(mask.startTimeSecs));
    }
    ...
    if (
      ...
      pointsArray.push(await this.findAmplitudeAtTime(mask.endTimeSecs));
    }
    const geo = new THREE.BufferGeometry();
    geo.addAttribute('position', new THREE.BufferAttribute(new Float32Array(pointsArray), 2));
  ...
  private readonly findDataSegmentForTime = (time: number): WeavessTypes.DataSegment => {
    const dataSegments = this.getThisChannelSegments().flatMap(cs => cs.dataSegments);
   ...
   */
  public readonly findAmplitudeAtTime = async (time: number): Promise<number> => {
    const dataSegment = this.findDataSegmentForTime(time);
    if (dataSegment === undefined) {
      logger.warn(`dataSegment not found for when building mask end point at ${time} seconds`);
      return 0;
    }
    if (WeavessTypes.isDataClaimCheck(dataSegment.data)) {
      ...
      const float32Array = await this.convertDataClaimCheckToFloat32(
        this.props.getPositionBuffer,
        dataSegment.data
      );
    ...
    if (dataSegment && WeavessTypes.isDataByTime(dataSegment.data)) {
      if (WeavessTypes.isFloat32Array(dataSegment.data.values)) {
    ...
    if (dataSegment && WeavessTypes.isDataBySampleRate(dataSegment.data)) {
