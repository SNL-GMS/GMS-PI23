...
export class PopoverButtonComponent extends React.Component<PopoverProps, PopoverState> {
  ...
  public render(): JSX.Element {
    ...
    const iconAlignText = this.props.onlyShowIcon ? Alignment.CENTER : Alignment.LEFT;
    const iconClassName = this.props.onlyShowIcon ? 'toolbar-button--icon-only' : 'toolbar-button';
    const iconLabel = this.props.onlyShowIcon ? null : this.props.label;
    const iconSymbol = this.props.icon ? this.props.icon : IconNames.CHEVRON_DOWN;
    return (
      <div
        ref={ref => {
          if (ref) {
