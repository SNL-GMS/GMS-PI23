...
export class FormBody extends React.Component<FormBodyProps, FormBodyState> {
  ...
  private readonly buildDisplayText = (item: any) => {
    return item.displayText !== undefined ? item.displayText : '';
  };

  // eslint-disable-next-line class-methods-use-this
  private readonly buildToolTip = (item: any) => {
          ...
          <React.Fragment key={item.itemKey}>
            ...
            {item.itemType === ItemType.Input && item.value !== undefined ? (
              ...
              <FormDisplayText
                ...
                className={item.className}
                displayText={this.buildDisplayText(item)}
