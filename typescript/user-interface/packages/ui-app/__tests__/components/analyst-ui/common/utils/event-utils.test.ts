...
describe('Event utils', () => {
  ...
  it('isSignalDetectionCompleteAssociated detects signal detections associated with a complete event', () => {
    ...
    expect(
      isSignalDetectionCompleteAssociated(signalDetectionsData[0], [eventData], eventStatuses)
    ).toBeTruthy();
  ...
  it('isSignalDetectionOtherAssociated detects signal detections  associated with a non open event', () => {
    // test with open event == associatedEvent
    expect(
      isSignalDetectionOtherAssociated(signalDetectionsData[0], [eventData], eventData.id)
    ).toBeFalsy();
    // test with open event != associatedEvent
    expect(
      isSignalDetectionOtherAssociated(signalDetectionsData[0], [eventData], 'otherEventId')
    ).toBeTruthy();
  });

  it('isSignalDetectionAssociated detects signal detections  associated an event', () => {
    // test with associated SD
    expect(isSignalDetectionAssociated(signalDetectionsData[0], [eventData])).toBeTruthy();
    // test with unassociated SD
    expect(isSignalDetectionAssociated(signalDetectionsData[1], [eventData])).toBeFalsy();
  });
});
