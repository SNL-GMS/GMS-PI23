...
describe('export data', () => {
  describe('exportChannelSegmentsBySelectedStations', () => {
    ...
    it('returns only the filtered UIChannelSegments', () => {
      ...
      expect(result[0].channelSegment.channelName).toMatch(/ASAR.+/);
      expect(result[1].channelSegment.channelName).toMatch(/ASAR.+/);
      expect(result[2].channelSegment.channelName).toMatch(/ASAR.+/);
    });
...
describe('getAssocStatusColor', () => {
  ...
  it('returns associated to other event', () => {
    expect(getAssocStatusColor('Other', defaultTheme)).toBe(defaultTheme.colors.otherEventSDColor);
  });
});
