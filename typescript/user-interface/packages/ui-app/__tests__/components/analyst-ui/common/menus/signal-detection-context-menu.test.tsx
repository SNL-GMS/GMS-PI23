...
import type { SignalDetectionContextMenuContentProps } from '../../../../../src/ts/components/analyst-ui/common/menus/signal-detection-context-menu';
import { SignalDetectionContextMenuContent } from '../../../../../src/ts/components/analyst-ui/common/menus/signal-detection-context-menu';
...
describe('signal-detection-context-menu', () => {
  ...
  it('should have basic props built on render', () => {
    const buildProps = wrapper.props() as SignalDetectionContextMenuContentProps;
    expect(buildProps).toMatchSnapshot();
  });

  it('should have a function for checking if we can generate fk', () => {
    const instance = wrapper.find('SignalDetectionContextMenuContent').instance();
  ...
  it('should have a function for showing showSdDetailsPopover', () => {
    const instance = wrapper.find('SignalDetectionContextMenuContent').instance();
    const spy = jest.spyOn(instance, 'getSignalDetectionDetailsProps');
    expect(() =>
      instance.getSignalDetectionDetailsProps(signalDetectionsData[0], 100, 100)
    ).not.toThrow();
    flushPromises();
  ...
  it('should have a function for setSdIdsToShowFk', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/unbound-method
    expect(props.setSdIdsToShowFk).toHaveBeenCalledWith([signalDetectionsData[0].id]);
  });
