...
import { Colors, PopoverInteractionKind, Position } from '@blueprintjs/core';
...
export class FkRendering extends React.Component<FkRenderingProps, FkRenderingState> {
  ...
  public async componentDidMount(): Promise<void> {
    ...
    const fkDisplaySpectrum = fkData.values[this.props.currentMovieSpectrumIndex];
    ...
    if (this.canvasRef) {
      fkUtil.drawImage(this.canvasRef, this.currentImage, false);
      fkUtil.draw(
        this.canvasRef,
        fkData,
        predictedPoint,
  ...
  public async componentDidUpdate(prevProps: FkRenderingProps): Promise<void> {
    const currentFkData = getFkDummyData(this.props.data);
    const predictedPoint = fkUtil.getPredictedPoint(this.props.signalDetectionFeaturePredictions);
    const newFkDisplayData = fkUtil.getFkHeatmapArrayFromFkSpectra(
      currentFkData.values[this.props.currentMovieSpectrumIndex],
      this.props.fkUnitDisplayed
    );
