...
export const ianMapTooltipHandleAltClick = (
  ...
  if (selectedEntity?.properties) {
    ...
    if (properties.type === 'Event location') {
      mapContextMenusCb.eventDetailsCb(event, properties.event);
    }

    if (properties.type === 'Signal detection') {
      mapContextMenusCb.signalDetectionDetailsCb(event, { sd: properties });
    }
    ...
    if (isSiteOrStation(properties.type)) {
      mapContextMenusCb.stationDetailsCb(event, {
        ...
        elevation: properties.coordinates.elevation,
        detailedType: properties.statype,
