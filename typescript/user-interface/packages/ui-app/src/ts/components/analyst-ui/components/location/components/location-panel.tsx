...
export class LocationPanel extends React.Component<LocationPanelProps, LocationPanelState> {
    ...
    return (
      <>
        <SignalDetectionContextMenus
          getOpenCallback={this.setSignalDetectionContextMenusOpenCallback}
        />
        ...
        >
          <DeprecatedToolbar
            ...
            toolbarWidthPx={
              this.props.widthOfDisplayPx - userPreferences.list.widthOfTableMarginsPx
            }
          />
          <HorizontalDivider
            topHeightPx={height}
            ...
            bottom={
              <LocationSignalDetections
                ...
                showSDContextMenu={(event, selectedSignalDetectionIds) => {
                  this.signalDetectionContextMenusCb.signalDetectionContextMenuCb(
                    event,
                    this.getSignalDetectionContextMenuContentProps(selectedSignalDetectionIds)
                  );
                }}
                showSDDetails={(event, signalDetectionId) => {
                  this.signalDetectionContextMenusCb.signalDetectionDetailsCb(
                    event,
   ...
   */
  private readonly setSignalDetectionContextMenusOpenCallback: SignalDetectionContextMenusGetOpenCallbackFunc = callback => {
    this.signalDetectionContextMenusCb = callback;
  };

  /**
   * Returns the props for {@link SignalDetectionContextMenuContentProps} for the provided
   * signal detection id.
   *
   * @param selectedSignalDetectionIds the selected signal detection ids
   */
  private readonly getSignalDetectionContextMenuContentProps = (
    ...
    const sds: SignalDetectionTypes.SignalDetection[] = this.props.signalDetectionsByStation.filter(
      sd => selectedSignalDetectionIds.indexOf(sd.id) >= 0
    );
    return {
  ...
  private readonly getSignalDetectionDetailsProps = (
    ...
    const signalDetection = this.props.signalDetectionsByStation.find(
