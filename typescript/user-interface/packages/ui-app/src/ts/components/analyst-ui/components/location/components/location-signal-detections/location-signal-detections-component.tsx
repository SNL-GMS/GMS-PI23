  ...
  private readonly onCellContextMenu = (params: any) => {
    ...
    const selectedSignalDetectionIds =
      selectedIdsInTable.indexOf(params.node.data.signalDetectionId) < 0
        ...
        : selectedIdsInTable;
    this.props.setSelectedSdIds(selectedSignalDetectionIds);
    this.props.showSDContextMenu(params.event, selectedSignalDetectionIds);
  };
  ...
  private readonly onRowClicked = (rowParams: any): void => {
    if (this.mainTable) {
      if (rowParams.event.altKey) {
        ...
        this.props.showSDDetails(rowParams.event, rowParams.data.signalDetectionId);
