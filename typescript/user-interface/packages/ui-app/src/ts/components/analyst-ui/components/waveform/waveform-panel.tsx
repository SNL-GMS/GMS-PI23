...
import { AnalystWaveformTypes, AnalystWorkspaceTypes, getStore } from '@gms/ui-state';
...
export class WaveformPanel extends React.PureComponent<WaveformDisplayProps, WaveformDisplayState> {
  ...
  public constructor(props: WaveformDisplayProps) {
    ...
    this.weavessConfiguration = {
      ...
      hotKeys: {
        ...
        maskCreate: systemConfig.defaultWeavessHotKeyOverrides.maskCreate,
        viewQcSegmentDetails: systemConfig.defaultWeavessHotKeyOverrides.viewQcSegmentDetails
      },
  ...
  private readonly buildCustomMeasureWindowLabel = (
    ...
    if (this.props.measurementMode.mode === AnalystWorkspaceTypes.WaveformDisplayMode.MEASUREMENT) {
      ...
      const amplitudeMeasurementValue: SignalDetectionTypes.AmplitudeMeasurementValue = SignalDetectionTypes.Util.findAmplitudeFeatureMeasurementValue(
  ...
  private readonly onLabelContextMenu = (
    ...
    const scaleAmplitudeChannelMenuItem = (
      <MenuItem
        ...
        labelElement={formatHotkeyString(
          getHotkeysForOS(
            this.props.processingAnalystConfigurationQuery.data?.keyboardShortcuts
              .scaleAllWaveformAmplitude.hotkeys
          )[0]
        )}
    ...
    const resetAmplitudeChannelMenuItem = (
      <MenuItem
        data-cy="reset-amplitude-selected-channels"
        text="Reset amplitude scaling for selected channels"
        labelElement={formatHotkeyString(
          getHotkeysForOS(
            this.props.processingAnalystConfigurationQuery.data?.keyboardShortcuts
              .resetSelectedWaveformAmplitudeScaling.hotkeys
          )[0]
        )}
        disabled={this.state.amplitudeScaleOption === AmplitudeScalingOptions.FIXED}
        onClick={() => this.resetAmplitudes(true)}
      />
    );
    ...
    const exportMenuItem = (
      <WaveformChannelExportMenuItem
        ...
        channelSegmentsRecord={this.props.channelSegments}
        disabled={this.props.waveformClientState.isLoading}
      />
    );
    ...
    const menu = isDefaultChannel ? (
      ...
      <Provider store={getStore()}>
        <Menu>
          {hideChannelMenuItem} {scaleAmplitudeChannelMenuItem} {resetAmplitudeChannelMenuItem}
          {exportMenuItem}
        </Menu>
      </Provider>
    );
   ...
   */
  private readonly resetAmplitudes = (force = false) => {
    if (this.context.weavessRef) {
      if (
        force ||
