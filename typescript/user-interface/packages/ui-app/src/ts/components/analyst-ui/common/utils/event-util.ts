...
export const getAssociatedDetections = (
  ...
  if (event) {
    return signalDetections.filter(
      sd =>
        sd &&
        isSignalDetectionOpenAssociated(sd, [event], event.id) &&
 ...
 * @param openEventId
 * @param eventsStatuses
 * @param unassociatedSDColor
 * @returns Color string
 */
export function getSignalDetectionAssociationColor(
  signalDetection: SignalDetectionTypes.SignalDetection,
  events: EventTypes.Event[],
  openEventId: string,
  eventsStatuses: Record<string, EventStatus>,
  unassociatedSDColor = semanticColors.analystUnassociated
): string {
  let color = unassociatedSDColor;
  if (!signalDetection || !events || events.length === 0 || !openEventId) {
    return color;
  }

  const associationStatus = getSignalDetectionAssociationStatus(
    signalDetection,
    events,
    openEventId,
    eventsStatuses
  ...
  if (associationStatus === EventTypes.AssociationStatus.OPEN_ASSOCIATED) {
    color = semanticColors.analystOpenEvent;
  } else if (associationStatus === EventTypes.AssociationStatus.COMPLETE_ASSOCIATED) {
    color = semanticColors.analystComplete;
  } else if (associationStatus === EventTypes.AssociationStatus.OTHER_ASSOCIATED) {
