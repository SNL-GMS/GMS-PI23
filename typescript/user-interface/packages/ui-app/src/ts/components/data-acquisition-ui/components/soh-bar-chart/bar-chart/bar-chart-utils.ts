    ...
    .map(channel => {
      ...
      if (maybeMatchingMonitorValue) {
        return {
          ...
          contributing: maybeMatchingMonitorValue.contributing,
...
export const getColorForStatus = (
  ...
  switch (status) {
    case SohTypes.SohStatusSummary.GOOD:
      return contributing ? semanticColors.dataAcqOk : semanticColors.dataAcqOkNotContributing;
    case SohTypes.SohStatusSummary.BAD:
      return contributing
        ? semanticColors.dataAcqStrongWarning
        : semanticColors.dataAcqStrongWarningNotContributing;
    case SohTypes.SohStatusSummary.MARGINAL:
      return contributing
        ? semanticColors.dataAcqWarning
        : semanticColors.dataAcqWarningNotContributing;
    case SohTypes.SohStatusSummary.NONE:
      return semanticColors.dataAcqNone;
    default:
      return '';
  }
};
...
export const buildData = (
  ...
  const barData = channelSoh
    .map(channel => ({
      ...
      id: channel.name,
      color: getColorForStatus(channel.status, channel.contributing)
    }))
    .filter(a => a !== undefined);
  const xTickLabels = channelSoh
    .map(channel => ({
      isHashedPattern: channel.isNullData,
      labelFontColor:
        channel.status === SohTypes.SohStatusSummary.MARGINAL
          ? gmsColors.gmsBackground
          : gmsColors.gmsProminent,
      labelBackgroundColor: getColorForStatus(channel.status, channel.contributing)
    }))
    .filter(a => a !== undefined);
  ...
  return { barData, barCategories, thresholdsBad, thresholdsMarginal, xTickLabels };
};
