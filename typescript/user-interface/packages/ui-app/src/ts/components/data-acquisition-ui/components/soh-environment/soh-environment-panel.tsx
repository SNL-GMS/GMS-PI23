...
export interface EnvironmentPanelProps {
  sohStatusContributors: SohTypes.SohContributor[];
...
> {
  ...
  public render(): JSX.Element {
    const currentRows = this.memoizeGetRows(
      ...
      this.props.sohStatusContributors,
  ...
  private readonly selectCells = (
    ...
    const { event } = params;
    const stateSelectedChannelMonitorPairs =
    ...
    let selectedChannelMonitorPairs =
      event.metaKey || event.ctrlKey || (isAlreadySelected && !shouldRemoveIfExisting)
        ? stateSelectedChannelMonitorPairs
        : [];
