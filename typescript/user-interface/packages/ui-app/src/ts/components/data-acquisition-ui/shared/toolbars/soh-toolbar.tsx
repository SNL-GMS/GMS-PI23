...
export const DrillDownFilterableSohTypesDisplayStrings = Immutable.Map<string, string>([
  ['GOOD', 'Good'],
  ['MARGINAL', 'Marginal'],
  ['BAD', 'Bad']
]);
...
export function SohToolbar(props: SohToolbarProps) {
  ...
  let toolTipForIssue = messageConfig.tooltipMessages.sohToolbar.lastUpdateTime;
  if (lastUpdated !== undefined) toolTipForIssue += ` at ${lastUpdatedStr}`;
  ...
  const updateTimeDisplay: DeprecatedToolbarTypes.LabelValueItem = {
    ...
    tooltipForIssue: toolTipForIssue,
    hasIssue: isStale,
    widthPx: 400,
    rank: 0,
