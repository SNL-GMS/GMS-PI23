...
import { NOT_APPLICABLE, UNKNOWN } from '../../shared/types';
...
export interface StationStatisticsPanelProps {
  ...
  columnsSelected: Record<Columns, boolean>;
  setGroupSelected: (group: string) => void;
  setColumnsSelected: (columns: Record<Columns, boolean>) => void;
}
...
> {
            ...
            >
              ...
              {needsAttentionSohData.length !== 0 ? (
                <StationStatisticsTable
                  ...
                  columnsSelected={this.convertColumnsRecordToMap(this.props.columnsSelected)}
  ...
  /**
   * Converts Record to Map due to not wanting Maps in slice, but Depreciated toolbar uses Maps
   *
   * @param state Redux state of columns
   * @returns OrderedMap of Columns to show
   */
  private readonly convertColumnsRecordToMap = (
    state: Record<Columns, boolean>
  ): Immutable.OrderedMap<Columns, boolean> =>
    (Immutable.OrderedMap(state) as unknown) as Immutable.OrderedMap<Columns, boolean>;

  /**
   * Converts map to record since toolbar wants map and api-slice uses record
   *
   * @param columnsMap map of columns shown used by toolbar
   * @returns Record<Columns, boolean>
   */
  private readonly convertColumnsMapToRecord = (
    columnsMap: Immutable.OrderedMap<Columns, boolean>
  ): Record<Columns, boolean> =>
    (Object.fromEntries(columnsMap) as unknown) as Record<Columns, boolean>;
  ...
  private readonly generateColumnDisplaySelector = (): DeprecatedToolbarTypes.ToolbarItem => ({
    ...
    values: this.convertColumnsRecordToMap(this.props.columnsSelected),
  ...
  private readonly generateChannelCellData = (
    ...
    if (monitorType === 'ENVIRONMENT' && contributorData.value === NOT_APPLICABLE) {
      return {
    ...
    if (contributorData && contributorData.value && contributorData.value === UNKNOWN) {
      return {
        value: UNKNOWN,
        status: contributorData.statusSummary,
        isContributing: contributorData ? contributorData.contributing : false
      };
    }
  ...
  private readonly setColumnsToDisplay = (
    ...
    this.props.setColumnsSelected(this.convertColumnsMapToRecord(columnsToDisplay));
  ...
  private readonly getEnvRollup = (
    ...
    if (envContributors && envContributors.length === 0) {
      // When no env contributors, set value so can distinguish N/A vs Unknown
      // NOT_APPLICABLE is used for N/A
      return {
        contributing: true, // Contributing since want the value to be bright and not dim
        statusSummary: SohTypes.SohStatusSummary.NONE,
        type: undefined,
        value: NOT_APPLICABLE,
        valuePresent: false
      };
  ...
  private readonly getStationStats = (
    ...
    // if NOT_APPLICABLE should be N/A
    // if defined and UNKNOWN should be unknown
    // otherwise pass it along
    if (stationAggregate && stationAggregate.value === UNKNOWN) {
      return UNKNOWN;
    }

    if (
      stationAggType === SohTypes.StationAggregateType.ENVIRONMENTAL_ISSUES &&
      !stationAggregate
    ) {
