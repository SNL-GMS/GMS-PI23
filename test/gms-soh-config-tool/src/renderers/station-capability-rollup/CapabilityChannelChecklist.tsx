...
import { setStationCapabilityRollup } from '../../state/station-controls-slice';
import {
...
export const CapabilityChannelChecklist: React.FC<ChannelChecklistProps> = ({
  ...
  const defaultRollup = useAppSelector(
    (store) =>
      store.stationControls.stationCapabilityRollup[stationName ?? ''][
        groupName
      ]
  );
    ...
    (newlySelected: string[], checkbox: string) => {
      dispatch(
        setStationCapabilityRollup({
          stationName,
          groupName,
          rollup: updateStationCapabilityChannels(
            rollupId,
            defaultRollup,
            newlySelected
          ),
      ...
      renderRightElement={(checkbox) =>
        includes(checkedChannels, checkbox) ? (
          ...
          />
            ...
            errors,
            `${AppSections.GROUP} ${groupName} ${AppSections.CHANNEL_CAPABILITY} ${checkbox}`
          ).length > 0 ? (
          <Tooltip
            title={
