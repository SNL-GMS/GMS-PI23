 ...
 */
export const StationCapabilityRollup: React.FC<
  StationCapabilityRollupProps
> = ({ groupName, allChannelNames }: StationCapabilityRollupProps) => {
  ...
  const rollup = useAppSelector(
    (state) =>
      state.stationControls.stationCapabilityRollup[stationName ?? ''][
        groupName
      ]
  );
  ...
  const determineRollupStatus = (rollup: RollupEntry): string => {
    if (rollup && rollup.rollupType === RollupType.ROLLUP_OF_ROLLUPS) {
      return `${rollup.operatorType} ROLLUPS`;
    }
    if (rollup && rollup.channels) {
  ...
  const hasErrors =
    determineSectionContainsErrorState(errors, groupName).length > 0;
        ...
        <AppBar sx={{ position: 'relative' }}>
          <Toolbar>
            <Typography sx={{ ml: 2, flex: 1 }} variant='h6' component='div'>
              {`${stationName}: Station Capability Rollup for Station Group ${groupName}`}
            </Typography>
            <IconButton
              edge='start'
              color='inherit'
