...
import { updateAllStationCapabilityChannels } from '../station-capability-rollup/util';
...
export const ChannelChecklist: React.FC<ChannelChecklistProps> = ({
  ...
  const defaultRollup = useAppSelector(
    (store) => store.stationControls.stationCapabilityRollup
  );
    ...
    batch(() => {
      stationGroupNames.forEach((groupName) => {
        updateStationCapabilityRollup(
          stationName ?? '',
          groupName,
          updateAllStationCapabilityChannels(
