...
/**
 * Updates the isIncluded flag for MonitorTypeConfig
 *
 * @param monitorTypeConfig
 * @param isIncluded
 * @returns updated config
 */
export const updateIsIncludedForMonitorType = (
  monitorTypeConfig: MonitorTypeConfig,
  isIncluded: boolean
): MonitorTypeConfig => {
  return produce(monitorTypeConfig, (draft) => {
...
/**
 * Determines if monitor type is an environmental type
 * @param monitorType string name on monitor type
 * @returns boolean
 */
export const isMonitorAnEnvironmentalType = (monitorType: string) => {
  // PO said its ok to hardcode these for now
  return (
    monitorType !== 'TIMELINESS' &&
    monitorType !== 'LAG' &&
    monitorType !== 'MISSING'
  );
};
