...
const useResolvedMonitorTypesForRollup = (
  ...
  React.useEffect(() => {
    if (monitorTypeConfigs != null && !hasMonitorThresholdsLoaded) {
...
> = (props) => {
  ...
  const stationEnvMonitorsEnabled = useAppSelector(
    (state) => state.stationControls.stationEnvironmental
  )[stationName ?? ''].included;
  ...
  return (
    <>
      ...
      <Accordion
        ...
        title={
          isMonitorAnEnvironmentalType(props.checkboxName) &&
          !stationEnvMonitorsEnabled
            ? 'Disabled due to environmentals being disabled, enable Include Environmentals at top to enable.'
            : ''
        }
        disabled={
          isMonitorAnEnvironmentalType(props.checkboxName) &&
...
export const MonitorTypesRollup: React.FC<MonitorTypesRollupProps> = () => {
  ...
  const stationGroupNames = useAppSelector((state) =>
    state.stationControls.stationGroups[stationName ?? '']?.map(
      (stationGroups) => stationGroups.name
    )
  );
  const [includedStationGroups, setIncludedStationGroups] = React.useState({
    stationGroups: [''],
  });

  const [updateAllChannelCapabilities, setUpdateAllChannelCapabilities] =
    React.useState(true);
  ...
  const updateData = React.useCallback(
    (newlyChecked: string[], checkbox: string) => {
      ...
      if (newlySelectedMonitorTypes != null) {
        ...
        if (updateAllChannelCapabilities) {
        ...
        } else if (!updateAllChannelCapabilities) {
          // If monitor wasn't included and now is, must set it to included so selected groups
          // adding monitor doesn't conflict with logic that monitor must be checked to be checked in
          // channel capability.
          if (
            confirmToggle.newlySelected.find(
              (name) => name === confirmToggle.checkbox
            ) !== undefined
          ) {
            setMonitorTypesForRollup(newlySelectedMonitorTypes);
          }
          updateAllMonitorsForAllCapabilityRollups(
            checkbox,
            newlyChecked,
            includedStationGroups.stationGroups
          );
        }
      } else {
  ...
  React.useEffect(() => {
    if (
      ...
      setIncludedStationGroups({ stationGroups: stationGroupNames });
  ...
  let disabledMonitorsTypes: string[] = [];
  if (!hasChannelCapabilityRollupLoaded) {
    disabledMonitorsTypes = supportedMonitorTypes;
  } else if (hasEnvMonitorTypesLoaded && !stationEnvironmentals.included) {
    // Disable env monitors when not included
    disabledMonitorsTypes = supportedMonitorTypes.filter((monitorType) =>
      isMonitorAnEnvironmentalType(monitorType)
    );
  }

  const handleGroupChecklistToggle = (
    newlySelected: string[],
    checkbox: string
  ) => {
    setIncludedStationGroups({ stationGroups: newlySelected });
  };
  ...
  return (
    <>
      ...
      >
        ...
        <DialogContent>
          <FormControlLabel
            control={<Switch checked={updateAllChannelCapabilities} />}
            label='Toggle monitor and update all channel capabilities for all groups'
            ...
            onChange={(value) => {
              ...
              setUpdateAllChannelCapabilities(castedValue.target.checked);
            }}
          />
          {!updateAllChannelCapabilities ? (
            <>
              <div>{`Groups to ${
                confirmToggle.newlySelected.find(
                  ...
                  ? 'add'
                  : 'remove'
              } ${confirmToggle.checkbox} for channel capabilities`}</div>
              <Checklist
                checkboxes={stationGroupNames}
                checkedBoxes={includedStationGroups.stationGroups}
                nonIdealState={'No station groups found'}
                handleToggle={handleGroupChecklistToggle}
                helpText={(stationGroup) =>
                  `Checkbox results in station ${stationGroup} channel capabilities reflecting monitor toggle`
                }
      ...
      <Checklist
        ...
        disabledCheckboxes={disabledMonitorsTypes}
        disabledText={(monitorName) =>
          !hasChannelCapabilityRollupLoaded
            ...
            : `${monitorName} is disabled due to environmentals not being included for station, enable at the top to toggle`
        }
