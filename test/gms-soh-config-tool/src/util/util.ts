 ...
 * @param stationName name of the station
 * @returns ChannelsPerMonitorForStationParams
 */
...
/**
 * Uses default.json and other default constrain types
 * to determine default station env monitor types
 *
 * @param stationEnvMonitorTypesConfigsFromDisk
 * @param defaultConfig
 * @param stationName
 * @returns default params for station env monitor types
 */
export const determineDefaultStationEnvMonitorTypes = (
  stationEnvMonitorTypesConfigsFromDisk: ConfigurationOption<
    Partial<StationEnvMonitorTypesParams>
  >[],
  defaultConfig:
  ...
  if (!defaultConfig) {
    throw new Error(
      'default.json must exist with name default-station-env-monitor-types'
    );
  }
  if (!stationName) {
    throw new Error('No station selected to update station-env-monitor-types');
  }

  if (!defaultConfig.parameters) {
    throw new Error('default.json must have parameters');
  }

  if (!defaultConfig.parameters.envMonitorTypes) {
    throw new Error('default.json must has envMonitorTypes in parameters');
  }

  let defaultStationEnvMonitorTypes: StationEnvMonitorTypesParams =
    defaultConfig.parameters as unknown as StationEnvMonitorTypesParams;

  stationEnvMonitorTypesConfigsFromDisk.forEach((config) => {
    config.constraints.forEach((constraint) => {
      if (constraint.constraintType === 'DEFAULT') {
        const d = config.constraints.find((c) => c.criterion === 'StationName');
        if (d) {
          if (includes(d.value, stationName)) {
            defaultStationEnvMonitorTypes =
              config.parameters as unknown as StationEnvMonitorTypesParams;
          }
        }
      }
    });
  ...
  return defaultStationEnvMonitorTypes;
};

/**
 * Uses default.json and other default constrain types
 * to determine default station group capability configs per group
 *
 * @param stationGroupCapabilityConfigsFromDisk configs from disk
 ...
 */
export const determineDefaultStationGroupCapabilities = (
  stationGroupCapabilityConfigsFromDisk: ConfigurationOption<
  ...
  if (!defaultConfig) {
    throw new Error(
      'default.json must exist with name default-station-group-capability-rollup'
    );
  }
  if (!defaultConfig.parameters.stationsToGroupRollupOperator) {
    throw new Error(
      'default.json has no parameters default-station-group-capability-rollup'
    );
  }
  if (!stationGroupNames) {
    throw new Error('Must have at least one station group');
  }

  const stationGroupCapabilityRollupDefaults: Record<
    string,
    RollupOperatorOperands
  > = {};
  stationGroupNames.forEach((stationGroupName) => {
    stationGroupCapabilityRollupDefaults[stationGroupName] = defaultConfig
      .parameters.stationsToGroupRollupOperator as RollupOperatorOperands;
    stationGroupCapabilityConfigsFromDisk.forEach((config) => {
      config.constraints.forEach((constraint) => {
        if (constraint.constraintType === 'DEFAULT') {
          ...
          if (groupNameConstraint) {
            if (
              ...
              config.parameters &&
              config.parameters.stationsToGroupRollupOperator
            ) {
              stationGroupCapabilityRollupDefaults[stationGroupName] =
                config.parameters.stationsToGroupRollupOperator;
            }
          }
        }
  ...
  return stationGroupCapabilityRollupDefaults;
};
