 ...
 * @param allStationNames names of all the channels
 * @returns boolean
 */
export const doesStationGroupCapabilityEntryMatchDefault = (
  ...
  // defaults and default overrides(default.json overrides.json) will never have listed stations thus
  // with defaults since no stations are provided it means all are included, so
  // if the lengths do not match it must be different than default
  if (
    entry.stationOperands &&
    allStationNames.length !== entry.stationOperands.length
  ) {
    return false;
  }
