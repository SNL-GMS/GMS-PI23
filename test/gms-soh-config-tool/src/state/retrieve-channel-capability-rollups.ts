...
import { RollupOperatorOperands } from './retrieve-station-capability';
import {
  OperatorType,
  RollupEntry,
  RollupType,
...
const convertQueryEntryToRollup = (
  ...
  let monitors = rollupOperatorOperands.sohMonitorTypeOperands;
  if (
    !rollupOperatorOperands.sohMonitorTypeOperands ||
    rollupOperatorOperands.sohMonitorTypeOperands.length === 0
  ) {
    ...
    rollups: rollupOperatorOperands.rollupOperatorOperands
      ? rollupOperatorOperands.rollupOperatorOperands.map(
          (rollupOperatorOperand) =>
            convertQueryEntryToRollup(rollupOperatorOperand, monitorNames)
        )
      : undefined,
    threshold: {
