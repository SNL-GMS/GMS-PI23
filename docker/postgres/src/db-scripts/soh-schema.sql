...
create index analog_acei_idx
    on channel_env_issue_analog (channel_name, type, start_time desc nulls last, end_time desc nulls last);

create table if not exists channel_env_issue_analog_default partition of channeL_env_issue_analog default;
...
begin
	...
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists channel_env_issue_analog_' || to_char(today, 'YYYY_MM_DD') || ' partition of channel_env_issue_analog for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists channel_env_issue_analog_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of channel_env_issue_analog for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create table if not exists channel_env_issue_boolean_default partition of channel_env_issue_boolean default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists channel_env_issue_boolean_' || to_char(today, 'YYYY_MM_DD') || ' partition of channel_env_issue_boolean for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists channel_env_issue_boolean_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of channel_env_issue_boolean for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
(
	...
	primary key (id, reception_time)
) partition by range(reception_time);

create table if not exists raw_station_data_frame_default partition of raw_station_data_frame default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists raw_station_data_frame_' || to_char(today, 'YYYY_MM_DD') || ' partition of raw_station_data_frame for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists raw_station_data_frame_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of raw_station_data_frame for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
(
	...
	primary key (raw_station_data_frame_id, reception_time, channel_name)
) partition by range(reception_time);

CREATE INDEX rsdf_channel_names_raw_station_data_frame_id_idx ON raw_station_data_frame_channel_names (raw_station_data_frame_id, reception_time);

alter table raw_station_data_frame_channel_names add constraint fk_raw_station_data_frame foreign key (raw_station_data_frame_id, reception_time) references raw_station_data_frame (id, reception_time) on delete cascade;

create table if not exists raw_station_data_frame_channel_names_default partition of raw_station_data_frame_channel_names default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists raw_station_data_frame_channel_names_' || to_char(today, 'YYYY_MM_DD') || ' partition of raw_station_data_frame_channel_names for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists raw_station_data_frame_channel_names_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of raw_station_data_frame_channel_names for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
CREATE INDEX waveform_summary_raw_station_data_frame_id_idx ON waveform_summary (raw_station_data_frame_id, id, reception_time);
create index channel_name_end_time_idx on waveform_summary (id, reception_time, channel_name, end_time);

alter table waveform_summary add constraint fk_raw_station_data_frame_waveform_summary foreign key (raw_station_data_frame_id, reception_time) references raw_station_data_frame (id, reception_time) on delete cascade;

create table if not exists waveform_summary_default partition of waveform_summary default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists waveform_summary_' || to_char(today, 'YYYY_MM_DD') || ' partition of waveform_summary for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists waveform_summary_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of waveform_summary for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create table if not exists station_soh(
	...
	primary key (station_name, id, creation_time)
) partition by range (creation_time);

alter table station_soh add constraint station_soh_unique_station_name_creation_time unique (station_name, id, creation_time);

CREATE INDEX station_soh_creation_time_and_station_name_idx ON station_soh (station_name, id, creation_time);
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists station_soh_' || to_char(today, 'YYYY_MM_DD') || ' partition of station_soh for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists station_soh_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of station_soh for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create table if not exists channel_soh(
	...
	primary key (station_soh_station_name, id, creation_time),
	  foreign key (station_soh_station_name, station_soh_id, creation_time) references station_soh(station_name, id, creation_time) on delete cascade
) partition by range (creation_time);
...
create table if not exists channel_soh_default partition of channel_soh default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists channel_soh_' || to_char(today, 'YYYY_MM_DD') || ' partition of channel_soh for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists channel_soh_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of channel_soh for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create table if not exists station_aggregate_default partition of station_aggregate default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists station_aggregate_' || to_char(today, 'YYYY_MM_DD') || ' partition of station_aggregate for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists station_aggregate_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of station_aggregate for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists channel_soh_monitor_value_status_' || to_char(today, 'YYYY_MM_DD') || ' partition of channel_soh_monitor_value_status for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists channel_soh_monitor_value_status_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of channel_soh_monitor_value_status for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists station_soh_monitor_value_status_' || to_char(today, 'YYYY_MM_DD') || ' partition of station_soh_monitor_value_status for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists station_soh_monitor_value_status_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of station_soh_monitor_value_status for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create table if not exists capability_soh_rollup(
  ...
  station_group_name varchar(255) not null,
  primary key (id, capability_rollup_time)
) partition by range (capability_rollup_time);
...
create table if not exists capability_soh_rollup_default partition of capability_soh_rollup default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists capability_soh_rollup_' || to_char(today, 'YYYY_MM_DD') || ' partition of capability_soh_rollup for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists capability_soh_rollup_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of capability_soh_rollup for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create table if not exists capability_station_soh_uuids(
  ...
  primary key (capability_rollup_id, capability_rollup_time, station_soh_id)
) partition by range (capability_rollup_time);

alter table capability_station_soh_uuids add constraint capability_station_soh_uuids_unique_rollup_station_soh unique (capability_rollup_id, station_soh_id, capability_rollup_time);
alter table capability_station_soh_uuids add constraint fk_capability_station_soh_uuid_capability_rollup foreign key (capability_rollup_id, capability_rollup_time) references capability_soh_rollup (id, capability_rollup_time) on delete cascade;

CREATE INDEX capability_station_soh_uuids_capability_rollup_id_idx on capability_station_soh_uuids (capability_rollup_id);

create table if not exists capability_station_soh_uuids_default partition of capability_station_soh_uuids default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists capability_station_soh_uuids_' || to_char(today, 'YYYY_MM_DD') || ' partition of capability_station_soh_uuids for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists capability_station_soh_uuids_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of capability_station_soh_uuids for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
alter table staged_capability_station_soh_uuids add constraint fk_staged_capability_uuids foreign key (capability_rollup_id, capability_rollup_time) references staged_capability_soh_rollup (id, capability_rollup_time) on delete cascade;
...
create table if not exists capability_station_soh_status_map(
  ...
  station_name varchar(255) not null,
   soh_status public.soh_status_enum not null,
   primary key (capability_rollup_id, capability_rollup_time, station_name)
) partition by range (capability_rollup_time);

alter table capability_station_soh_status_map add constraint capability_ss_status_map_unique_rollup_station unique (capability_rollup_id, capability_rollup_time, station_name);

alter table capability_station_soh_status_map add constraint fk_capability_ss_status_map_capability_rollup foreign key (capability_rollup_id, capability_rollup_time) references capability_soh_rollup (id, capability_rollup_time) on delete cascade;
...
create table if not exists capability_station_soh_status_map_default partition of capability_station_soh_status_map default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists capability_station_soh_status_map_' || to_char(today, 'YYYY_MM_DD') || ' partition of capability_station_soh_status_map for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists capability_station_soh_status_map_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of capability_station_soh_status_map for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
alter table staged_capability_station_soh_status_map add constraint fk_staged_capability_ss_map_capability_rollup foreign key (capability_rollup_id, capability_rollup_time) references staged_capability_soh_rollup (id, capability_rollup_time) on delete cascade;
...
create table if not exists system_message_default partition of system_message default;
...
begin
	today := date_trunc('day', now());
	tomorrow := date_trunc('day', now() + '1 day'::interval);
	execute 'create table if not exists system_message_' || to_char(today, 'YYYY_MM_DD') || ' partition of system_message for values from (''' || today::text || ''') to (''' || tomorrow::text || ''')';
	execute 'create table if not exists system_message_' || to_char(tomorrow, 'YYYY_MM_DD') || ' partition of system_message for values from (''' || tomorrow::text || ''') to (''' || date_trunc('day', tomorrow + '1 days'::interval) || ''')';
end
$$;
...
create or replace function copy_staged_capability_station_soh_uuids ()
	...
	$BODY$
	BEGIN
		insert into gms_soh.capability_station_soh_uuids (capability_rollup_id, capability_rollup_time, station_soh_id)
		values (new.capability_rollup_id, new.capability_rollup_time, new.station_soh_id)
...
create or replace function copy_staged_capability_station_soh_status_map ()
	...
	$BODY$
	BEGIN
		insert into gms_soh.capability_station_soh_status_map (capability_rollup_id, capability_rollup_time, station_name, soh_status)
		values (new.capability_rollup_id, new.capability_rollup_time, new.station_name, new.soh_status)
...
grant select on station to gms_soh_ttl_application;
...
grant select, delete on channel_env_issue_analog to gms_soh_ttl_application;
grant select, delete on channel_env_issue_boolean to gms_soh_ttl_application;
